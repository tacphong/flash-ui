package im
{
   ...
   public class IMController extends EventDispatcher
   {
      ...
      private static var _instance:im.IMController;
       
      
      private var _existChat:Vector.<PresentRecordInfo>;
      
      private var _imview:im.IMView;
      ...
      private var _groupFrame:im.FriendGroupFrame;
      ...
      public static function get Instance() : im.IMController
      {
         if(_instance == null)
         {
            _instance = new im.IMController();
         }
         return _instance;
      }
      ...
      protected function __addCustomHandler(param1:CrazyTankSocketEvent) : void
      {
         ...
         var _loc8_:String = _loc4_.readUTF();
         switch(_loc5_)
         {
            case 1:
               if(_loc6_)
               {
                  MessageTipManager.getInstance().show(LanguageMgr.GetTranslation("IM.addCustom.success",_loc8_));
                  this.customInfo = new CustomInfo();
                  this.customInfo.ID = _loc7_;
                  this.customInfo.Name = _loc8_;
                  dispatchEvent(new IMEvent(IMEvent.ADD_NEW_GROUP));
               }
               else
               {
                  MessageTipManager.getInstance().show(LanguageMgr.GetTranslation("IM.addCustom.fail",_loc8_));
               }
               break;
            case 2:
               if(_loc6_)
               {
                  MessageTipManager.getInstance().show(LanguageMgr.GetTranslation("IM.deleteCustom.success",_loc8_));
                  PlayerManager.Instance.deleteCustomGroup(_loc7_);
                  _loc2_ = 0;
                  while(_loc2_ < PlayerManager.Instance.customList.length)
                  {
                     if(PlayerManager.Instance.customList[_loc2_].ID == _loc7_)
                     {
                        PlayerManager.Instance.customList.splice(_loc2_,1);
                        break;
                     }
                     _loc2_++;
                  }
                  this.deleteCustomID = _loc7_;
                  dispatchEvent(new IMEvent(IMEvent.DELETE_GROUP));
               }
               else
               {
                  MessageTipManager.getInstance().show(LanguageMgr.GetTranslation("IM.deleteCustom.fail",_loc8_));
               }
               break;
            case 3:
               if(_loc6_)
               {
                  ...
                  {
                     if(PlayerManager.Instance.customList[_loc3_].ID == _loc7_)
                     {
                        PlayerManager.Instance.customList[_loc3_].Name = _loc8_;
                        break;
                     }
                     _loc3_++;
                  }
      ...
      private function __privateTalkHandler(param1:CrazyTankSocketEvent) : void
      {
         ...
         var _loc8_:String = _loc4_.readUTF();
         var _loc9_:Boolean = _loc4_.readBoolean();
         var _loc10_:int = 0;
         while(_loc10_ < this._existChat.length)
         {
            if(this._existChat[_loc10_].id == _loc5_)
            {
               _loc2_ = this._existChat[_loc10_];
               _loc2_.addMessage(_loc6_,_loc7_,_loc8_);
               if(_loc6_ != PlayerManager.Instance.Self.NickName)
               {
                  this._existChat.splice(_loc10_,1);
                  this._existChat.unshift(_loc2_);
               }
               break;
            }
            _loc10_++;
         ...
         this.saveInShared(_loc2_);
         this.getMessage();
         this.saveRecentContactsID(_loc2_.id);
         if(this._privateFrame != null && this._privateFrame.parent && this._privateFrame.playerInfo.ID == _loc5_)
         {
            _loc3_ = 0;
            while(_loc3_ < this._existChat.length)
            {
               if(this._existChat[_loc3_].id == _loc5_)
               {
                  this._privateFrame.addMessage(this._existChat[_loc3_].lastMessage);
                  this._existChat[_loc3_].exist = PresentRecordInfo.SHOW;
                  break;
               }
               _loc3_++;
            }
         ...
         {
            this.setExist(_loc5_,PresentRecordInfo.UNREAD);
            this.changeID = _loc5_;
            this.cancelflashState = false;
            dispatchEvent(new Event(HAS_NEW_MESSAGE));
         }
         if(PlayerManager.Instance.Self.playerState.AutoReply != "" && _loc6_ != PlayerManager.Instance.Self.NickName && !_loc9_)
         {
      ...
      {
         var _loc1_:int = 0;
         var _loc2_:Vector.<PresentRecordInfo> = new Vector.<PresentRecordInfo>();
         if(Boolean(this._messageBox))
         {
            _loc1_ = 0;
            while(_loc1_ < this._existChat.length)
            {
               if(this._existChat[_loc1_].exist != PresentRecordInfo.SHOW)
               {
                  _loc2_.push(this._existChat[_loc1_]);
               }
               if(_loc2_.length == MAX_MESSAGE_IN_BOX)
               {
                  break;
               }
               _loc1_++;
            }
            this._messageBox.message = _loc2_;
         }
         return _loc2_;
      }
      ...
      private function __friendResponse(param1:CrazyTankSocketEvent) : void
      {
         ...
         var _loc3_:int = param1.pkg.clientId;
         var _loc4_:int = param1.pkg.readInt();
         var _loc5_:String = param1.pkg.readUTF();
         var _loc6_:Boolean = param1.pkg.readBoolean();
         if(_loc6_)
         {
            _loc2_ = LanguageMgr.GetTranslation("tank.view.im.IMController.sameCityfriend");
            _loc2_ = _loc2_.replace(/r/g,"[" + _loc5_ + "]");
         }
         else
         {
            _loc2_ = "[" + _loc5_ + "]" + LanguageMgr.GetTranslation("tank.view.im.IMController.friend");
         }
      ...
      private function checkFriendExist(param1:String) : Boolean
      {
         ...
         var _loc5_:DictionaryData = PlayerManager.Instance.friendList;
      ...
      {
         var _loc1_:CMFriendInfo = null;
         this._isAddCMFriend = false;
         var _loc2_:DictionaryData = PlayerManager.Instance.CMFriendList;
         var _loc3_:DictionaryData = PlayerManager.Instance.friendList;
         for each(_loc1_ in _loc2_)
         {
            if(_loc1_.IsExist && !_loc3_[_loc1_.UserId])
            {
               SocketManager.Instance.out.sendAddFriend(_loc1_.NickName,0,true);
               _loc2_.remove(_loc1_.UserName);
            }
         }
      }
      ...
      public function getFullRecentContactsID() : String
      {
         var _loc1_:int = 0;
         var _loc2_:String = "";
         for each(_loc1_ in this._recentContactsList)
         {
            if(_loc1_ != 0)
            {
      ...
      public function getRecentContactsStranger() : Array
      {
         ...
         var _loc2_:Array = [];
         for each(_loc1_ in PlayerManager.Instance.recentContacts)
         {
            if(this.testAlikeName(_loc1_.NickName))
            {
               _loc2_.push(_loc1_);
            }
         }
      ...
      public function sortAcademyPlayer(param1:Array) : Array
      {
         ...
         if(_loc5_.getMasterOrApprentices().length <= 0)
         {
            return param1;
         }
         var _loc6_:DictionaryData = _loc5_.getMasterOrApprentices();
         if(_loc5_.getMasterOrApprentices().length > 0)
         {
            for each(_loc2_ in param1)
            {
               if(Boolean(_loc6_[_loc2_.ID]) && _loc2_.ID != _loc5_.ID)
               {
                  if(_loc2_.ID == _loc5_.masterID)
                  {
