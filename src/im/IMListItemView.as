package im
{
   ...
   public class IMListItemView extends Sprite implements IListCell, Disposeable, IDragable, IAcceptDrag
   {
      ...
      private function __itemOut(param1:MouseEvent) : void
      {
         ...
         this._privateChatBtn.visible = false;
         this._snsInviteBtn.visible = false;
         if(Boolean(this._CMFIcon) && this._info.BBSFriends)
         {
            this._CMFIcon.visible = false;
         }
      }
      ...
      private function updateTilte() : void
      {
         ...
         this._titleBG.setFrame(this._info.titleIsSelected ? int(1) : int(3));
      ...
      public function dragStop(param1:DragEffect) : void
      {
         ...
         var _loc4_:FriendListPlayer = param1.data as FriendListPlayer;
         var _loc5_:FriendListPlayer = _loc3_.getCellValue() as FriendListPlayer;
         if(_loc3_ && _loc5_.type == 0 && (_loc5_.titleType >= 10 || _loc5_.titleType == 0))
         {
            if(_loc4_.Relation == 1)
            {
               IMController.Instance.addFriend(_loc4_.NickName);
            }
            else
            {
               _loc2_ = PlayerManager.Instance.hasInFriendList(_loc4_.ID);
               if(_loc2_ || !_loc2_ && !IMController.Instance.isMaxFriend())
               {
                  SocketManager.Instance.out.sendAddFriend(_loc4_.NickName,_loc5_.titleType);
               }
            }
         }
      ...
      {
         if(Boolean(this._info) && this._info.type == 1)
         {
