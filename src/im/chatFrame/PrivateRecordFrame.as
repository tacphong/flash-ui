package im.chatFrame
{
   ...
   public class PrivateRecordFrame extends Frame
   {
      ...
      {
         if(Boolean(SharedManager.Instance.privateChatRecord[param1]))
         {
         ...
         this._totalPage = this._totalPage == 0 ? int(1) : int(this._totalPage);
      ...
      private function showPage(param1:int) : void
      {
         ...
         _loc3_ = _loc3_ < 0 ? int(0) : int(_loc3_);
         _loc4_ = _loc4_ > this._messages.length ? int(this._messages.length) : int(_loc4_);
