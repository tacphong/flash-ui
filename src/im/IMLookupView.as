package im
{
   ...
   public class IMLookupView extends Sprite implements Disposeable
   {
      ...
      private var _currentItem:im.IMLookupItem;
      ...
      private function friendStrTest() : void
      {
         ...
         var _loc3_:im.IMLookupItem = null;
         var _loc4_:im.IMLookupItem = null;
         var _loc5_:int = 0;
         while(_loc5_ < this._currentList.length)
         {
            ...
            _loc1_ = "";
            _loc2_ = "";
            if(this._currentList[_loc5_] is PlayerInfo)
            {
               _loc1_ = (this._currentList[_loc5_] as PlayerInfo).NickName;
               _loc2_ = this._inputText.text;
            }
            else if(this._currentList[_loc5_] is ConsortiaPlayerInfo)
            {
               _loc1_ = (this._currentList[_loc5_] as ConsortiaPlayerInfo).NickName;
               _loc2_ = this._inputText.text;
            }
            if(_loc2_ == "")
            {
            ...
            if(_loc1_.indexOf(this._inputText.text) != -1)
            {
               if(this._currentList[_loc5_] is PlayerInfo)
               {
                  _loc3_ = new im.IMLookupItem(this._currentList[_loc5_] as PlayerInfo);
               ...
               {
                  if(this.testAlikeName((this._currentList[_loc5_] as ConsortiaPlayerInfo).NickName))
                  {
                     _loc4_ = new im.IMLookupItem(this._currentList[_loc5_] as ConsortiaPlayerInfo);
      ...
      private function CMFriendStrTest() : void
      {
         ...
         var _loc3_:im.IMLookupItem = null;
         var _loc4_:int = 0;
         while(_loc4_ < this._currentList.length)
         {
            if(this._itemArray.length >= this.MAX_ITEM_NUM)
            {
               this.setFlexBg();
            ...
            _loc1_ = (this._currentList[_loc4_] as CMFriendInfo).NickName;
            if(_loc1_ == "")
            {
               _loc1_ = (this._currentList[_loc4_] as CMFriendInfo).OtherName;
            }
            _loc2_ = this._inputText.text;
            if(_loc2_ == "")
            {
            ...
            if(_loc1_.indexOf(this._inputText.text) != -1)
            {
               _loc3_ = new im.IMLookupItem(this._currentList[_loc4_] as CMFriendInfo);
               _loc3_.addEventListener(MouseEvent.CLICK,this.__clickHandler);
               this._list.addChild(_loc3_);
               this._itemArray.push(_loc3_);
            }
      ...
      private function setFlexBg() : void
      {
         ...
         else
         {
            ...
            {
               this._bg2.height = this._itemArray.length * this.ITEM_MIN_HEIGHT == 0 ? Number(this.ITEM_MAX_HEIGHT) : Number(this._itemArray.length * this.ITEM_MIN_HEIGHT);
            }
         }
      }
      ...
      public function dispose() : void
      {
         ...
         if(Boolean(this._NAN) && Boolean(this._NAN.bitmapData))
         {
