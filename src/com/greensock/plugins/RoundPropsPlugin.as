package com.greensock.plugins
{
   ...
   public class RoundPropsPlugin extends TweenPlugin
   {
      ...
      protected function _initAllProps() : void
      {
         ...
         var _loc4_:Array = this._tween.vars.roundProps;
         var _loc5_:int = int(_loc4_.length);
         while(--_loc5_ > -1)
         {
            _loc1_ = String(_loc4_[_loc5_]);
            _loc3_ = this._tween.cachedPT1;
            while(Boolean(_loc3_))
            {
               if(_loc3_.name == _loc1_)
               {
                  if(_loc3_.isPlugin)
                  {
                  ...
                  {
                     this.add(_loc3_.target,_loc1_,_loc3_.start,_loc3_.change);
                     this._removePropTween(_loc3_);
               ...
               {
                  _loc2_ = " " + _loc3_.target.overwriteProps.join(" ") + " ";
