package com.greensock.plugins
{
   ...
   public class MotionBlurPlugin extends TweenPlugin
   {
      ...
      override public function onInitTween(param1:Object, param2:*, param3:TweenLite) : Boolean
      {
         ...
         {
            x = Number(Number(this._tween.vars.x) || Number(this._target.x));
            y = Number(Number(this._tween.vars.y) || Number(this._target.y));
      ...
      override public function set changeFactor(param1:Number) : void
      {
         ...
         var _loc8_:Number = this._tween.cachedTime - this._time;
         if(_loc8_ < 0)
         {
            _loc8_ = -_loc8_;
         }
         ...
         var _loc9_:Number = this._target.x - this._xCurrent;
         var _loc10_:Number = this._target.y - this._yCurrent;
         var _loc11_:Number = this._target.x - this._xRef;
         var _loc12_:Number = this._target.y - this._yRef;
         _changeFactor = param1;
         if(_loc11_ > 2 || _loc12_ > 2 || _loc11_ < -2 || _loc12_ < -2)
         {
            this._angle = Math.PI - Math.atan2(_loc12_,_loc11_);
            this._cos = Math.cos(this._angle);
            this._sin = Math.sin(this._angle);
            this._xRef = this._target.x;
            this._yRef = this._target.y;
         }
         this._blur.blurX = Math.sqrt(_loc9_ * _loc9_ + _loc10_ * _loc10_) * this._strength / _loc8_;
         ...
         {
            _loc5_ = this._target.parent.filters;
            if(_loc5_.length != 0)
            {
               this._target.parent.filters = _blankArray;
            }
            this._target.x = this._target.y = 20000;
            _loc6_ = this._target.visible;
            this._target.visible = true;
            _loc7_ = this._target.getBounds(this._target.parent);
            if(_loc7_.width + this._blur.blurX * 2 > 2870)
            {
               this._blur.blurX = _loc7_.width >= 2870 ? Number(0) : Number((2870 - _loc7_.width) * 0.5);
            }
            this._xOffset = 20000 - _loc7_.x + this._padding;
            this._yOffset = 20000 - _loc7_.y + this._padding;
            _loc7_.width += this._padding * 2;
            _loc7_.height += this._padding * 2;
            if(_loc7_.height > this._bdCache.height || _loc7_.width > this._bdCache.width)
            {
               this._bdCache = new BitmapData(_loc7_.width,_loc7_.height,true,16777215);
               this._rectCache = new Rectangle(0,0,this._bdCache.width,this._bdCache.height);
            }
            this._matrix.tx = this._padding - _loc7_.x;
            this._matrix.ty = this._padding - _loc7_.y;
            ...
            this._bdCache.draw(this._target.parent,this._matrix,_ct,"normal",_loc7_,this._smoothing);
            if(Boolean(this._parentMask))
            {
               this._target.parent.mask = this._parentMask;
            }
            this._target.visible = _loc6_;
            this._target.x = this._xCurrent;
            this._target.y = this._yCurrent;
            if(_loc5_.length != 0)
            {
               this._target.parent.filters = _loc5_;
            }
            this._cached = true;
         }
         ...
         if((_loc2_ = this._matrix.a * this._bdCache.width) < 0)
         {
            this._matrix.tx = -_loc2_;
            _loc2_ = -_loc2_;
         }
         if((_loc4_ = this._matrix.c * this._bdCache.height) < 0)
         {
            this._matrix.tx -= _loc4_;
            _loc2_ -= _loc4_;
         }
         else
         {
         ...
         if((_loc3_ = this._matrix.d * this._bdCache.height) < 0)
         {
            this._matrix.ty = -_loc3_;
            _loc3_ = -_loc3_;
         }
         if((_loc4_ = this._matrix.b * this._bdCache.width) < 0)
         {
            this._matrix.ty -= _loc4_;
            _loc3_ -= _loc4_;
         }
         else
         {
         ...
         _loc2_ += this._blur.blurX * 2;
         this._matrix.tx += this._blur.blurX;
         if(_loc2_ > this._bd.width || _loc3_ > this._bd.height)
         {
            this._bd = this._bitmap.bitmapData = new BitmapData(_loc2_,_loc3_,true,16777215);
