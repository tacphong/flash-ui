package com.greensock.plugins
{
   ...
   public class BezierPlugin extends TweenPlugin
   {
      ...
      protected function init(param1:TweenLite, param2:Array, param3:Boolean) : void
      {
         ...
         var _loc7_:Object = param1.vars.isTV == true ? param1.vars.exposedVars : param1.vars;
         if(_loc7_.orientToBezier == true)
         {
            this._orientData = [["x","y","rotation",0,0.01]];
            this._orient = true;
         }
         else if(_loc7_.orientToBezier is Array)
         {
         ...
         {
            for(_loc5_ in param2[_loc4_])
            {
               if(_loc8_[_loc5_] == undefined)
               {
                  _loc8_[_loc5_] = [param1.target[_loc5_]];
               }
               if(typeof param2[_loc4_][_loc5_] == "number")
               {
                  _loc8_[_loc5_].push(param2[_loc4_][_loc5_]);
               }
               else
               {
                  _loc8_[_loc5_].push(param1.target[_loc5_] + Number(param2[_loc4_][_loc5_]));
               }
            }
            _loc4_ += 1;
         }
         for(this.overwriteProps[this.overwriteProps.length] in _loc8_)
         {
            if(_loc7_[_loc5_] != undefined)
            {
               if(typeof _loc7_[_loc5_] == "number")
               {
                  _loc8_[_loc5_].push(_loc7_[_loc5_]);
               }
               else
               {
                  _loc8_[_loc5_].push(param1.target[_loc5_] + Number(_loc7_[_loc5_]));
               }
               _loc6_ = {};
               _loc6_[_loc5_] = true;
         ...
         this._beziers = parseBeziers(_loc8_,param3);
      }
      ...
      override public function set changeFactor(param1:Number) : void
      {
         ...
         if(this._orient)
         {
            ...
            while(Boolean(_loc2_--))
            {
               ...
               _loc12_ = Number(Number(_loc11_[3]) || Number(0));
