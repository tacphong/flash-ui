package com.pickgliss.ui.controls
{
   ...
   public class Silder extends Component implements IOrientable
   {
      ...
      override protected function onProppertiesUpdate() : void
      {
         ...
         if(Boolean(_changedPropeties[P_value]) || Boolean(_changedPropeties[P_thumbShowInnerRect]) || Boolean(_changedPropeties[P_maskShowAreaInnerRect]) || Boolean(_changedPropeties[Component.P_width]) || Boolean(_changedPropeties[Component.P_height]) || Boolean(_changedPropeties[P_maximum]) || Boolean(_changedPropeties[P_minimum]))
         {
            this.updateThumbPos();
            this.updateMask();
            if(Boolean(this._bar))
      ...
      protected function scrollThumbToCurrentMousePosition() : void
      {
         ...
         var _loc7_:Rectangle = this._thumbShowInnerRect.getInnerRect(_width,_height);
         if(this.isVertical())
         {
            _loc1_ = _loc7_.y;
            _loc2_ = _loc7_.y + _loc7_.height;
            _loc3_ = Math.min(_loc2_,Math.max(_loc1_,_loc6_ - this._thumbDownOffset));
         }
         else
         {
            _loc1_ = _loc7_.x;
            _loc2_ = _loc7_.x + _loc7_.width;
            _loc3_ = Math.min(_loc2_,Math.max(_loc1_,_loc5_ - this._thumbDownOffset));
         }
         this.value = this.getValueWithThumbMaxMinPos(_loc1_,_loc2_,_loc3_);
      }
