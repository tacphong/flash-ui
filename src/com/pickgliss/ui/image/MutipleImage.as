package com.pickgliss.ui.image
{
   ...
   public class MutipleImage extends Image
   {
      ...
      override protected function updateSize() : void
      {
         ...
         if(Boolean(_changedPropeties[Component.P_width]) || Boolean(_changedPropeties[Component.P_height]) || Boolean(_changedPropeties[P_imageRect]))
         {
            _loc1_ = 0;
            while(_loc1_ < this._images.length)
            {
               if(Boolean(this._imagesRect) && Boolean(this._imagesRect[_loc1_]))
               {
      ...
      private function creatImages() : void
      {
         ...
         var _loc3_:int = 0;
         while(_loc3_ < this._imageLinks.length)
         {
            _loc2_ = String(this._imageLinks[_loc3_]).split("|");
