package com.pickgliss.ui.image
{
   ...
   public class Image extends Component
   {
      ...
      override protected function onProppertiesUpdate() : void
      {
         ...
         if(Boolean(_changedPropeties[P_reourceLink]))
         {
            this.resetDisplay();
         }
         this.updateSize();
         if(Boolean(_changedPropeties[P_scale9Grid]))
         {
            this.updateScale9Grid();
         }
         if(Boolean(_changedPropeties[P_frameFilters]) || Boolean(_changedPropeties[P_reourceLink]))
         {
            this.setFilters();
         }
      }
      ...
      {
         if(Boolean(this._frameFilter) && Boolean(this._display))
         {
      ...
      {
         if(Boolean(_changedPropeties[Component.P_width]))
         {
            this._display.width = _width;
         }
         if(Boolean(_changedPropeties[Component.P_height]))
         {
