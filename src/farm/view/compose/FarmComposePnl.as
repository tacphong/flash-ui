package farm.view.compose
{
   ...
   public class FarmComposePnl extends Sprite implements Disposeable
   {
      ...
      private var _configmPnl:farm.view.compose.ConfirmComposeAlertFrame;
      
      private var _confirmComposeFoodAlertFrame:farm.view.compose.ConfirmComposeFoodAlertFrame;
      ...
      private function initView() : void
      {
         ...
         while(_loc4_ < 4)
         {
            _loc2_ = ComponentFactory.Instance.creat("asset.farm.baseImage5");
            _loc3_ = ComponentFactory.Instance.creat("farmHouse.composeItem",[_loc2_]);
            PositionUtils.setPos(_loc3_,"farm.houseCompose.point" + _loc4_);
            this._itemComposeVec.push(_loc3_);
            addChild(_loc3_);
            _loc4_++;
         }
         ...
         this._result = new ShopItemCell(_loc5_);
         ...
         _loc6_[AlphaShinerAnimation.COLOR] = EffectColorType.GOLD;
         this._selectComposeBtnShine = EffectManager.Instance.creatEffect(EffectTypes.ALPHA_SHINER_ANIMATION,this._selectComposeBtn,_loc6_);
      ...
      private function setCellInfo(param1:int, param2:ItemTemplateInfo, param3:int = 1) : void
      {
         ...
         {
            this._maxCount = this._maxCount > this._itemComposeVec[param1].maxCount ? int(this._itemComposeVec[param1].maxCount) : int(this._maxCount);
         }
      }
      ...
      private function __configmCount(param1:SelectComposeItemEvent) : void
      {
         ...
         if(Boolean(this._foodID) && Boolean(_loc2_))
         {
            if(PetBagController.instance().haveTaskOrderByID(PetFarmGuildeTaskType.PET_TASK5))
            {
      ...
      {
         var _loc1_:FoodComposeListTemplateInfo = null;
         var _loc2_:int = 0;
         if(this._foodID != 0)
         {
            for each(_loc1_ in FarmComposeHouseController.instance().getComposeDetailByID(this._foodID))
            {
               this.setCellInfo(_loc2_++,ItemManager.Instance.getTemplateById(_loc1_.VegetableID),_loc1_.NeedCount);
            }
            this.upComposeBtn();
         }
      }
