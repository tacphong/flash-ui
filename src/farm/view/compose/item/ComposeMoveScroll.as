package farm.view.compose.item
{
   ...
   public class ComposeMoveScroll extends Sprite implements Disposeable
   {
      ...
      private var _petsImgVec:Vector.<farm.view.compose.item.FarmHouseItem>;
      ...
      {
         var _loc1_:farm.view.compose.item.FarmHouseItem = null;
         ...
         addChild(this._hBox);
         var _loc2_:int = 0;
         while(_loc2_ < this.SHOW_HOUSEITEM_COUNT)
         {
            _loc1_ = new farm.view.compose.item.FarmHouseItem();
            this._petsImgVec[_loc2_] = _loc1_;
      ...
      private function update() : void
      {
         ...
         {
            _loc1_ = this._bag.items.length > this._start + this.SHOW_HOUSEITEM_COUNT ? int(this._start + this.SHOW_HOUSEITEM_COUNT) : int(this._bag.items.length);
            _loc2_ = this._start;
            while(_loc2_ < _loc1_)
            {
