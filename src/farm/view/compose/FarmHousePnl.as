package farm.view.compose
{
   ...
   public class FarmHousePnl extends Sprite implements Disposeable
   {
      ...
      private function initView() : void
      {
         ...
         var _loc2_:int = 0;
         while(_loc2_ < House_ITEM_NUM)
         {
            _loc1_ = new FarmHouseItem(_loc2_);
            this._listView.addChild(_loc1_);
            this._cells.push(_loc1_);
            _loc2_++;
         }
         this._bagdata = PlayerManager.Instance.Self.getBag(BagInfo.VEGETABLE);
         this._totalPage = this._bagdata.items.list.length % House_ITEM_NUM == 0 ? (this._bagdata.items.list.length / House_ITEM_NUM == 0 ? int(1) : int(this._bagdata.items.list.length / House_ITEM_NUM)) : int(this._bagdata.items.list.length / House_ITEM_NUM + 1);
         this._currentPage = 1;
         this.update();
      }
      ...
      private function update() : void
      {
         ...
         var _loc2_:int = this._bagdata.items.list.length < this._currentPage * House_ITEM_NUM ? int(this._bagdata.items.list.length) : int(this._currentPage * House_ITEM_NUM);
