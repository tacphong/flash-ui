package farm.viewx
{
   ...
   public class FarmFriendListView extends Sprite implements Disposeable
   {
      ...
      {
         var _loc2_:FarmFriendListItem = null;
         var _loc3_:FramFriendStateInfo = null;
         var _loc4_:FramFriendStateInfo = FarmModelController.instance.model.friendStateListStolenInfo[FarmModelController.instance.model.currentFarmerId];
         for each(_loc2_ in this._list.list.cell)
         {
            if(Boolean(_loc2_.info))
            {
               if(_loc2_.info.id == _loc4_.id)
               {
                  _loc2_.setCellValue(_loc4_);
                  break;
               }
            }
         }
         for each(_loc3_ in (this._list.list.model as VectorListModel).elements)
         {
            if(_loc3_.id == _loc4_.id)
            {
               _loc3_.landStateVec = _loc4_.landStateVec;
               break;
            }
         }
      }
      ...
      {
         var _loc1_:int = this.isOpen ? int(2) : int(1);
         ...
         this.isOpen = _loc1_ == 1 ? Boolean(true) : Boolean(false);
      }
      ...
      private function getInsertIndex(param1:PlayerInfo) : int
      {
         ...
         while(_loc5_ >= 0)
         {
            _loc2_ = (_loc4_[_loc5_] as FramFriendStateInfo).playerinfo;
            if(!(param1.IsVIP && !_loc2_.IsVIP))
            {
               if(!param1.IsVIP && _loc2_.IsVIP)
               {
                  return _loc5_ + 1;
         ...
         return _loc3_ < 0 ? int(0) : int(_loc3_);
      }
