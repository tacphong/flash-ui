package farm.viewx
{
   ...
   public class ManureOrSeedSelectedView extends Sprite implements Disposeable
   {
      ...
      private var _cells:Vector.<farm.viewx.FarmCell>;
      ...
      private var _currentCell:farm.viewx.FarmCell;
      ...
      private function initView() : void
      {
         ...
         while(_loc2_ < 4)
         {
            this._cells[_loc2_] = new farm.viewx.FarmCell();
            this._cells[_loc2_].addEventListener(MouseEvent.CLICK,this.__clickHandler);
            this._hBox.addChild(this._cells[_loc2_]);
            _loc2_++;
         }
         this.manureVec = new Array();
         var _loc3_:int = 0;
         while(_loc3_ < manureIdArr.length)
         {
            _loc1_ = new InventoryItemInfo();
            _loc1_.TemplateID = manureIdArr[_loc3_];
            ObjectUtils.copyProperties(_loc1_,ItemManager.Instance.getTemplateById(_loc1_.TemplateID));
            this.manureVec.push(_loc1_);
            _loc3_++;
         }
      }
      ...
      private function cellInfos() : void
      {
         var _loc1_:int = 0;
         var _loc2_:int = 0;
         var _loc3_:Array = this._type == SEED ? PlayerManager.Instance.Self.getBag(BagInfo.FARM).findItems(EquipType.SEED) : PlayerManager.Instance.Self.getBag(BagInfo.FARM).findItems(EquipType.MANURE);
         if(this._type == MANURE)
         {
            _loc1_ = 0;
            while(_loc1_ < this.manureVec.length)
            {
               this.manureVec[_loc1_].Count = 0;
               _loc2_ = 0;
               while(_loc2_ < _loc3_.length)
               {
                  if(_loc3_[_loc2_].TemplateID == this.manureVec[_loc1_].TemplateID)
                  {
                     ObjectUtils.copyProperties(this.manureVec[_loc1_],_loc3_[_loc2_]);
                     break;
                  }
                  _loc2_++;
               }
