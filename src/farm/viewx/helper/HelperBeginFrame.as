package farm.viewx.helper
{
   ...
   public class HelperBeginFrame extends BaseAlerFrame
   {
      ...
      public function set needCount(param1:int) : void
      {
         ...
         {
            _loc2_ = _loc3_[_loc4_].TemplateID;
            if(this._seedID == _loc2_)
            {
               this._needMoney = this._needCount * _loc3_[_loc4_].AValue1;
               this._moneyType = _loc3_[_loc4_].APrice1;
               if(this._needCount * _loc3_[_loc4_].getItemPrice(1).giftValue > 0)
               {
                  this._isDDTMoney = true;
                  this._moneyTypeText = LanguageMgr.GetTranslation("ddtMoney");
               }
               if(this._needCount * _loc3_[_loc4_].getItemPrice(1).moneyValue > 0)
               {
