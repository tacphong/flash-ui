package farm.viewx.helper
{
   ...
   public class HelperItem extends Sprite
   {
      ...
      private var _helperSetView:farm.viewx.helper.HelperSetView;
      ...
      private function setSeedPanelData() : void
      {
         ...
         var _loc2_:VectorListModel = this._cbxSeed.listPanel.vectorListModel;
         _loc2_.clear();
         _loc2_.append(this._selectTipMsg);
         var _loc3_:Vector.<ShopItemInfo> = ShopManager.Instance.getValidGoodByType(ShopType.FARM_SEED_TYPE);
         var _loc4_:int = 0;
         while(_loc4_ < _loc3_.length)
         {
            _loc1_ = _loc3_[_loc4_];
            this._seedInfos[_loc1_.TemplateInfo.Name] = _loc1_.TemplateInfo.TemplateID;
            _loc2_.append(_loc1_.TemplateInfo.Name);
            _loc4_++;
         }
      ...
      private function setFertilizerPanelData() : void
      {
         ...
         var _loc2_:VectorListModel = this._cbxFertilizer.listPanel.vectorListModel;
         _loc2_.clear();
         _loc2_.append(this._selectTipMsg);
         ...
         {
            _loc1_ = _loc3_[_loc4_];
            this._fertilizerInfos[_loc1_.TemplateInfo.Name] = _loc1_.TemplateInfo.TemplateID;
            _loc2_.append(_loc1_.TemplateInfo.Name);
            _loc4_++;
         }
      ...
      private function __seedCheck() : Boolean
      {
         ...
         {
            _loc1_ = int(this._seedInfos[this._SeedTxt.text]);
            _loc2_ = PlayerManager.Instance.Self.getBag(BagInfo.FARM).findItems(EquipType.SEED);
            for each(_loc3_ in _loc2_)
            {
               if(_loc3_.TemplateID == _loc1_)
               {
                  if(_loc3_.Count > 0)
                  {
                     _loc4_ = true;
      ...
      private function __fertilizerItemClickCheck() : Boolean
      {
         ...
         {
            _loc1_ = int(this._fertilizerInfos[this._cbxFertilizer.textField.text]);
            _loc2_ = PlayerManager.Instance.Self.getBag(BagInfo.FARM).findItems(EquipType.MANURE);
            for each(_loc3_ in _loc2_)
            {
               if(_loc3_.TemplateID == _loc1_)
               {
      ...
      public function get getItemData() : Object
      {
         ...
         while(_loc6_ < _loc5_.length)
         {
            ...
            this._fertilizerInfos[_loc2_.TemplateInfo.Name] = _loc2_.TemplateInfo.TemplateID;
            _loc6_++;
         }
         var _loc7_:Object = {};
         _loc7_.currentfindIndex = this.currentfindIndex;
         if(this.currentSeed == this._selectTipMsg)
         {
            _loc7_.currentSeedText = 0;
         }
         else
         {
            _loc7_.currentSeedText = this._seedInfos[this.currentSeed];
         }
         if(this.currentFertilizer == this._selectTipMsg)
         {
            _loc7_.currentFertilizerText = 0;
         }
         else
         {
            _loc7_.currentFertilizerText = this._fertilizerInfos[this.currentFertilizer];
         }
         _loc7_.currentSeedNum = this.currentSeedNum;
         _loc7_.autoFertilizerNum = this.currentFertilizerNum;
         return _loc7_;
      }
      ...
      public function onKeyStart() : Boolean
      {
         ...
         {
            _loc2_ = int(this._seedInfos[this._SeedTxt.text]);
         }
         var _loc3_:int = 0;
         if(this._fertilizerInfos.hasOwnProperty(this._FertilizerTxt.text))
         {
            _loc3_ = int(this._fertilizerInfos[this._FertilizerTxt.text]);
         }
         if(_loc2_ > 0)
         {
            return true;
