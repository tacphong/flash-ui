package farm.viewx.helper
{
   ...
   public class SeedSelect extends ChatBasePanel implements Disposeable
   {
      ...
      private var _itemList:Vector.<farm.viewx.helper.HelperSetItem>;
      ...
      {
         var _loc1_:farm.viewx.helper.HelperSetItem = null;
         var _loc2_:Vector.<ShopItemInfo> = ShopManager.Instance.getValidGoodByType(ShopType.FARM_SEED_TYPE);
         var _loc3_:int = 0;
         while(_loc3_ < _loc2_.length)
         {
            _loc1_ = new farm.viewx.helper.HelperSetItem();
            _loc1_.info = _loc2_[_loc3_];
            _loc1_.addEventListener(SelectComposeItemEvent.SELECT_SEED,this.__itemClick);
