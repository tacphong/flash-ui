package ddt.data.player
{
   ...
   public class SelfInfo extends PlayerInfo
   {
      ...
      public function SelfInfo()
      {
         ...
         _isSelf = true;
         this._dungeonCount = new DictionaryData();
      }
      ...
      {
         if(Boolean(this._questList) && Boolean(this._questList[param1]))
         {
            return true;
         }
         return false;
      ...
      public function sendOverTimeListByBody() : void
      {
         ...
         var _loc3_:Array = PlayerManager.Instance.Self.Bag.findOvertimeItemsByBodyII();
         for each(_loc1_ in _loc3_)
         {
            if(_loc1_.CategoryID == 50 || _loc1_.CategoryID == 51 || _loc1_.CategoryID == 52)
            {
               if(PlayerManager.Instance.Self.pets.length > 0)
               {
                  ...
                  {
                     SocketManager.Instance.out.delPetEquip(PlayerManager.Instance.Self.pets[_loc2_].Place,_loc1_.Place);
                  }
               }
               return;
            }
            SocketManager.Instance.out.sendItemOverDue(_loc1_.BagType,_loc1_.Place);
         }
      }
