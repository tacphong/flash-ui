package ddt.data.goods
{
   ...
   public class ItemPrice
   {
      ...
      public function get IsMixed() : Boolean
      {
         var _loc1_:* = null;
         var _loc2_:int = 0;
         for(_loc1_ in this._prices)
         {
            if(this._prices[_loc1_] > 0)
            {
               _loc2_++;
            }
         }
      ...
      public function toString() : String
      {
         ...
         if(this.moneyValue > 0)
         {
            _loc2_ += this.moneyValue.toString() + Price.MONEYTOSTRING;
         }
         if(this.goldValue > 0)
         {
            _loc2_ += this.goldValue.toString() + Price.GOLDTOSTRING;
         }
         if(this.gesteValue > 0)
         {
            _loc2_ += this.gesteValue.toString() + Price.GESTETOSTRING;
         }
         if(this.giftValue > 0)
         {
            _loc2_ += this.giftValue.toString() + Price.GIFTTOSTRING;
         }
         if(this.ddtPetScoreValue > 0)
         {
            _loc2_ += this.ddtPetScoreValue.toString() + Price.PETSCORETOSTRING;
         }
         for(_loc1_ in this._prices)
         {
            if(_loc1_ != Price.MONEYTOSTRING && _loc1_ != Price.GOLDTOSTRING && _loc1_ != Price.GESTETOSTRING && _loc1_ != Price.GIFTTOSTRING && _loc1_ != Price.PETSCORETOSTRING)
            {
               _loc2_ += this._prices[_loc1_].toString() + _loc1_;
            }
         }
         return _loc2_;
      }
