package ddt.data.analyze
{
   ...
   public class UserBoxInfoAnalyzer extends DataAnalyzer
   {
      ...
      private function parseShop() : void
      {
         ...
         var _loc4_:TimeBoxInfo = null;
         var _loc5_:int = 0;
         while(_loc5_ < this._goodsList.length())
         {
            _loc1_ = int(this._goodsList[_loc5_].@Type);
            switch(_loc1_)
            {
               case 0:
                  _loc2_ = new TimeBoxInfo();
                  ObjectUtils.copyPorpertiesByXML(_loc2_,this._goodsList[_loc5_]);
                  this.boxTemplateID[_loc2_.TemplateID] = _loc2_.TemplateID;
                  this.timeBoxList.add(_loc2_.ID,_loc2_);
                  break;
               case 1:
                  _loc3_ = new GradeBoxInfo();
                  ObjectUtils.copyPorpertiesByXML(_loc3_,this._goodsList[_loc5_]);
                  this.boxTemplateID[_loc3_.TemplateID] = _loc3_.TemplateID;
                  this.gradeBoxList.add(_loc3_.ID,_loc3_);
                  break;
               case 2:
                  _loc4_ = new TimeBoxInfo();
                  ObjectUtils.copyPorpertiesByXML(_loc4_,this._goodsList[_loc5_]);
