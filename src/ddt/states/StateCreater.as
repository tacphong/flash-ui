   ...
   public class StateCreater implements IStateCreator
   {
      ...
      public function createAsync(param1:String, param2:Function) : void
      {
         ...
         else
         {
            ...
            _loc3_ = 0;
            while(_loc3_ < _loc4_.neededUIModule.length)
            {
               UIModuleLoader.Instance.addUIModuleImp(_loc4_.neededUIModule[_loc3_],param1);
               _loc3_++;
            }
         }
      }
      ...
      {
         var _loc2_:BaseStateView = null;
         if(StringUtils.isEmpty(param1.state))
         {
            return;
         }
         var _loc3_:StateLoadingInfo = this.getStateLoadingInfo(param1.state);
         if(_loc3_.completeedUIModule.indexOf(param1.module) == -1)
         {
            _loc3_.completeedUIModule.push(param1.module);
         }
         var _loc4_:Boolean = true;
         var _loc5_:int = 0;
         while(_loc5_ < _loc3_.neededUIModule.length)
         {
            if(_loc3_.completeedUIModule.indexOf(_loc3_.neededUIModule[_loc5_]) == -1)
            {
               _loc4_ = false;
            }
            _loc5_++;
         }
         _loc3_.isComplete = _loc4_;
         if(_loc3_.isComplete && this._currentStateType == _loc3_.state)
         {
            UIModuleLoader.Instance.removeEventListener(UIModuleEvent.UI_MODULE_COMPLETE,this.__onUimoduleLoadComplete);
            UIModuleLoader.Instance.removeEventListener(UIModuleEvent.UI_MODULE_PROGRESS,this.__onUimoduleLoadProgress);
            UIModuleSmallLoading.Instance.hide();
            _loc2_ = this.create(param1.state);
      ...
      public function getNeededUIModuleByType(param1:String) : String
      {
         ...
         {
            return UIModuleTypes.PYRAMID + "," + UIModuleTypes.SHOP;
         }
         return "";
      }
   }
