package ddt.view.chat
{
   ...
   public class ChatInputView extends Sprite
   {
      ...
      private var _channelPanel:ddt.view.chat.ChatChannelPanel;
      
      private var _channelState:ScaleFrameImage;
      
      private var _faceBtn:BaseButton;
      
      private var _facePanel:ddt.view.chat.ChatFacePanel;
      
      private var _fastReplyBtn:BaseButton;
      
      private var _fastReplyPanel:ddt.view.chat.ChatFastReplyPanel;
      
      private var _friendListBtn:BaseButton;
      
      private var _friendListPanel:ddt.view.chat.ChatFriendListPanel;
      
      private var _inputField:ddt.view.chat.ChatInputField;
      
      private var _lastRecentSendTime:int = -30000;
      
      private var _lastSendChatTime:int = -30000;
      
      private var _chatPrivateFrame:ddt.view.chat.ChatPrivateFrame;
      ...
      public function get fastReplyPanel() : ddt.view.chat.ChatFastReplyPanel
      {
      ...
      public function set channel(param1:int) : void
      {
         ...
         this._channelState.setFrame(this._channel == ChatInputView.WORLDBOSS_ROOM ? int(int(5 + 1)) : int(int(this._channel + 1)));
      ...
      public function getCurrentInputChannel() : int
      {
         ...
         switch(ChatManager.Instance.state)
         {
            ...
            case ChatManager.CHAT_LITTLEGAME:
               _loc1_ = int(HOTSPRING_ROOM);
               break;
            case ChatManager.CHAT_ACADEMY_VIEW:
               _loc1_ = int(CURRENT);
               break;
            case ChatManager.CHAT_WORLDBOS_ROOM:
               _loc1_ = int(WORLDBOSS_ROOM);
         }
         return _loc1_;
      }
      
      public function get inputField() : ddt.view.chat.ChatInputField
      {
      ...
      private function __onInputTextChanged(param1:ChatEvent) : void
      {
         ...
         _loc3_.channel = this.channelII = this.getCurrentInputChannel();
         ...
         _loc3_.receiver = this._inputField.privateChatName;
         _loc3_.sender = ChatFormats.replaceUnacceptableChar(_loc3_.sender);
         _loc3_.receiver = ChatFormats.replaceUnacceptableChar(this._inputField.privateChatName);
         if(this.checkCanSendChannel(_loc3_))
         {
            _loc2_ = false;
            _loc2_ = _loc3_.channel == CROSS_BUGLE || _loc3_.channel == BIG_BUGLE || _loc3_.channel == SMALL_BUGLE || this.checkCanSendTime();
            if(_loc2_)
            {
               ChatManager.Instance.sendChat(_loc3_);
               if(_loc3_.channel != BIG_BUGLE && _loc3_.channel != SMALL_BUGLE && _loc3_.channel != CROSS_BUGLE)
               {
