package ddt.view.chat
{
   ...
   public class ChatInputField extends Sprite
   {
      ...
      public function sendCurrnetText() : void
      {
         ...
         var _loc5_:RegExp = /\/\S*\s?/;
         var _loc6_:Array = _loc5_.exec(this._inputField.text);
         var _loc7_:String = this._inputField.text.toLocaleLowerCase();
         ...
         {
            _loc1_ = 0;
            while(_loc1_ < this.CHANNEL_KEY_SET.length)
            {
               if(_loc7_.indexOf("/" + this.CHANNEL_KEY_SET[_loc1_]) == 0)
               {
                  _loc8_ = true;
                  SoundManager.instance.play("008");
                  this._inputField.text = _loc7_.substring(2);
                  dispatchEvent(new ChatEvent(ChatEvent.INPUT_CHANNEL_CHANNGED,CHANNEL_SET[_loc1_]));
               }
               _loc1_++;
            }
            if(!_loc8_)
            {
               _loc2_ = ChatManager.Instance.model.customFastReply;
               _loc1_ = 0;
               while(_loc1_ < 5)
               {
                  if(_loc7_.indexOf("/" + String(_loc1_ + 1)) == 0 && (_loc7_.length == 2 || _loc7_.charAt(2) == " "))
                  {
                     _loc9_ = true;
                     if(_loc2_.length > _loc1_)
                     {
                        this._inputField.text = _loc2_[_loc1_];
                     }
                     else
                     {
            ...
            if(_loc6_ && !_loc8_ && !_loc9_)
            {
               _loc3_ = String(_loc6_[0]).replace(" ","");
               _loc3_ = _loc3_.replace("/","");
               if(_loc3_ == "")
               {
                  return;
               }
               this._inputField.text = this._inputField.text.replace(_loc5_,"");
         ...
         if(_loc7_.substr(0,2) != "/" + this.CHANNEL_KEY_SET[2])
         {
            _loc4_ = this.parasMsgs(this._inputField.text);
            this._inputField.text = "";
            if(_loc4_ == "")
            {
               return;
            }
            dispatchEvent(new ChatEvent(ChatEvent.INPUT_TEXT_CHANGED,_loc4_));
         }
      }
