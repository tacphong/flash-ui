package ddt.view.caddyII
{
   ...
   public class CaddyBagView extends Sprite implements Disposeable
   {
      ...
      private var _items:Vector.<ddt.view.caddyII.CaddyCell>;
      ...
      {
         var _loc1_:ddt.view.caddyII.CaddyCell = null;
         ...
         var _loc2_:uint = 0;
         while(_loc2_ < SUM_NUMBER)
         {
            _loc1_ = new ddt.view.caddyII.CaddyCell(_loc2_);
            this._items.push(_loc1_);
            _loc1_.addEventListener(CellEvent.ITEM_CLICK,this.__itemClick);
      ...
      public function getSellAllPriceString() : String
      {
         ...
         var _loc2_:Number = 0;
         var _loc3_:Number = 0;
         for each(_loc1_ in CaddyModel.instance.bagInfo.items)
         {
            if(_loc1_.ReclaimType == 1)
            {
               _loc2_ += _loc1_.ReclaimValue;
            }
            else if(_loc1_.ReclaimType == 2)
            {
               _loc3_ += _loc1_.ReclaimValue;
            }
         }
         return (_loc2_ > 0 ? _loc2_ + LanguageMgr.GetTranslation("tank.hotSpring.gold") : "") + (_loc2_ > 0 && _loc3_ > 0 ? "," : "") + (_loc3_ > 0 ? _loc3_ + LanguageMgr.GetTranslation("tank.gameover.takecard.gifttoken") : "");
      }
      ...
      public function _update(param1:BagEvent) : void
      {
         ...
         var _loc4_:CaddyEvent = null;
         var _loc5_:Dictionary = param1.changedSlots;
         for(_loc2_ in _loc5_)
         {
            _loc3_ = CaddyModel.instance.bagInfo.getItemAt(int(_loc2_));
            if(Boolean(_loc3_))
            {
               this._selectedGoodsInfo = _loc3_;
               this._selectPlace = _loc3_.Place;
               _loc4_ = new CaddyEvent(GET_GOODSINFO);
               _loc4_.info = this._selectedGoodsInfo;
               dispatchEvent(_loc4_);
            }
            else
            {
               this._items[_loc2_].info = null;
            }
         }
      }
      ...
      {
         SoundManager.instance.play("008");
         var _loc2_:ddt.view.caddyII.CaddyCell = param1.data as CaddyCell;
