package ddt.view
{
   ...
   public class ColorEditor extends Sprite implements Disposeable
   {
      ...
      {
         var _loc1_:ColorItem = null;
         var _loc2_:ColorItem = null;
         var _loc3_:int = 0;
         while(_loc3_ < this._colors.length)
         {
            _loc1_ = ComponentFactory.Instance.creatComponentByStylename("shop.ColorItem");
            _loc1_.setup(this._colors[_loc3_]);
            this._colorsArr.push(_loc1_);
            this._colorlist.addChild(_loc1_);
            this._ciGroup.addSelectItem(_loc1_);
            _loc1_.addEventListener(MouseEvent.MOUSE_DOWN,this.__colorItemClick);
            _loc3_++;
         }
         var _loc4_:int = 0;
         while(_loc4_ < this._skins.length)
         {
            _loc2_ = ComponentFactory.Instance.creatComponentByStylename("shop.ColorItem");
            _loc2_.setup(this._skins[_loc4_]);
            this._skinsArr.push(_loc2_);
            this._skincolorlist.addChild(_loc2_);
            this._siGroup.addSelectItem(_loc2_);
            _loc2_.addEventListener(MouseEvent.MOUSE_DOWN,this.__skinItemClick);
            _loc4_++;
         }
      }
