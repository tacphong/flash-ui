package ddt.view
{
   ...
   public class NovicePlatinumCard extends Frame
   {
      ...
      public function setup() : void
      {
         ...
         var _loc3_:Array = ActiveEventsManager.Instance.model.list;
         if(_loc3_ == null)
         {
            this.dispose();
            return;
         ...
         {
            if(_loc3_[_loc4_].Type == 10)
            {
               this._activeEventsInfo = _loc3_[_loc4_] as ActiveEventsInfo;
            }
            _loc4_++;
         }
         if(Boolean(this._activeEventsInfo))
         {
            ...
            _loc2_ = LoaderManager.Instance.creatLoader(PathManager.solveRequestPath("UserGetActiveState.ashx"),BaseLoader.REQUEST_LOADER,_loc1_);
            _loc2_.addEventListener(LoaderEvent.COMPLETE,this.__loadComplete);
            _loc2_.addEventListener(LoaderEvent.LOAD_ERROR,this.__loadError);
