package ddt.view.tips
{
   ...
   public class MarriedTip extends OneLineTip
   {
      ...
      private function fitTextWidth() : void
      {
         ...
         var _loc4_:int = _loc3_.x + width - 1000;
         if(_loc4_ > 0)
         {
            _loc1_ = _contentTxt.text;
            _contentTxt.text = _loc1_.substring(0,_loc1_.length - 3);
            _loc2_ = _contentTxt.getCharIndexAtPoint(_contentTxt.width - _loc4_ - 20,5);
            _contentTxt.text = _contentTxt.text.substring(0,_loc2_) + "..." + _loc1_.substr(_loc1_.length - 3);
         }
         _contentTxt.setTextFormat(this._nickNameTF,0,_contentTxt.length - 3);
         updateTransform();
      }
