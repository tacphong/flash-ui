package ddt.view.academyCommon.academyIcon
{
   ...
   public class AcademyIconTip extends Sprite implements Disposeable, ITip
   {
      ...
      private function update() : void
      {
         ...
         var _loc2_:int = 0;
         var _loc3_:int = 0;
         var _loc4_:Boolean = PlayerManager.Instance.Self.ID == this._tipData.ID;
         ...
         {
            _loc1_ = 0;
            while(_loc1_ <= (this._tipData.getMasterOrApprentices().length >= 3 ? 2 : this._tipData.getMasterOrApprentices().length))
            {
               if(Boolean(this._tipData.getMasterOrApprentices().list[_loc1_]) && this._tipData.getMasterOrApprentices().list[_loc1_] != "")
               {
                  this._textFrameArray[_loc1_].text = LanguageMgr.GetTranslation("ddt.view.academyCommon.academyIcon.AcademyIconTip.master",this._tipData.getMasterOrApprentices().list[_loc1_]);
                  this._textFrameArray[_loc1_].setTextFormat(this._contentLabel,0,this._tipData.getMasterOrApprentices().list[_loc1_].length);
                  this._textFrameArray[_loc1_].visible = true;
               }
               else
               {
                  _loc2_ = 3 - this._tipData.getMasterOrApprentices().length;
                  this._textFrameArray[_loc1_].text = LanguageMgr.GetTranslation("ddt.view.academyCommon.academyIcon.AcademyIconTip.masterExplanation",_loc2_);
                  this._textFrameArray[_loc1_].visible = true;
               }
               _loc1_++;
            }
         ...
         else
         {
            if(this._tipData.Grade >= AcademyManager.ACADEMY_LEVEL_MIN)
            {
               if(!_loc4_)
               {
                  ...
                  this._textFrameArray[0].text = LanguageMgr.GetTranslation("ddt.view.academyCommon.academyIcon.AcademyIconTip.masterExplanation",_loc3_);
                  this._textFrameArray[0].visible = true;
               }
            }
            else if(!_loc4_)
