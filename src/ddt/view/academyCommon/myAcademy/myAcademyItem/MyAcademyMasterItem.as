package ddt.view.academyCommon.myAcademy.myAcademyItem
{
   ...
   public class MyAcademyMasterItem extends Sprite implements Disposeable
   {
      ...
      {
         if(Boolean(this._info) && (param1.data as PlayerInfo).ID == this._info.ID)
         {
      ...
      protected function __removeClick(param1:MouseEvent) : void
      {
         ...
         {
            _loc4_.data = LanguageMgr.GetTranslation("ddt.view.academyCommon.myAcademy.MyAcademyApprenticeItem.remove",this._info.NickName);
            _loc2_ = AlertManager.Instance.alert("academySimpleAlert",_loc4_,LayerManager.ALPHA_BLOCKGOUND);
            _loc2_.addEventListener(FrameEvent.RESPONSE,this.__frameEvent);
         }
         else
         {
            _loc4_.data = LanguageMgr.GetTranslation("ddt.view.academyCommon.myAcademy.MyAcademyApprenticeItem.removeII",this._info.NickName);
            _loc3_ = AlertManager.Instance.alert("academySimpleAlert",_loc4_,LayerManager.ALPHA_BLOCKGOUND);
            _loc3_.addEventListener(FrameEvent.RESPONSE,this.__alerFrameEvent);
         }
      }
      ...
      protected function getTimerOvertop() : Boolean
      {
         ...
         return _loc3_ > 72 ? Boolean(true) : Boolean(false);
      }
