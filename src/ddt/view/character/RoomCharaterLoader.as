package ddt.view.character
{
   ...
   public class RoomCharaterLoader extends BaseCharacterLoader
   {
      ...
      override protected function drawCharacter() : void
      {
         ...
         this._suit = new BitmapData(_loc3_ * 4,_loc4_,true,0);
         if(Boolean(this._faceUpBmd))
         {
            this._faceUpBmd.dispose();
         }
         this._faceUpBmd = new BitmapData(_loc3_,_loc4_,true,0);
         if(Boolean(this._faceBmd))
         {
            this._faceBmd.dispose();
         }
         this._faceBmd = new BitmapData(_loc3_ * 4,_loc4_,true,0);
         if(_info.getShowSuits())
         {
            ...
            if(_info.WeaponID != 0 && _info.WeaponID != -1 && this.showWeapon)
            {
               ...
               _loc2_.draw(_layers[7].getContent());
               _loc1_.graphics.beginBitmapFill(_loc2_,null,true,true);
               _loc1_.graphics.drawRect(0,0,_loc3_ * 4,_loc4_);
               _loc1_.graphics.endFill();
               this._suit.draw(_loc1_,null,null,BlendMode.NORMAL);
               _loc2_.dispose();
            }
         }
         else
