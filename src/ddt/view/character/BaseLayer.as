package ddt.view.character
{
   ...
   public class BaseLayer extends Sprite implements ILayer
   {
      ...
      protected function initColors(param1:String) : void
      {
         ...
         var _loc5_:int = 0;
         while(_loc5_ < this._bitmaps.length)
         {
            if(Boolean(this._bitmaps[_loc5_]))
            {
               addChild(this._bitmaps[_loc5_]);
               this._bitmaps[_loc5_].visible = false;
            }
            _loc5_++;
         }
         ...
         if(this._colors.length == this._bitmaps.length)
         {
            ...
            {
               if(!StringHelper.isNullOrEmpty(this._colors[_loc2_]) && this._colors[_loc2_].toString() != "undefined" && this._colors[_loc2_].toString() != "null")
               {
                  if(this._bitmaps[_loc2_] != null)
                  {
                     this._bitmaps[_loc2_].visible = true;
                     this._bitmaps[_loc2_].transform.colorTransform = BitmapUtils.getColorTransfromByColor(this._colors[_loc2_]);
                     _loc3_ = BitmapUtils.getHightlightColorTransfrom(this._colors[_loc2_]);
                     _loc4_ = new Bitmap(this._bitmaps[_loc2_].bitmapData,"auto",true);
                     if(Boolean(_loc3_))
                     {
                        _loc4_.transform.colorTransform = _loc3_;
                     }
                     _loc4_.blendMode = BlendMode.HARDLIGHT;
                     addChild(_loc4_);
                  }
               }
               else if(this._bitmaps[_loc2_] != null)
               {
                  this._bitmaps[_loc2_].transform.colorTransform = new ColorTransform();
               }
               _loc2_++;
            }
         }
      ...
      final public function load(param1:Function) : void
      {
      ...
      public function reSetBitmap() : void
      {
         ...
         while(_loc1_ < this._queueLoader.loaders.length)
         {
            ...
            if(Boolean(this._bitmaps[_loc1_]))
            {
