   ...
   public class GameCharacter extends BaseCharacter
   {
      ...
      protected function CreateCryFrace(param1:String) : void
      {
         ...
         if(this._colors.length == this._cryBmps.length)
         {
            ...
            {
               if(!StringHelper.isNullOrEmpty(this._colors[_loc3_]) && this._colors[_loc3_].toString() != "undefined" && this._colors[_loc3_].toString() != "null" && Boolean(this._cryBmps[_loc3_]))
               {
               ...
               else if(Boolean(this._cryBmps[_loc3_]))
               {
      ...
      private function clearBomded() : void
      {
         ...
         if(Boolean(this.blackBm.parent))
         {
            removeChild(this.blackBm);
         }
         addChild(_body);
      ...
      override protected function setContent() : void
      {
         ...
         if(_loader != null)
         {
            ...
            if(_info.getShowSuits())
            {
               if(Boolean(this._normalSuit) && this._normalSuit != _loc1_[6])
               {
                  this._normalSuit.dispose();
               }
               this._normalSuit = _loc1_[6];
               if(Boolean(this._lackHpSuit) && this._lackHpSuit != _loc1_[7])
               {
            ...
            {
               if(Boolean(this._spBitmapData) && this._spBitmapData != _loc1_[1])
               {
               ...
               if(Boolean(this._faceupBitmapData) && this._faceupBitmapData != _loc1_[2])
               {
                  this._faceupBitmapData.dispose();
               }
               this._faceupBitmapData = _loc1_[2];
               if(Boolean(this._faceBitmapData) && this._faceBitmapData != _loc1_[3])
               {
                  this._faceBitmapData.dispose();
               }
               this._faceBitmapData = _loc1_[3];
               if(Boolean(this._lackHpFaceBitmapdata) && this._lackHpFaceBitmapdata != _loc1_[4])
               {
               ...
               if(Boolean(this._faceDownBitmapdata) && this._faceDownBitmapdata != _loc1_[5])
               {
      ...
      {
         var _loc2_:int = 0;
         var _loc3_:Point = new Point(_characterWidth * 11 - _characterWidth / 2,_characterHeight * 3 - _characterHeight / 2);
         var _loc4_:int = _loc3_.x - 5;
         while(_loc4_ < _loc3_.x + 5)
         {
            _loc2_ = _loc3_.y - 5;
            while(_loc2_ < _loc3_.y + 5)
            {
               if(param1.getPixel(_loc4_,_loc2_) != 0)
               {
                  return true;
               }
               _loc2_++;
      ...
      private function drawSoul() : void
      {
         ...
         if(_info.getShowSuits())
         {
            ...
            while(_loc1_ < 4)
            {
               _loc8_.x = _characterWidth * _loc1_;
               this._soulFace.copyPixels(this._lackHpSuit,_frames[36],_loc8_,null,null,true);
               _loc1_++;
            }
         }
         else
         {
            ...
            _loc2_ = 0;
            _loc3_ = new Matrix();
            _loc4_ = new BitmapData(this._faceBitmapData.width,this._faceBitmapData.height,true,0);
            _loc5_ = 0;
            while(_loc5_ < 4)
            {
               _loc8_.x = _characterWidth * _loc5_;
               switch(_loc5_)
               {
                  case 0:
                     _loc2_ = 0;
               ...
               _loc8_.x = _characterWidth * _loc5_;
               this._soulFace.copyPixels(this._faceBitmapData,_frames[_loc2_],_loc8_,null,null,true);
               _loc5_++;
            }
            _loc3_.scale(0.75,0.75);
            _loc8_.x = _loc8_.y = 0;
            _loc4_.draw(this._soulFace,_loc3_,null,null,null,true);
            _loc6_ = new Rectangle(0,0,_characterWidth,_characterHeight);
            this._soulFace.fillRect(this._soulFace.rect,0);
            _loc7_ = 0;
            while(_loc7_ < 4)
            {
               _loc6_.x = _loc7_ * _characterWidth * 0.75;
               _loc8_.x = _characterWidth * _loc7_ + 7;
               _loc8_.y = 5;
               this._soulFace.copyPixels(this._faceDownBitmapdata,_frames[36],new Point(_loc7_ * _characterWidth,0),null,null,true);
               this._soulFace.copyPixels(_loc4_,_loc6_,_loc8_,null,null,true);
               this._soulFace.copyPixels(this._faceupBitmapData,_frames[36],new Point(_loc7_ * _characterWidth,0),null,null,true);
               _loc7_++;
            }
            _loc8_.x = _loc8_.y = 0;
            this._soulFace.applyFilter(this._soulFace,this._soulFace.rect,_loc8_,grayFilter);
            _loc4_.dispose();
         }
      }
