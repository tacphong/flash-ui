package ddt.view.character
{
   ...
   public class GameCharacterLoader extends BaseCharacterLoader
   {
      ...
      {
         var _loc1_:Number = Number(_layers[1].width);
         var _loc2_:Number = Number(_layers[1].height);
      ...
      private function drawNormal() : void
      {
         ...
         var _loc5_:Number = Number(_layers[1].width);
         var _loc6_:Number = Number(_layers[1].height);
         ...
         this._face = new BitmapData(_loc5_,_loc6_,true,0);
         if(Boolean(this._faceup))
         {
            this._faceup.dispose();
         }
         this._faceup = new BitmapData(_loc5_,_loc6_,true,0);
         if(Boolean(this._sp))
         {
            for each(_loc1_ in this._sp)
            {
               _loc1_.dispose();
            }
         }
         ...
         {
            for each(_loc2_ in this._lackHpFace)
            {
               _loc2_.dispose();
            }
         }
         ...
         this._faceDown = new BitmapData(_loc5_,_loc6_,true,0);
         var _loc7_:int = 7;
         while(_loc7_ >= 0)
         {
            if(_layers[_loc7_].info.CategoryID == EquipType.WING)
            {
               _wing = _layers[_loc7_].getContent() as MovieClip;
            }
            else if(_loc7_ == 5)
            {
               this._face.draw((_layers[_loc7_] as ILayer).getContent(),null,null,BlendMode.NORMAL);
            }
            else if(_loc7_ == 6)
            {
               this._faceDown.draw((_layers[_loc7_] as ILayer).getContent(),null,null,BlendMode.NORMAL);
            }
            else if(_loc7_ < 5)
            {
               this._faceup.draw((_layers[_loc7_] as ILayer).getContent(),null,null,BlendMode.NORMAL);
            }
            _loc7_--;
         }
         var _loc8_:Number = Number(_layers[8].width);
         var _loc9_:Number = Number(_layers[8].height);
         var _loc10_:int = 8;
         while(_loc10_ < _layers.length)
         {
            _loc3_ = new BitmapData(_loc8_,_loc9_,true,0);
            _loc4_ = new BitmapData(_loc8_,_loc9_,true,0);
            _loc3_.draw((_layers[_loc10_] as ILayer).getContent(),null,null,BlendMode.NORMAL);
            _loc4_.draw((_layers[_loc10_ + 1] as ILayer).getContent(),null,null,BlendMode.NORMAL);
            this._lackHpFace.push(_loc3_);
            this._sp.push(_loc4_);
            _loc10_ += 2;
         }
      }
