package ddt.view.character
{
   ...
   public class BaseCharacterLoader implements ICharacterLoader
   {
      ...
      protected var _layers:Vector.<ddt.view.character.ILayer>;
      
      protected var _layerFactory:ddt.view.character.ILayerFactory;
      ...
      {
         var _loc1_:ddt.view.character.ILayer = null;
      ...
      final public function load(param1:Function = null) : void
      {
         var _loc2_:ddt.view.character.ILayer = null;
         ...
         this.initLayers();
         var _loc3_:int = int(this._layers.length);
         var _loc4_:int = 0;
         while(_loc4_ < _loc3_)
         {
            _loc2_ = this._layers[_loc4_];
            _loc2_.load(this.__layerComplete);
            _loc4_++;
         }
      }
      ...
      public function getUnCompleteLog() : String
      {
         ...
         while(_loc4_ < _loc3_)
         {
            _loc1_ = this._layers[_loc4_];
            if(!this._layers[_loc4_].isComplete)
            {
               _loc2_ += "unLoaded templete: " + this._layers[_loc4_].info.TemplateID.toString() + "\n";
            }
            _loc4_++;
         }
         return _loc2_;
      ...
      private function __layerComplete(param1:ddt.view.character.ILayer) : void
      {
      ...
      {
         var _loc1_:Number = Number(this._layers[1].width);
         var _loc2_:Number = Number(this._layers[1].height);
         ...
         var _loc3_:int = this._layers.length - 1;
         for(; _loc3_ >= 0; _loc3_--)
         {
      ...
      public function setFactory(param1:ddt.view.character.ILayerFactory) : void
      {
      ...
      public function update() : void
      {
         ...
         var _loc8_:ddt.view.character.ILayer = null;
         var _loc9_:ItemTemplateInfo = null;
         var _loc10_:ddt.view.character.ILayer = null;
         var _loc11_:int = 0;
         if(Boolean(this._info.Style) && Boolean(this._layers))
         {
            ...
            while(_loc5_ < _loc1_.length)
            {
               ...
               if(!(_loc6_ == -1 || _loc6_ == 9))
               {
                  if(this._recordStyle.indexOf(_loc1_[_loc5_]) == -1)
                  {
                     ...
                     if(Boolean(this._layers[_loc6_]))
                     {
            ...
            if(_loc3_ && !_loc4_)
            {
               ...
               if(Boolean(this._layers[_loc11_]))
               {
      ...
      protected function getCharacterLoader(param1:ItemTemplateInfo, param2:String = "", param3:String = null) : ddt.view.character.ILayer
      {
