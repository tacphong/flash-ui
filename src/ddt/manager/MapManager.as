package ddt.manager
{
   ...
   public class MapManager extends EventDispatcher
   {
      ...
      public static const PVE_ADVANCED_MAP:Array = [13,14,15,16,17];
      ...
      public static function setupDungeonInfo(param1:DungeonAnalyzer) : void
      {
         ...
         while(_loc3_ < _pveList.length)
         {
            ...
            if(MapManager.PVE_ADVANCED_MAP.indexOf(_loc2_.ID) != -1)
            {
               _pveAdvancedList.push(_loc2_);
            }
            else if(_loc2_.Type == PVE_LINK_MAP)
            {
               _pveLinkList.push(_loc2_);
            }
            else if(_loc2_.Type == PVE_BOSS_MAP)
            {
               _pveBossList.push(_loc2_);
            }
            else if(_loc2_.Type == PVE_EXPROL_MAP)
            {
               _pveExplrolList.push(_loc2_);
            }
            else if(_loc2_.Type == FIGHT_LIB)
            {
               _fightLibList.push(_loc2_);
            }
            else if(_loc2_.Type == PVE_ACADEMY_MAP)
            {
               _pveAcademyList.push(_loc2_);
            }
            else if(_loc2_.Type == PVE_ACTIVITY_MAP || _loc2_.Type == PVE_SPECIAL_MAP || _loc2_.Type == DOUBLE_SPECIAL1 || _loc2_.Type == DOUBLE_SPECIAL2)
            {
      ...
      public static function buildMap() : void
      {
         ...
         var _loc3_:uint = 0;
         while(_loc3_ < _openList.length)
         {
            if(_openList[_loc3_].serverID == ServerManager.Instance.current.ID)
            {
               _loc1_ = _openList[_loc3_].maps;
            }
            _loc3_++;
         }
         if(Boolean(_openList) && Boolean(_list))
         {
            for each(_loc2_ in _list)
            {
               if(Boolean(_loc1_))
               {
                  _loc2_.isOpen = _loc1_.indexOf(String(_loc2_.ID)) > -1;
               }
            }
