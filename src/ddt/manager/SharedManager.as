package ddt.manager
{
   ...
   public class SharedManager extends EventDispatcher
   {
      ...
      private static var instance:ddt.manager.SharedManager = new ddt.manager.SharedManager();
      ...
      public function SharedManager()
      {
         ...
         this._manualNewChapters = [];
         this._manualNewPages = [];
      ...
      public static function get Instance() : ddt.manager.SharedManager
      {
         return instance;
      }
      ...
      private function load() : void
      {
         ...
         try
         {
            ...
            if(Boolean(so) && Boolean(so.data))
            {
               ...
               {
                  this.manualNewPages = so.data["manualNewPages"];
               }
               if(so.data["manualNewDebris"] != undefined)
               {
                  this.manualNewChapters = so.data["manualNewDebris"];
               }
            }
            return;
         }
      ...
      public function save() : void
      {
         ...
         try
         {
            ...
            _loc1_.data["manualNewDebris"] = this.manualNewChapters;
            _loc1_.data["manualNewPages"] = this.manualNewPages;
            _loc1_.flush(20 * 1024 * 1024);
         }
         catch(e:Error)
         {
      ...
      public function changed() : void
      {
         ...
         {
            if(Boolean(RIGHT_PROP[int(int(_loc1_) - 1)]))
            {
