package ddt.manager
{
   ...
   public class BossBoxManager extends EventDispatcher
   {
      ...
      private static var _instance:ddt.manager.BossBoxManager;
      ...
      public static function get instance() : ddt.manager.BossBoxManager
      {
         if(_instance == null)
         {
            _instance = new ddt.manager.BossBoxManager();
         }
         return _instance;
      }
      ...
      private function _updateGradeII(param1:PlayerPropertyEvent) : void
      {
         ...
         {
            if(Boolean(this.timeBoxList[1]) && PlayerManager.Instance.Self.Grade > this.timeBoxList[1].Level)
            {
      ...
      {
         var _loc3_:GradeBoxInfo = null;
         var _loc4_:Boolean = false;
         for each(_loc3_ in this.gradeBoxList)
         {
            if(PlayerManager.Instance.Self.Sex)
            {
               if(_loc3_.Level > param1 && _loc3_.Level <= param2 && Boolean(_loc3_.Condition))
               {
                  if(this._boxShowArray.indexOf(_loc3_.ID + ",grade") == -1)
                  {
                     this._boxShowArray.push(_loc3_.ID + ",grade");
                  }
                  _loc4_ = true;
               }
            }
            else if(_loc3_.Level > param1 && _loc3_.Level <= param2 && !_loc3_.Condition)
            {
               if(this._boxShowArray.indexOf(_loc3_.ID + ",grade") == -1)
               {
                  this._boxShowArray.push(_loc3_.ID + ",grade");
      ...
      private function _getShowBoxID(param1:String) : int
      {
         var _loc2_:int = 0;
         var _loc3_:int = 0;
         while(_loc3_ < this._boxShowArray.length)
         {
            if(String(this._boxShowArray[_loc3_]).indexOf(param1) > 0)
            {
               _loc2_ = int(String(this._boxShowArray[_loc3_]).split(",")[0]);
               this._selectedBoxID = this._boxShowArray.splice(_loc3_,1);
               return _loc2_;
            }
            _loc3_++;
         }
