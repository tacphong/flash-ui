package par.enginees
{
   ...
   public class ParticleEnginee
   {
      ...
      public function update() : void
      {
         ...
         var _loc4_:Number = 0.04;
         for each(_loc2_ in this._emitters)
         {
            _loc2_.execute(_loc4_);
         }
         _loc3_ = 0;
         while(_loc3_ < this.particles.length)
         {
            _loc1_ = this.particles[_loc3_];
            _loc1_.age += _loc4_;
            if(_loc1_.age >= _loc1_.life)
            {
               this.particles.splice(_loc3_,1);
               this._render.removeParticle(_loc1_);
               this.cacheParticle(_loc1_);
               _loc3_--;
            }
      ...
      {
         if(Boolean(this.spareParticles[param1.name]) && this.spareParticles[param1.name].length > 0)
         {
