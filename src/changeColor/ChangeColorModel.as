package changeColor
{
   ...
   public class ChangeColorModel extends EventDispatcher
   {
      ...
      private function getColorEditableThings() : void
      {
         ...
         {
            if(_loc1_.Place > BagInfo.PERSONAL_EQUIP_COUNT)
            {
               if((EquipType.isEditable(_loc1_) || _loc1_.CategoryID == EquipType.FACE) && _loc1_.getRemainDate() > 0)
               {
                  this._colorEditableThings.add(_loc2_++,_loc1_);
               }
            }
         }
      }
