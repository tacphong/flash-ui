package tryonSystem
{
   ...
   public class TryonPanelView extends Sprite implements Disposeable
   {
      ...
      private var _controller:tryonSystem.TryonSystemController;
      
      private var _model:tryonSystem.TryonModel;
      ...
      public function TryonPanelView(param1:tryonSystem.TryonSystemController)
      {
         super();
      ...
      private function initView() : void
      {
         ...
         var _loc4_:TryonCell = null;
         var _loc5_:MovieImage = null;
         var _loc6_:GlowFilterAnimation = null;
         var _loc7_:PersonalInfoCell = null;
         ...
         this._itemList.y = 32;
         var _loc8_:AnimationControl = new AnimationControl();
         _loc8_.addEventListener(Event.COMPLETE,this._cellLightComplete);
         for each(_loc1_ in this._model.items)
         {
            _loc3_ = new Sprite();
            ...
            _loc4_.info = _loc1_;
            _loc4_.addEventListener(MouseEvent.CLICK,this.__onClick);
            _loc4_.buttonMode = true;
            this._itemList.addChild(_loc4_);
            this._cells.push(_loc4_);
            if(_loc1_.CategoryID == 3)
            {
            ...
            _loc5_ = ComponentFactory.Instance.creatComponentByStylename("asset.core.itemBigShinelight");
            _loc5_.movie.play();
            _loc4_.addChildAt(_loc5_,1);
            _loc6_ = new GlowFilterAnimation();
            _loc6_.start(_loc5_,false,16763955,0,0);
            _loc6_.addMovie(0,0,19,0);
            _loc8_.addMovies(_loc6_);
         }
         ...
         while(_loc2_ < 8)
         {
            _loc7_ = new PersonalInfoCell(_loc2_,this._bagItems[CELL_PLACE[_loc2_]] as InventoryItemInfo,true);
            this._bagCells.push(_loc7_);
            _loc2_++;
         }
