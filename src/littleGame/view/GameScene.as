package littleGame.view
{
   ...
   public class GameScene extends Sprite implements Disposeable
   {
      ...
      private var selfGameLiving:littleGame.view.GameLittleSelf;
      ...
      {
         var _loc1_:LittleLiving = null;
         var _loc2_:Dictionary = this._game.livings;
         for each(_loc1_ in _loc2_)
         {
            this.drawLiving(_loc1_);
         }
      }
      ...
      private function drawLiving(param1:LittleLiving) : GameLittleLiving
      {
         ...
         if(param1.isSelf)
         {
            _loc2_ = new littleGame.view.GameLittleSelf(param1 as LittleSelf);
      ...
      private function drawGrid() : void
      {
         var _loc1_:int = 0;
         var _loc2_:int = 0;
         var _loc3_:Grid = this._game.grid;
         var _loc4_:BitmapData = new BitmapData(_loc3_.width,_loc3_.height,true,0);
         var _loc5_:Array = _loc3_.nodes;
         var _loc6_:int = int(_loc5_.length);
         var _loc7_:int = 0;
         while(_loc7_ < _loc6_)
         {
            _loc1_ = int(_loc5_[_loc7_].length);
            _loc2_ = 0;
            while(_loc2_ < _loc1_)
            {
               if(!_loc5_[_loc7_][_loc2_].walkable)
               {
                  _loc4_.fillRect(new Rectangle(_loc2_ * _loc3_.cellSize,_loc7_ * _loc3_.cellSize,_loc3_.cellSize,_loc3_.cellSize),2583625728);
               }
               _loc2_++;
            }
            _loc7_++;
         }
         addChild(new Bitmap(_loc4_));
      }
      ...
      private function sortDepth() : void
      {
         ...
         var _loc3_:int = 0;
         while(_loc3_ < this._backLivingLayer.numChildren)
         {
            _loc1_ = this._backLivingLayer.getChildAt(_loc3_);
         ...
         _loc2_.sortOn(["y"],[Array.NUMERIC]);
         var _loc4_:int = 0;
         while(_loc4_ < _loc2_.length)
         {
            this._backLivingLayer.setChildIndex(_loc2_[_loc4_],_loc4_);
            _loc4_++;
         }
      }
      ...
      private function updateLivingVisible() : void
      {
         ...
         if(Boolean(this.selfGameLiving))
         {
      ...
      private function sortLiving(param1:GameLittleLiving) : void
      {
         ...
         var _loc4_:Vector.<Rectangle> = this._game.stones;
         for each(_loc2_ in _loc4_)
         {
            if(_loc2_.intersects(_loc3_))
            {
               if(_loc3_.bottom <= _loc2_.bottom)
               {
      ...
      private function __click(param1:MouseEvent) : void
      {
         ...
         var _loc3_:LittleSelf = this._game.selfPlayer;
         var _loc4_:int = mouseX / this._game.grid.cellSize;
         var _loc5_:int = mouseY / this._game.grid.cellSize;
         var _loc6_:Array = LittleGameManager.Instance.fillPath(_loc3_,this._game.grid,_loc3_.pos.x,_loc3_.pos.y,_loc4_,_loc5_);
         if(Boolean(_loc6_))
         {
            LittleGameManager.Instance.selfMoveTo(this._game,_loc3_,_loc3_.pos.x,_loc3_.pos.y,_loc4_,_loc5_,this._game.clock.time,_loc6_);
            _loc2_ = new MovieClipWrapper(ClassUtils.CreatInstance("asset.hotSpring.MouseClickMovie"),true,true);
            _loc2_.movie.mouseChildren = _loc2_.movie.mouseEnabled = false;
            _loc2_.x = mouseX;
            _loc2_.y = mouseY;
