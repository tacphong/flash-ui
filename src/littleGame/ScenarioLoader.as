package littleGame
{
   ...
   public class ScenarioLoader extends EventDispatcher implements IProcessObject
   {
      ...
      private function loadObjects() : void
      {
         ...
         var _loc2_:Array = this._scene.objects.split(",");
         this._total = _loc2_.length + 2;
         this._objectLoaders = new Vector.<BaseLoader>();
         var _loc3_:int = int(_loc2_.length);
         var _loc4_:int = 0;
         while(_loc4_ < _loc3_)
         {
            _loc1_ = LoaderManager.Instance.creatLoader(PathManager.solveLittleGameObjectPath(_loc2_[_loc4_]),BaseLoader.MODULE_LOADER);
            _loc1_.addEventListener(LoaderEvent.LOAD_ERROR,this.__loaderError);
            _loc1_.addEventListener(LoaderEvent.COMPLETE,this.__objectComplete);
            LoaderManager.Instance.startLoad(_loc1_);
            this._objectLoaders.push(_loc1_);
            _loc4_++;
         }
      }
      ...
      private function loadConfigData() : void
      {
         ...
         var _loc2_:int = this._localW + GridProcessCount / this._h;
         _loc2_ = _loc2_ > this._w ? int(this._w) : int(_loc2_);
         var _loc3_:int = this._localW;
         while(_loc3_ < _loc2_)
         {
            _loc1_ = 0;
            while(_loc1_ < this._h)
            {
               this._grid.setNodeWalkAble(_loc3_,_loc1_,this._configBytes.readByte() == 1 ? Boolean(true) : Boolean(false));
