package littleGame.object
{
   ...
   public class BigBoguInhaled extends NormalBoguInhaled
   {
      ...
      {
         super.removeEvent();
         if(Boolean(this._clickSoundChannel))
         {
      ...
      {
         var _loc1_:* = null;
         var _loc2_:ILittleObject = null;
         this.drawBackground();
         drawMark();
         lockLivings();
         _scene.selfInhaled = true;
         ChatManager.Instance.focusFuncEnabled = false;
         var _loc3_:Dictionary = _scene.littleObjects;
         for(_loc1_ in _loc3_)
         {
            _loc2_ = _loc3_[_loc1_];
            if(_loc2_.type == LittleObjectType.BoguGiveup)
            {
               _scene.removeObject(_loc2_);
            }
         }
      ...
      {
         var _loc2_:PriceShape = null;
         _loc2_ = null;
         ++_clickCount;
         if(Boolean(_inhaleAsset))
         {
         ...
         if(_clickCount >= _totalClick)
         {
            ...
            _loc3_ = _totalScore * 0.2;
            _loc2_ = new PriceShape(_loc3_);
            _loc2_.alpha = 0;
            _loc2_.x = 300;
            _loc2_.y = this._scoreShape.y;
            addChild(_loc2_);
            TweenLite.to(_loc2_,0.2,{
               "delay":0.2,
               "alpha":1,
               "y":_loc2_.y - _loc2_.height - 20,
