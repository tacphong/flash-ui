package AvatarCollection.view
{
   ...
   public class AvatarCollectionItemCell extends Sprite implements Disposeable
   {
      ...
      private function buyClickHandler(param1:MouseEvent) : void
      {
         ...
         var _loc2_:BaseAlerFrame = AlertManager.Instance.simpleAlert(LanguageMgr.GetTranslation("tips"),LanguageMgr.GetTranslation("avatarCollection.buyConfirm.tipTxt",this._data.buyPrice),LanguageMgr.GetTranslation("ok"),LanguageMgr.GetTranslation("cancel"),true,true,true,2);
      ...
      private function onBuyConfirmResponse(param1:FrameEvent) : void
      {
         ...
         var _loc10_:BaseAlerFrame = param1.currentTarget as BaseAlerFrame;
         _loc10_.removeEventListener(FrameEvent.RESPONSE,this.onBuyConfirmResponse);
         if(param1.responseCode == FrameEvent.ENTER_CLICK || param1.responseCode == FrameEvent.SUBMIT_CLICK)
         {
            _loc2_ = _loc10_.isBand;
            if(_loc2_ && PlayerManager.Instance.Self.Gift < this._data.buyPrice)
            {
               MessageTipManager.getInstance().show(LanguageMgr.GetTranslation("store.view.transfer.StoreIITransferBG.lijinbuzu"));
               return;
            }
            if(!_loc2_ && PlayerManager.Instance.Self.Money < this._data.buyPrice)
            {
               LeavePageManager.showFillFrame();
               return;
            }
            SocketManager.Instance.addEventListener(CrazyTankSocketEvent.BUY_GOODS,this.onBuyedGoods);
            _loc3_ = [this._data.goodsId];
            _loc4_ = [1];
            _loc5_ = [""];
            _loc6_ = [""];
            _loc7_ = [""];
            _loc8_ = [this._data.isDiscount];
            _loc9_ = [_loc10_.isBand];
      ...
      private function sendActive() : void
      {
         ...
         SocketManager.Instance.out.sendAvatarCollectionActive(this._data.id,this._data.itemId,this._data.sex,this._data.Type);
      }
