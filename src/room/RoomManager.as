package room
{
   ...
   public class RoomManager extends EventDispatcher
   {
      ...
      private static var _instance:room.RoomManager;
      ...
      public static function get Instance() : room.RoomManager
      {
         if(_instance == null)
         {
            _instance = new room.RoomManager();
         }
         return _instance;
      }
      ...
      private function __paymentFailed(param1:CrazyTankSocketEvent) : void
      {
         ...
         var _loc7_:Boolean = _loc5_.readBoolean();
         if(_loc6_ == 0)
         {
            if(!_loc7_)
            {
         ...
         else if(_loc6_ == 2)
         {
            if(!_loc7_)
            {
               _loc4_ = AlertManager.Instance.simpleAlert(LanguageMgr.GetTranslation("tank.room.RoomIIView2.notenoughmoney.title"),LanguageMgr.GetTranslation("tank.room.RoomIIView2.notenoughmoney.content"),LanguageMgr.GetTranslation("ok"),LanguageMgr.GetTranslation("cancel"),false,true,true,LayerManager.ALPHA_BLOCKGOUND);
      ...
      private function __createRoom(param1:CrazyTankSocketEvent) : void
      {
         ...
         _loc2_.hardLevel = _loc3_.readByte();
         _loc2_.timeType = _loc3_.readByte();
         _loc2_.totalPlayer = _loc3_.readByte();
         _loc2_.viewerCnt = _loc3_.readByte();
         _loc2_.placeCount = _loc3_.readByte();
         _loc2_.isLocked = _loc3_.readBoolean();
         _loc2_.mapId = _loc3_.readInt();
         _loc2_.started = _loc3_.readBoolean();
         _loc2_.Name = _loc3_.readUTF();
         _loc2_.gameMode = _loc3_.readByte();
         _loc2_.levelLimits = _loc3_.readInt();
         _loc2_.isCrossZone = _loc3_.readBoolean();
         _loc2_.isWithinLeageTime = _loc3_.readBoolean();
         _loc2_.isOpenBoss = _loc3_.readBoolean();
         _loc2_.pic = param1.pkg.readUTF();
         this.setCurrent(_loc2_);
      }
      ...
      protected function __createSingleRoom(param1:CrazyTankSocketEvent) : void
      {
         ...
         this.setCurrent(_loc4_);
         PlayerManager.Instance.Self.ZoneID = _loc3_.readInt();
         if(GameManager.Instance.Current != null)
         {
            _loc2_ = GameManager.Instance.Current.findRoomPlayer(PlayerManager.Instance.Self.ID,PlayerManager.Instance.Self.ZoneID);
         }
         if(_loc2_ == null)
         {
      ...
      private function __addPlayerInRoom(param1:CrazyTankSocketEvent) : void
      {
         ...
         if(Boolean(this._current))
         {
            ...
            if(_loc15_.isSelf && Boolean(this._current))
            {
      ...
      private function __removePlayerInRoom(param1:CrazyTankSocketEvent) : void
      {
         ...
         if(Boolean(this._current))
         {
            ...
            if(Boolean(_loc4_) && _loc4_.isSelf)
            {
               ...
               else if(StateManager.currentStateType == StateType.FIGHTING || StateManager.currentStateType == StateType.ROOM_LOADING || StateManager.currentStateType == StateType.GAME_LOADING)
               {
                  ...
                  else if(this._current.type == RoomInfo.CATCH_BEAST)
                  {
                     StateManager.setState(StateType.MAIN);
                  }
                  else
      ...
      private function __buffObtain(param1:CrazyTankSocketEvent) : void
      {
         ...
         if(_loc10_.clientId == PlayerManager.Instance.Self.ID)
         {
            return;
         }
         if(this._current.findPlayerByID(_loc10_.clientId) != null)
         {
            ...
            while(_loc3_ < _loc2_)
            {
               ...
               _loc5_ = _loc10_.readBoolean();
               _loc6_ = _loc10_.readDate();
               _loc7_ = _loc10_.readInt();
               _loc8_ = _loc10_.readInt();
               _loc9_ = new BuffInfo(_loc4_,_loc5_,_loc6_,_loc7_,_loc8_);
               this._current.findPlayerByID(_loc10_.clientId).playerInfo.buffInfo.add(_loc9_.Type,_loc9_);
               _loc3_++;
            }
      ...
      private function __buffUpdate(param1:CrazyTankSocketEvent) : void
      {
         ...
         if(this._current.findPlayerByID(_loc10_.clientId) != null)
         {
            ...
            while(_loc3_ < _loc2_)
            {
               ...
               {
                  this._current.findPlayerByID(_loc10_.clientId).playerInfo.buffInfo.remove(_loc9_.Type);
               }
               _loc3_++;
            }
            param1.stopImmediatePropagation();
      ...
      public function isIdenticalRoom(param1:int = 0, param2:String = "") : Boolean
      {
         ...
         var _loc4_:DictionaryData = this.current.players;
         ...
         {
            if(_loc3_.playerInfo.ID == param1 || _loc3_.playerInfo.NickName == param2)
            {
               return true;
            }
         }
