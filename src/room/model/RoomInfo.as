package room.model
{
   ...
   public class RoomInfo extends EventDispatcher
   {
      ...
      public function updatePlaceState(param1:Array) : void
      {
         ...
         var _loc4_:int = 0;
         var _loc5_:int = this.type == CHALLENGE_ROOM ? int(8) : int(4);
         var _loc6_:int = -100;
         var _loc7_:int = 0;
         while(_loc7_ < 10)
         {
            if(this._placesState[_loc7_] != param1[_loc7_])
            {
               if(_loc7_ >= 8)
               {
                  if(param1[_loc7_] != -1)
                  {
                     _loc3_ = this.findPlayerByID(param1[_loc7_]);
                     if(_loc3_ != null)
                     {
                        _loc3_.place = _loc7_;
                     }
                  }
                  else if(this._placesState[_loc7_] != -1)
                  {
                     _loc3_ = this.findPlayerByID(this._placesState[_loc7_]);
                     if(_loc3_ != null)
                     {
                        if(_loc6_ != -100)
                        {
      ...
      public function canPlayGuidMode() : Boolean
      {
         ...
         if(this.selfRoomPlayer.isViewer)
         {
            for each(_loc3_ in this.players)
            {
               if(_loc3_ != this.selfRoomPlayer)
               {
                  _loc1_ = _loc3_.playerInfo.ConsortiaID;
                  break;
               }
            }
         }
         ...
         for each(_loc2_ in this.players)
         {
            if(!_loc2_.isViewer)
            {
               if(_loc2_.playerInfo.ConsortiaID != _loc1_)
               {
                  _loc4_ = false;
                  break;
               }
      ...
      public function isAllReady() : Boolean
      {
         ...
         for each(_loc1_ in this._players)
         {
            if(!_loc1_.isReady && !_loc1_.isHost && !_loc1_.isViewer)
            {
               _loc2_ = false;
               break;
            }
