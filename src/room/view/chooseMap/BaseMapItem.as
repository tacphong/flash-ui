package room.view.chooseMap
{
   ...
   public class BaseMapItem extends Sprite implements Disposeable
   {
      ...
      protected var powerNum:String;
      
      protected var _battleNumText:FilterFrameText;
      ...
      {
         this._battleNumText = ComponentFactory.Instance.creatComponentByStylename("room.mapItem.recommend.text");
         addChild(this._battleNumText);
         this._battleNumText.visible = false;
      }
      
      public function setBattleNum(param1:String, param2:int = 0) : void
      {
         if(param1.length <= 0)
         {
            this._battleNumText.text = LanguageMgr.GetTranslation("tank.dungeon.noFightPower");
            return;
         }
         this.powerNum = param1;
         this._battleNumText.text = LanguageMgr.GetTranslation("tank.dungeon.battleNum",this.powerNum.split("|")[param2]);
         var _loc3_:String = "";
      }
      
      private function updateBattleNum() : void
      {
         this._battleNumText.visible = this.mapId > -1;
      }
      ...
      protected function __onComplete(param1:LoaderEvent) : void
      {
         ...
         if(BaseLoader(param1.loader).isSuccess)
         {
            ...
            this._battleNumText.visible = true;
         }
      }
      ...
      private function __onClick(param1:MouseEvent) : void
      {
         ...
         if(this._mapId > -1)
         {
            ...
            if(Boolean(_loc2_) && _loc2_.LevelLimits > PlayerManager.Instance.Self.Grade)
            {
      ...
      public function dispose() : void
      {
         ...
         this._bg = null;
         ObjectUtils.disposeObject(this._battleNumText);
         this._battleNumText = null;
         ObjectUtils.disposeAllChildren(this._mapIconContaioner);
