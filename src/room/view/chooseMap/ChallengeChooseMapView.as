package room.view.chooseMap
{
   ...
   public class ChallengeChooseMapView extends Sprite implements Disposeable
   {
      ...
      private var _currentSelectedItem:room.view.chooseMap.BaseMapItem;
      ...
      {
         var _loc1_:MapInfo = null;
         var _loc2_:DisplayObject = null;
         var _loc3_:room.view.chooseMap.BaseMapItem = null;
         ...
         _loc4_.title = LanguageMgr.GetTranslation("tank.room.RoomIIMapSetPanel.room");
         _loc4_.showCancel = _loc4_.moveEnable = false;
         _loc4_.submitLabel = LanguageMgr.GetTranslation("ok");
         this._isDisposed = false;
         this._frame.info = _loc4_;
         ...
         var _loc5_:Rectangle = ComponentFactory.Instance.creatCustomObject("asset.chooseMapView.rect");
         var _loc6_:Rectangle = ComponentFactory.Instance.creatCustomObject("asset.chooseMapView.rect1");
         this._roundTimeBg.graphics.beginFill(0,0.3);
         this._roundTimeBg.graphics.drawRoundRect(_loc6_.x,_loc6_.y,_loc6_.width,_loc5_.height,5,5);
         ...
         _loc7_.graphics.drawRect(this._explainTxt.x,this._explainTxt.y,this._explainTxt.width,this._explainTxt.height);
         ...
         for each(_loc1_ in this._mapInfoList)
         {
            if(!(_loc1_.Type != 0 && _loc1_.Type != 1 && _loc1_.Type != 3))
            {
               if(_loc1_.canSelect)
               {
                  _loc2_ = new MapSmallIcon(_loc1_.ID);
                  if(_loc2_ != null)
                  {
                     _loc3_ = new room.view.chooseMap.BaseMapItem();
                     if(_loc1_.ID == RoomManager.Instance.current.mapId)
                     {
                        _loc3_.selected = true;
                        this._currentSelectedItem = _loc3_;
                        this._mapId = _loc1_.ID;
                     }
                     if(_loc1_.isOpen)
                     {
                        _loc3_.mapId = _loc1_.ID;
                        _loc3_.addEventListener(Event.SELECT,this.__mapItemClick);
                        this._mapList.addChild(_loc3_);
                     }
                  }
               }
            }
      ...
      {
         if(Boolean(param1.currentTarget.isSuccess))
         {
      ...
      {
         var _loc1_:room.view.chooseMap.BaseMapItem = null;
         if(!this._isDisposed)
         {
