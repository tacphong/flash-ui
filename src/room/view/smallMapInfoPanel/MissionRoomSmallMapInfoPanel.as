package room.view.smallMapInfoPanel
{
   ...
   public class MissionRoomSmallMapInfoPanel extends BaseSmallMapInfoPanel
   {
      ...
      protected var _fightTxt:FilterFrameText;
      
      protected var _fightNumTxt:FilterFrameText;
      ...
      override protected function initView() : void
      {
         ...
         this._fightTxt = ComponentFactory.Instance.creatComponentByStylename("asset.ddtroom.smallMap.fightTxt");
         addChild(this._fightTxt);
         this._fightNumTxt = ComponentFactory.Instance.creatComponentByStylename("asset.ddtroom.smallMap.fightNumTxt");
         addChild(this._fightNumTxt);
         ...
         this._fightTxt.text = LanguageMgr.GetTranslation("ddt.dungeonRoom.fightRange");
      }
      
      override protected function updateView() : void
      {
         super.updateView();
         this._fightTxt.visible = this._levelRangeTitle.visible = this._modeTitle.visible = this._diffTitle.visible = this._levelRange.visible = this._mode.visible = this._diff.visible = Boolean(_info) && _info.mapId != 0 && _info.mapId != 10000;
         this.solveLeveRange();
      ...
      private function solveLeveRange() : void
      {
         ...
         var _loc3_:String = MapManager.getDungeonInfo(_info.mapId).AdviceTips;
         if(Boolean(_loc3_))
         {
            _loc1_ = _loc3_.split("|");
            this._levelRange.text = "";
            if(_info.hardLevel >= _loc1_.length)
            {
               return;
            }
            this._levelRange.text = _loc1_[_info.hardLevel];
            _loc2_ = MapManager.getDungeonInfo(_info.mapId).FightPower;
            if(!_loc2_ || _loc2_ == "")
            {
               this._fightNumTxt.text = LanguageMgr.GetTranslation("tank.dungeon.noFightPower1");
            }
            else
            {
               this._fightNumTxt.text = _loc2_.split("|")[this._info.hardLevel];
            }
         }
      }
