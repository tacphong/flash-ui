package room.view.roomView
{
   ...
   public class BaseRoomView extends Sprite implements Disposeable
   {
      ...
      protected function initPlayerItems() : void
      {
         ...
         {
            _loc1_ = this._playerItems[_loc3_] as RoomPlayerItem;
            _loc1_.info = this._info.findPlayerByPlace(_loc3_);
            _loc1_.opened = this._info.placesState[_loc3_] != 0;
            _loc3_++;
         }
         if(this.isViewerRoom)
         {
            ...
            while(_loc3_ < 2)
            {
               if(Boolean(this._viewerItems) && Boolean(this._viewerItems[_loc3_]))
               {
                  _loc2_ = this._viewerItems[_loc3_] as RoomViewerItem;
                  _loc2_.info = this._info.findPlayerByPlace(_loc3_ + 8);
                  _loc2_.opened = this._info.placesState[_loc3_ + 8] != 0;
               }
               _loc3_++;
            }
         }
      ...
      protected function initEvents() : void
      {
         ...
         if(this.isViewerRoom)
         {
            ...
            while(_loc1_ < 2)
            {
               if(Boolean(this._viewerItems) && Boolean(this._viewerItems[_loc1_]))
               {
      ...
      private function __switchClickEnabled(param1:RoomEvent) : void
      {
         ...
         while(_loc3_ < this._playerItems.length)
         {
            _loc2_ = this._playerItems[_loc3_] as RoomPlayerItem;
      ...
      protected function __updatePlayerItems(param1:RoomEvent) : void
      {
         ...
         {
            _loc2_ = this._playerItems[_loc3_] as RoomPlayerItem;
            _loc2_.opened = this._info.placesState[_loc3_] != 0;
            _loc3_++;
         }
         if(this.isViewerRoom)
         {
      ...
      protected function academyDungeonAllow() : Boolean
      {
         ...
         for each(_loc1_ in RoomManager.Instance.current.players)
         {
            if(_loc1_.playerInfo.apprenticeshipState == AcademyManager.NONE_STATE && !_loc1_.isViewer)
            {
               MessageTipManager.getInstance().show(LanguageMgr.GetTranslation("ddt.room.roomStart.academy.warning1"));
               return false;
            }
            if((_loc1_.playerInfo.apprenticeshipState == AcademyManager.MASTER_STATE || _loc1_.playerInfo.apprenticeshipState == AcademyManager.MASTER_FULL_STATE || _loc1_.playerInfo.apprenticeshipState == AcademyManager.APPRENTICE_STATE) && !_loc1_.isViewer)
            {
               _loc3_++;
            }
         }
         ...
         {
            if((_loc2_.playerInfo.apprenticeshipState == AcademyManager.MASTER_STATE || _loc2_.playerInfo.apprenticeshipState == AcademyManager.MASTER_FULL_STATE) && !_loc2_.isViewer)
            {
               _loc3_++;
               if(_loc3_ > 1)
               {
