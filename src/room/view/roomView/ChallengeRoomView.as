package room.view.roomView
{
   ...
   public class ChallengeRoomView extends BaseRoomView implements Disposeable
   {
      ...
      override protected function updateButtons() : void
      {
         ...
         {
            _loc1_ = _playerItems[_loc4_] as RoomPlayerItem;
            if(Boolean(_loc1_.info) && _loc1_.info.team == RoomPlayer.BLUE_TEAM)
            {
               _loc2_ = true;
            }
            if(Boolean(_loc1_.info) && _loc1_.info.team == RoomPlayer.RED_TEAM)
            {
               _loc3_ = true;
            }
            _loc4_++;
      ...
      override protected function initTileList() : void
      {
         ...
         while(_loc3_ < 8)
         {
            _loc1_ = new RoomPlayerItem(_loc3_);
            this._playerItemContainers[RoomPlayer.BLUE_TEAM - 1].addChild(_loc1_);
            _playerItems.push(_loc1_);
            _loc2_ = new RoomPlayerItem(_loc3_ + 1);
            this._playerItemContainers[RoomPlayer.RED_TEAM - 1].addChild(_loc2_);
      ...
      {
         var _loc1_:DefyAfficheViewFrame = null;
         ...
         {
            if(this._self.NickName == _info.defyInfo[0][_loc2_])
            {
