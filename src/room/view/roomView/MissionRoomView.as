package room.view.roomView
{
   ...
   public class MissionRoomView extends BaseRoomView
   {
      ...
      override protected function initTileList() : void
      {
         ...
         var _loc3_:Point = ComponentFactory.Instance.creatCustomObject("room.dungeonRoom.listSpace");
         this._playerItemContainer.hSpace = _loc3_.x;
         this._playerItemContainer.vSpace = _loc3_.y;
         _loc1_ = ComponentFactory.Instance.creatCustomObject("asset.room.playerListPos");
         this._playerItemContainer.x = this._rightBg.x + _loc1_.x;
         this._playerItemContainer.y = this._rightBg.y + _loc1_.y;
         var _loc4_:int = 0;
         while(_loc4_ < 4)
         {
            _loc2_ = new RoomPlayerItem(_loc4_);
            this._playerItemContainer.addChild(_loc2_);
