package room.view.roomView
{
   ...
   public class DungeonRoomView extends BaseRoomView
   {
      ...
      override protected function initTileList() : void
      {
         ...
         var _loc2_:Point = ComponentFactory.Instance.creatCustomObject("room.dungeonRoom.listSpace");
         this._playerItemContainer.hSpace = _loc2_.x;
         this._playerItemContainer.vSpace = _loc2_.y;
         _loc3_ = ComponentFactory.Instance.creatCustomObject("asset.room.playerListPos");
         this._playerItemContainer.x = this._rightBg.x + _loc3_.x;
         this._playerItemContainer.y = this._rightBg.y + _loc3_.y;
         var _loc4_:int = 0;
         while(_loc4_ < 4)
         {
            _loc1_ = new RoomPlayerItem(_loc4_);
            this._playerItemContainer.addChild(_loc1_);
      ...
      {
         var _loc1_:DungeonChooseMapFrame = null;
         var _loc2_:DungeonInfo = MapManager.getDungeonInfo(_info.mapId);
         if(super.checkCanStartGame())
         {
            ...
            {
               _loc1_ = ComponentFactory.Instance.creatCustomObject("asset.room.dungeonChooseMapFrame");
               _loc1_.show();
               dispatchEvent(new RoomEvent(RoomEvent.OPEN_DUNGEON_CHOOSER));
               return false;
            }
            if(RoomManager.Instance.current.players.length - RoomManager.Instance.current.currentViewerCnt == 1 && _loc2_.Type != MapManager.PVE_ACADEMY_MAP)
            {
            ...
            //if(_loc2_.Type == MapManager.PVE_ACADEMY_MAP && !super.academyDungeonAllow())
            //{
               //return false;
            //}
            return true;
