package room.view
{
   ...
   public class RoomRightPropView extends Sprite implements Disposeable
   {
      ...
      private function initView() : void
      {
         ...
         var _loc2_:RoomPropCell = null;
         var _loc3_:PropInfo = null;
         var _loc4_:RoomPropCell = null;
         ...
         this._upCells = [];
         var _loc5_:int = 0;
         while(_loc5_ < UPCELLS_NUMBER)
         {
            _loc2_ = new RoomPropCell(true,_loc5_);
            this._upCellsContainer.addChild(_loc2_);
            this._upCells.push(_loc2_);
            _loc5_++;
         }
         var _loc6_:Vector.<ShopItemInfo> = ShopManager.Instance.getGoodsByType(ShopType.ROOM_PROP);
         var _loc7_:int = 0;
         while(_loc7_ < this.getPropNum())
         {
            _loc3_ = new PropInfo(_loc6_[_loc7_].TemplateInfo);
            _loc4_ = new RoomPropCell(false,_loc5_);
            this._downCellsContainer.addChild(_loc4_);
            _loc4_.info = _loc3_;
            _loc7_++;
         }
         ...
         for each(_loc1_ in PlayerManager.Instance.Self.getBag(BagInfo.FIGHTBAG).items)
         {
            this._upCells[_loc1_.Place].info = new PropInfo(PlayerManager.Instance.Self.getBag(BagInfo.FIGHTBAG).items[_loc1_.Place]);
         }
      ...
      {
         if(Boolean(param1.changedProperties[PlayerInfo.GOLD]))
         {
         }
      }
      ...
      private function _cellClick(param1:MouseEvent) : void
      {
         var _loc2_:RoomPropCell = null;
         var _loc3_:int = 0;
         for each(_loc2_ in this._upCells)
         {
            if(_loc2_.info != null)
            {
               _loc3_++;
