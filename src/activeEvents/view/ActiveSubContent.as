package activeEvents.view
{
   ...
   public class ActiveSubContent extends Sprite implements Disposeable
   {
      ...
      private function _activeGetBtnClick(param1:MouseEvent) : void
      {
         ...
         _loc3_.writeUTFBytes(this._textInput.text);
         ...
         _loc4_["activeID"] = this._info.ActiveID;
         ...
         this._loader = LoaderManager.Instance.creatLoader(PathManager.solveRequestPath("ActivePullDown.ashx"),BaseLoader.REQUEST_LOADER,_loc4_);
      ...
      {
         if(Boolean(this._info.Title) || Boolean(this._info.Description))
         {
            if(Boolean(this._info.Title))
            {
               this._discriptionTitle.text = this._info.Title;
            }
            if(Boolean(this._info.Description))
            {
      ...
      {
         if(Boolean(this._info.activeTime()))
         {
      ...
      {
         if(Boolean(this._info.AwardContent))
         {
