package eliteGame.view
{
   ...
   public class EliteGamePaarungFrame extends Frame
   {
      ...
      {
         var _loc3_:EliteGamePaarungText = null;
         _loc3_ = null;
         this._texts[param1] = new Vector.<EliteGamePaarungText>();
         var _loc4_:int = 1;
         while(_loc4_ <= param2)
         {
            _loc3_ = ComponentFactory.Instance.creatComponentByStylename("eliteGame.paarungView." + param1 + "." + _loc4_);
            _loc3_.addEventListener(TextEvent.LINK,this.__onTextClicked);
            _loc3_.mouseEnabled = true;
            addToContent(_loc3_);
            this._texts[param1].push(_loc3_);
            _loc4_++;
         }
      }
      ...
      private function addText(param1:String, param2:Vector.<int>) : void
      {
         ...
         else
         {
            ...
            while(_loc3_ < param2.length)
            {
               ...
               {
                  if(Boolean(this._texts[param1][_loc4_].canAccept(this.getRankWithId(param2[_loc3_]))))
                  {
