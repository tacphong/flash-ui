   ...
   public class PetSkillPnl extends Sprite implements Disposeable
   {
      ...
      protected function creatItems() : void
      {
         ...
         for each(_loc2_ in this._itemInfoVec)
         {
            if(Boolean(_loc2_))
            {
               _loc1_ = new SkillItem(_loc2_,_loc3_++,true,this._isWatch);
         ...
         _loc4_ = this._isWatch ? int(14) : int(8);
         while(_loc3_ < _loc4_)
         {
            _loc1_ = new SkillItem(null,_loc3_++,true,this._isWatch);
      ...
      private function __skillItemClick(param1:PetItemEvent) : void
      {
         ...
         if(Boolean(_loc2_) && Boolean(PetBagController.instance().petModel.currentPetInfo))
         {
