package petsBag.view
{
   ...
   public class PetsBagOtherView extends PetsBagView
   {
      ...
      override public function updatePetBagView() : void
      {
         ...
         _petExpProgress.setProgress(Boolean(_loc1_) ? Number(_loc1_.GP) : Number(0),Boolean(_loc1_) ? Number(_loc1_.MaxGP) : Number(0));
      ...
      override public function updatePetsPropByEvolution() : void
      {
         ...
         for each(_loc1_ in PetsAdvancedManager.Instance.evolutionDataList)
         {
            ...
            if(_loc1_.ID == _infoPlayer.evolutionGrade)
            {
               _currentGradeInfo = _loc1_;
               break;
            }
         }
         ...
         if(Boolean(_loc2_))
         {
            _attackPbtn.propValue = _loc2_.IsEquip ? int(_loc2_.Attack + _currentGradeInfo.Attack) : int(_loc2_.Attack);
            _defencePbtn.propValue = _loc2_.IsEquip ? int(_loc2_.Defence + _currentGradeInfo.Defence) : int(_loc2_.Defence);
            _HPPbtn.propValue = _loc2_.IsEquip ? int(_loc2_.Blood + _currentGradeInfo.Blood) : int(_loc2_.Blood);
            _agilityPbtn.propValue = _loc2_.IsEquip ? int(_loc2_.Agility + _currentGradeInfo.Agility) : int(_loc2_.Agility);
            _luckPbtn.propValue = _loc2_.IsEquip ? int(_loc2_.Luck + _currentGradeInfo.Lucky) : int(_loc2_.Luck);
         }
      }
