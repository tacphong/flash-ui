package petsBag.view
{
   ...
   public class PetsBagOutView extends PetsBagView
   {
      ...
      private var _petGameSkillPnl:petsBag.view.PetGameSkillPnl;
      ...
      override protected function initView() : void
      {
         ...
         this._washBoneBtn = ComponentFactory.Instance.creatComponentByStylename("petsBag.button.washBonePet");
         this._washBoneBtn.text = LanguageMgr.GetTranslation("ddt.pets.washBone");
      ...
      override protected function initEvent() : void
      {
         ...
         this._washBoneBtn.addEventListener(MouseEvent.CLICK,this.__washBonePet);
         ...
         if(Boolean(_loc3_))
         {
            switch(param1.type)
            {
               case DictionaryEvent.ADD:
                  _petMoveScroll.refreshPetInfo(_loc3_,1);
                  break;
               case DictionaryEvent.UPDATE:
                  _petMoveScroll.refreshPetInfo(_loc3_);
                  _loc2_ = PetBagController.instance().petModel.currentPetInfo;
                  if(Boolean(_loc2_) && _loc2_.Place == _loc3_.Place)
                  {
                     PetBagController.instance().petModel.currentPetInfo = _loc3_;
                  }
                  break;
               case DictionaryEvent.REMOVE:
                  _petMoveScroll.refreshPetInfo(_loc3_,2);
      ...
      protected function __AlertRePetNameResponse(param1:FrameEvent) : void
      {
         ...
         switch(param1.responseCode)
         {
            ...
            case FrameEvent.SUBMIT_CLICK:
               ...
               if(Boolean(PetBagController.instance().petModel.currentPetInfo) && _loc2_.petName.length > 0)
               {
      ...
      private function __convertPet(param1:MouseEvent) : void
      {
         ...
         if(Boolean(_loc3_))
         {
            if(_loc3_.StarLevel >= PetconfigAnalyzer.PetCofnig.NotRemoveStar)
            {
               MessageTipManager.getInstance().show(LanguageMgr.GetTranslation("ddt.pets.convertCannotTxt"));
            }
            else if(_loc3_.StarLevel >= PetconfigAnalyzer.PetCofnig.HighRemoveStar)
            {
               _loc2_ = AlertManager.Instance.simpleAlert(LanguageMgr.GetTranslation("tips"),LanguageMgr.GetTranslation("ddt.farms.convertPet",_loc3_.StarLevel),LanguageMgr.GetTranslation("ok"),LanguageMgr.GetTranslation("cancel"),false,true,false,LayerManager.ALPHA_BLOCKGOUND);
               _loc2_.enterEnable = false;
               _loc2_.addEventListener(FrameEvent.RESPONSE,this.__alertReleasePet2);
            }
            else
            {
      ...
      private function __releasePet(param1:MouseEvent) : void
      {
         ...
         if(Boolean(_loc3_))
         {
            ...
            {
               _loc2_ = AlertManager.Instance.simpleAlert(LanguageMgr.GetTranslation("tips"),LanguageMgr.GetTranslation("ddt.farms.releasePet",StringHelper.trim(PetBagController.instance().petModel.currentPetInfo.Name)),LanguageMgr.GetTranslation("ok"),LanguageMgr.GetTranslation("cancel"),false,true,false,LayerManager.ALPHA_BLOCKGOUND);
               _loc2_.addEventListener(FrameEvent.RESPONSE,this.__alertReleasePet);
            }
         }
      }
      ...
      override protected function removeEvent() : void
      {
         ...
         this._washBoneBtn.removeEventListener(MouseEvent.CLICK,this.__washBonePet);
