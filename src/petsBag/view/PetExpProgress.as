package petsBag.view
{
   ...
   public class PetExpProgress extends Component
   {
      ...
      {
         var _loc1_:int = 0;
         var _loc2_:int = PetExperience.getLevelByGP(this._gp);
         var _loc3_:int = this._gp - PetExperience.expericence[_loc2_ - 1];
         _loc1_ = PetExperience.expericence[_loc2_ >= PetExperience.MAX_LEVEL ? PetExperience.MAX_LEVEL - 1 : _loc2_] - PetExperience.expericence[_loc2_ - 1];
         var _loc4_:int = this._maxGp - this._gp;
         var _loc5_:Number = _loc1_ == 0 ? Number(0) : Number(_loc3_ / _loc1_);
         var _loc6_:Number = _loc1_ == 0 ? Number(0) : Number((_loc3_ + _loc4_) / _loc1_);
         this._gpMask.width = this._gpItem.width * _loc6_;
         this._maxGpMask.width = this._maxGpItem.width * _loc5_;
         _tipData = [_loc3_,_loc1_].join("/") + LanguageMgr.GetTranslation("ddt.petbag.petExpProgressTip",_loc4_);
         this._progressLabel.text = [_loc3_,_loc1_].join("/");
      }
