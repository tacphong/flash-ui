package petsBag.view.item
{
   ...
   public class FeedItem extends BaseCell
   {
      ...
      override public function dragDrop(param1:DragEffect) : void
      {
         ...
         if(Boolean(_loc4_) && _loc4_.CategoryID == EquipType.FOOD)
         {
            if(EquipType.isPetSpeciallFood(_loc4_))
            {
               if(Boolean(info))
               {
               ...
               SocketManager.Instance.out.sendMoveGoods(_loc4_.BagType,_loc4_.Place,BagInfo.STOREBAG,-1,1);
            }
            else
            {
               _loc2_ = ComponentFactory.Instance.creatComponentByStylename("petsBag.PetFoodNumberSelectFrame");
               _loc2_.foodInfo = _loc4_;
               _loc2_.petInfo = PetBagController.instance().petModel.currentPetInfo;
               _loc2_.addEventListener(FrameEvent.RESPONSE,this.__onFoodAmountResponse);
               _loc3_ = this.needMaxFood(PetBagController.instance().petModel.currentPetInfo.Hunger,int(_loc4_.Property1));
      ...
      private function needMaxFood(param1:int, param2:int) : int
      {
         ...
         return int(Math.ceil(_loc4_ / Number(param2)));
      }
      ...
      override public function dragStop(param1:DragEffect) : void
      {
         ...
         {
            if(Boolean(_loc2_) && _loc2_.BagType == 11)
            {
               param1.action = DragEffect.NONE;
               super.dragStop(param1);
            }
            else if(Boolean(_loc2_) && _loc2_.BagType == 12)
            {
               locked = false;
            }
            else
