package petsBag.petsAdvanced
{
   ...
   public class PetsEatView extends Sprite implements Disposeable
   {
      ...
      private var _bagCell:petsBag.petsAdvanced.PetsAdvancedCell;
      
      private var _progress:petsBag.petsAdvanced.PetsAdvancedProgressBar;
      ...
      private var _petsImgVec:Vector.<petsBag.petsAdvanced.PetsEatSmallItem>;
      ...
      public function initView() : void
      {
         ...
         this._eatPetsMc.scaleX = this._eatPetsMc.scaleY = 0.95;
      ...
      {
         var _loc2_:petsBag.petsAdvanced.PetsEatSmallItem = null;
         var _loc1_:* = null;
         _loc2_ = null;
         this.clearPets();
         var _loc3_:int = 0;
         for(_loc1_ in PlayerManager.Instance.Self.pets)
         {
            if(!PlayerManager.Instance.Self.pets[_loc1_].IsEquip && PlayerManager.Instance.Self.pets[_loc1_].StarLevel >= 3)
            {
               _loc2_ = new petsBag.petsAdvanced.PetsEatSmallItem();
               _loc2_.info = PlayerManager.Instance.Self.pets[_loc1_];
               _loc2_.initTips();
               _loc2_.x = 17 + _loc3_ % 2 * 85;
               _loc2_.y = Math.floor(_loc3_ / 2) * 82;
               _loc2_.addEventListener("selected",this.selectedHandler);
               _loc3_++;
               this._listContainer.addChild(_loc2_);
      ...
      private function selectedHandler(param1:Event) : void
      {
         ...
         this.checkItem();
         var _loc4_:int = 0;
         while(_loc4_ < this._selectedArr.length)
         {
            _loc3_ = this._selectedArr[_loc4_][1];
            _loc2_ += Math.pow(10,_loc3_.StarLevel - 2) + 5 * Math.max(_loc3_.Level - 8,_loc3_.Level * 0.2);
            _loc4_++;
         }
      ...
      {
         var _loc1_:petsBag.petsAdvanced.PetsEatSmallItem = null;
         this._selectedArr = [];
         var _loc2_:int = 0;
         while(_loc2_ < this._petsImgVec.length)
         {
            _loc1_ = this._petsImgVec[_loc2_] as PetsEatSmallItem;
            if(_loc1_.selected)
            {
               this._selectedArr.push([_loc1_.info.Place,_loc1_.info]);
            }
            _loc2_++;
         }
      }
      ...
      private function _eatBtnHandler(param1:MouseEvent) : void
      {
         ...
         switch(param1.target)
         {
            ...
            case this._eatStoneBtn:
               ...
               _loc4_ = this._bagCell.getExpOfBagcell();
               if(this._autoUseBtn.selected)
               {
                  _loc5_ = Math.ceil((this._progress.max - this._progress.currentExp) / _loc4_);
                  _loc6_ = _loc5_ < this._bagCell.getCount() ? int(_loc5_) : int(this._bagCell.getCount());
                  SocketManager.Instance.out.eatPetsHandler(this._chooseMc.currentFrame - 1,2,_loc6_,[]);
               }
               else
               {
