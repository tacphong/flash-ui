package petsBag.petsAdvanced
{
   ...
   public class PetsEvolutionView extends PetsAdvancedView
   {
      ...
      private function updateData() : void
      {
         ...
         var _loc3_:int = _petInfo.IsEquip ? int(_petInfo.Blood + this._currentGradeInfo.Blood) : int(_petInfo.Blood);
         var _loc4_:int = _petInfo.IsEquip ? int(_petInfo.Attack + this._currentGradeInfo.Attack) : int(_petInfo.Attack);
         var _loc5_:int = _petInfo.IsEquip ? int(_petInfo.Defence + this._currentGradeInfo.Defence) : int(_petInfo.Defence);
         var _loc6_:int = _petInfo.IsEquip ? int(_petInfo.Agility + this._currentGradeInfo.Agility) : int(_petInfo.Agility);
         var _loc7_:int = _petInfo.IsEquip ? int(_petInfo.Luck + this._currentGradeInfo.Lucky) : int(_petInfo.Luck);
         var _loc8_:Array = [_loc3_,_loc4_,_loc5_,_loc6_,_loc7_];
         if(_self.evolutionGrade >= PetsAdvancedManager.Instance.evolutionDataList.length)
         {
            _tip.tipData = "0/0";
         ...
         else if(Boolean(this._nextGradeInfo))
         {
            ...
            _loc2_ = [this._nextGradeInfo.Blood - this._currentGradeInfo.Blood,this._nextGradeInfo.Attack - this._currentGradeInfo.Attack,this._nextGradeInfo.Defence - this._currentGradeInfo.Defence,this._nextGradeInfo.Agility - this._currentGradeInfo.Agility,this._nextGradeInfo.Lucky - this._currentGradeInfo.Lucky];
         }
         var _loc9_:int = 0;
         while(_loc9_ < _itemVector.length)
         {
            _itemVector[_loc9_].setData(_loc9_,_loc8_[_loc9_],_loc2_[_loc9_]);
            _loc9_++;
         }
         this.setAddedTxt();
      }
