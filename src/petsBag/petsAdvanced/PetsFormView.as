package petsBag.petsAdvanced
{
   ...
   public class PetsFormView extends Sprite implements Disposeable
   {
      ...
      private var _petsVec:Vector.<petsBag.petsAdvanced.PetsFormPetsItem>;
      ...
      {
         var _loc1_:petsBag.petsAdvanced.PetsFormPetsItem = null;
         _loc1_ = null;
         var _loc2_:PetsFormData = null;
         var _loc3_:int = 0;
         while(_loc3_ < 6)
         {
            ...
            if(_loc3_ < 3)
            {
               _loc1_.x = 34 + 180 * _loc3_;
               _loc1_.y = 95;
            }
            else
            {
               _loc1_.x = 34 + 180 * (_loc3_ - 3);
               _loc1_.y = 243;
            }
            _loc2_ = null;
            if(_loc3_ < PetsAdvancedManager.Instance.formDataList.length)
            {
               _loc2_ = PetsAdvancedManager.Instance.formDataList[_loc3_];
               if(_loc2_.TemplateID == PlayerManager.Instance.Self.PetsID)
               {
                  _loc2_.ShowBtn = 2;
               }
            }
            _loc1_.setInfo(_loc3_,_loc2_);
            _loc1_.addEventListener(PetItemEvent.ITEM_CLICK,this.__onClickPetsItem);
            addChild(_loc1_);
            this._petsVec.push(_loc1_);
            _loc3_++;
         }
      ...
      private function setItemInfo() : void
      {
         ...
         {
            if(Boolean(this._petsVec[_loc3_].info) && this._petsVec[_loc3_].info.State == 1)
            {
      ...
      protected function __onGetPetsFormInfo(param1:CrazyTankSocketEvent) : void
      {
         ...
         while(_loc6_ < _loc5_)
         {
            ...
            _loc3_ = PetsAdvancedManager.Instance.getFormDataIndexByTempId(_loc2_);
            PetsAdvancedManager.Instance.formDataList[_loc3_].State = 1;
            PetsAdvancedManager.Instance.formDataList[_loc3_].ShowBtn = 1;
            _loc6_++;
         }
         this.creatPetsView();
      }
      ...
      protected function __onPetsWake(param1:CrazyTankSocketEvent) : void
      {
         ...
         {
            if(Boolean(this._petsVec[_loc5_].info))
            {
      ...
      protected function __onPetsFollowOrCall(param1:CrazyTankSocketEvent) : void
      {
         ...
         if(_loc5_ != -1)
         {
            ...
            this.resetItemInfo(_loc5_,_loc4_ ? int(2) : int(1),_loc2_);
            PlayerManager.Instance.Self.PetsID = _loc4_ ? int(_loc5_) : int(-1);
            if(_loc4_)
            {
      ...
      private function resetItemInfo(param1:int, param2:int, param3:int) : void
      {
         ...
         {
            if(Boolean(this._petsVec[_loc4_].info))
            {
      ...
      private function deletePets() : void
      {
         ...
         {
            if(Boolean(this._petsVec[_loc1_]))
            {
