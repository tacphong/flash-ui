package character
{
   ...
   public class MovieClipCharacter extends Sprite implements ICharacter
   {
      ...
      private function onEnterFrame(param1:Event) : void
      {
         ...
         else if(Boolean(this._currentAction))
         {
            MovieClip(this._currentAction.asset).stop();
         }
      }
      ...
      public function set description(param1:XML) : void
      {
         ...
         {
            if(Boolean(this._assets) && Boolean(this._assets[String(action.@asset)]))
            {
      ...
      public function doAction(param1:String) : void
      {
         ...
         var _loc3_:MovieClipAction = this._actionSet.getAction(param1) as MovieClipAction;
         if(Boolean(_loc3_))
         {
            if(this._currentAction != null)
            {
               if(_loc3_.priority >= this._currentAction.priority)
               {
                  _loc2_ = this._currentAction.asset as MovieClip;
      ...
      private function set _2032707372realRender(param1:Boolean) : void
      {
         ...
         if(this._realRender)
         {
            if(Boolean(this._currentAction) && Boolean(this._currentAction.asset))
            {
