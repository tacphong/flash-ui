package chickActivation
{
   ...
   public class ChickActivationManager extends EventDispatcher
   {
      ...
      private static var _instance:chickActivation.ChickActivationManager;
      ...
      public static function get instance() : chickActivation.ChickActivationManager
      {
         if(_instance == null)
         {
            _instance = new chickActivation.ChickActivationManager();
         }
         return _instance;
      }
      ...
      private function dataUpdate(param1:PackageIn) : void
      {
         ...
         {
            _loc2_ = 0;
            while(_loc2_ < this.model.gainArr.length - 1)
            {
               if(this.model.gainArr[_loc2_] != _loc3_[_loc2_] && _loc3_[_loc2_] > 0)
               {
                  _loc5_ = _loc2_;
                  break;
               }
            ...
            if(_loc5_ != -1)
            {
               this.model.dataChange(ChickActivationEvent.GET_REWARD,_loc5_);
            }
         }
         this.model.gainArr = _loc3_;
         this.model.dataChange(ChickActivationEvent.UPDATE_DATA);
      }
