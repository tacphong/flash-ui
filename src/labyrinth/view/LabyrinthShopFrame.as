package labyrinth.view
{
   ...
   public class LabyrinthShopFrame extends BaseAlerFrame
   {
      ...
      private var _goodItems:Vector.<labyrinth.view.LabyrinthShopItem>;
      ...
      override protected function init() : void
      {
         ...
         var _loc7_:AlertInfo = new AlertInfo(_loc1_,"",LanguageMgr.GetTranslation("tank.calendar.Activity.BackButtonText"),false);
         info = _loc7_;
         ...
         _loc8_ = 0;
         while(_loc8_ < SHOP_ITEM_NUM)
         {
            this._goodItems[_loc8_] = ComponentFactory.Instance.creatCustomObject("labyrinth.view.labyrinthShopItem");
            _loc5_ = this._goodItems[_loc8_].width;
            _loc6_ = this._goodItems[_loc8_].height;
            _loc5_ *= int(_loc8_ % 2);
            _loc6_ *= int(_loc8_ / 2);
            if((_loc8_ + 1) % 2 == 0)
            {
               this._goodItems[_loc8_].x = _loc5_ - 5;
            }
            else
            {
               this._goodItems[_loc8_].x = _loc5_;
            }
            this._goodItems[_loc8_].y = _loc6_ + _loc8_ / 2 * 2;
            this._goodItemContainerAll.addChild(this._goodItems[_loc8_]);
            this._goodItems[_loc8_].setItemLight(this._rightItemLightMc);
            _loc8_++;
         }
         this.loadList();
         this.initEvent();
