package labyrinth.view
{
   ...
   public class CleanOutContentItem extends Sprite implements IListCell
   {
      ...
      private function init() : void
      {
         ...
         var _loc1_:int = LabyrinthManager.Instance.model.currentFloor == 0 ? int(LabyrinthManager.Instance.model.currentFloor + 1) : int(LabyrinthManager.Instance.model.currentFloor);
      ...
      private function updateItem() : void
      {
         ...
         if(!this._info)
         {
            _loc1_ = LabyrinthManager.Instance.model.currentFloor == 0 ? int(LabyrinthManager.Instance.model.currentFloor + 1) : int(LabyrinthManager.Instance.model.currentFloor);
            this._floorNumContent.text = LanguageMgr.GetTranslation("ddt.labyrinth.CleanOutItem.ValueTextII",_loc1_);
            this._expNum.text = "";
            return;
         }
         ...
         var _loc2_:String = "";
         var _loc3_:int = 0;
         while(_loc3_ < this._info.TemplateIDs.length)
         {
            _loc2_ += "," + ItemManager.Instance.getTemplateById(this._info.TemplateIDs[_loc3_]["TemplateID"]).Name;
            if(this._info.TemplateIDs[_loc3_]["num"] != 0)
            {
               _loc2_ += "x" + this._info.TemplateIDs[_loc3_]["num"].toString();
            }
            _loc3_++;
         }
         if(this._info.HardCurrency > 0)
         {
            _loc2_ += LanguageMgr.GetTranslation("dt.labyrinth.CleanOutContentItem.HardCurrency",this._info.HardCurrency);
         }
         this._expNum.text += _loc2_;
      }
