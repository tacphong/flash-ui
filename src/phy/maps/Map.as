package phy.maps
{
   ...
   public class Map extends Sprite
   {
      ...
      protected var _stone:phy.maps.Ground;
      
      protected var _middle:DisplayObject;
      
      protected var _ground:phy.maps.Ground;
      ...
      public function Map(param1:DisplayObject, param2:phy.maps.Ground = null, param3:phy.maps.Ground = null, param4:DisplayObject = null)
      {
         super();
         ...
         this._stone = param3;
         this._middle = param4;
         if(Boolean(this._middle))
         {
            addChild(this._middle);
         }
         if(Boolean(this._stone))
         {
            addChild(this._stone);
         }
         this._ground = param2;
         if(Boolean(this._ground))
         {
      ...
      public function get ground() : phy.maps.Ground
      {
         return this._ground;
      }
      
      public function get stone() : phy.maps.Ground
      {
         return this._stone;
      }
      ...
      {
         param1 = param1 < 0 ? int(0) : (param1 >= this._bound.width ? int(this._bound.width - 1) : int(param1));
         param2 = param2 < 0 ? int(0) : int(param2);
         param3 = param2 + param3 >= this._bound.height ? int(this._bound.height - param2 - 1) : int(param3);
         var _loc4_:int = 0;
         while(_loc4_ < param3)
         {
      ...
      {
         param1 = param1 < 0 ? int(0) : (param1 > this._bound.width ? int(this._bound.width) : int(param1));
         param2 = param2 < 0 ? int(0) : int(param2);
         param3 = param2 + param3 > this._bound.height ? int(this._bound.height - param2) : int(param3);
         var _loc4_:int = 0;
         while(_loc4_ < param3)
         {
            if(!this.IsEmpty(param1 - 1,param2) || !this.IsEmpty(param1 + 1,param2))
      ...
      public function removePhysical(param1:Physics) : void
      {
         ...
         {
            if(Boolean(this._objects) && !this._objects[param1])
            {
               return;
            }
            delete this._objects[param1];
      ...
      public function getPhysicalObjects(param1:Rectangle, param2:PhysicalObj) : Array
      {
         ...
         {
            if(_loc3_ != param2 && _loc3_.isLiving && _loc3_.canCollided)
            {
               _loc4_ = _loc3_.getCollideRect();
      ...
      public function getBoxesByRect(param1:Rectangle) : Array
      {
         ...
         for each(_loc2_ in this._phyicals)
         {
            if(_loc2_.isBox() && _loc2_.isLiving)
            {
               _loc3_ = _loc2_.getTestRect();
      ...
      public function getCollidedPhysicalObjects(param1:Rectangle, param2:PhysicalObj) : Array
      {
         ...
         for each(_loc3_ in this._phyicals)
         {
            if(_loc3_ != param2 && _loc3_.canCollided)
            {
               _loc4_ = _loc3_.getCollideRect();
               _loc4_.offset(_loc3_.x,_loc3_.y);
               if(_loc4_.intersects(param1))
      ...
      public function dispose() : void
      {
         ...
         while(_loc3_ >= 0)
         {
            ...
            if(_loc1_ is Physics)
            {
               Physics(_loc1_).dispose();
            }
            _loc3_--;
         }
         ...
         if(Boolean(this._middle))
         {
            if(Boolean(this._middle.parent))
            {
         ...
         this._sky = null;
         if(Boolean(this._mapThing) && Boolean(this._mapThing.parent))
         {
