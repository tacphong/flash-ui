package serverlist.view
{
   ...
   public class ServerDropList extends Frame
   {
      ...
      private function updateServerList() : void
      {
         ...
         var _loc2_:VectorListModel = this._cb.listPanel.vectorListModel;
         _loc2_.clear();
         for each(_loc1_ in this.getServerList())
         {
            _loc2_.append(_loc1_.Name);
         }
      }
      ...
      protected function getServerList() : Vector.<ServerInfo>
      {
         ...
         var _loc3_:Vector.<ServerInfo> = new Vector.<ServerInfo>();
         for each(_loc1_ in _loc2_)
         {
            if(_loc1_.Name != ServerManager.Instance.current.Name)
            {
               if(_loc1_.State != ServerInfo.MAINTAIN)
               {
                  _loc3_.push(_loc1_);
               }
            }
      ...
      private function getServerByName(param1:String) : ServerInfo
      {
         ...
         var _loc3_:Vector.<ServerInfo> = ServerManager.Instance.list;
