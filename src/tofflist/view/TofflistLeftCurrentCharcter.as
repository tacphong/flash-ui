package tofflist.view
{
   ...
   public class TofflistLeftCurrentCharcter extends Sprite implements Disposeable
   {
      ...
      private function getRank(param1:int) : void
      {
         ...
         while(_loc6_ < _loc5_)
         {
            _loc2_ = this.getRankBitmap(int(_loc4_.substr(_loc6_,1)));
            _loc2_.x = _loc6_ * 30;
         ...
         _loc3_ = ComponentFactory.Instance.creat("tofflist.rankPos");
         this._rankNumber.x = _loc3_.x;
         this._rankNumber.y = _loc3_.y;
      }
      ...
      {
         var _loc2_:ConsortiaApplyInfo = null;
         var _loc3_:int = 0;
         var _loc4_:Vector.<ConsortiaApplyInfo> = TofflistModel.Instance.myConsortiaAuditingApplyData;
         if(Boolean(_loc4_))
         {
            _loc3_ = 0;
