package consumeRank.views
{
   ...
   public class ConsumeRankView extends Sprite implements IRightView
   {
      ...
      public function updateView() : void
      {
         ...
         this._checkBg.tipData = LanguageMgr.GetTranslation("consumeRank.helpTxt",_loc3_.remain2);
         _loc6_ = ConsumeRankManager.instance.rankList;
         _loc1_ = ConsumeRankManager.instance.myConsume;
         this._myRank = -1;
         var _loc7_:int = 0;
         while(_loc7_ <= _loc6_.length - 1)
         {
            _loc2_ = _loc6_[_loc7_] as ConsumeRankVo;
         ...
         if(this._myRank >= 0)
         {
            ...
            else
            {
               ...
               _loc8_ = (_loc6_[this._myRank - 1] as ConsumeRankVo).consume - _loc1_ + 1;
               this._rankLabelTxt.text = LanguageMgr.GetTranslation("consumeRank.rankLabel",_loc8_);
            }
         ...
         else
         {
            ...
            if(_loc6_.length > 0)
            {
               _loc8_ = (_loc6_[_loc6_.length - 1] as ConsumeRankVo).consume - _loc1_ + 1;
            }
            else
            {
               _loc8_ = 1;
            ...
            this._rankLabelTxt.text = LanguageMgr.GetTranslation("consumeRank.outOfRankLabel",_loc1_,_loc8_);
            if(ConsumeRankManager.instance.status == 2)
            {
               this._rankLabelTxt.visible = true;
               this._rankLabelTxt.textColor = 16711680;
      ...
      {
         var _loc1_:ConsumeRankItem = null;
         var _loc2_:Array = ConsumeRankManager.instance.rankList;
         var _loc3_:Array = ConsumeRankManager.instance.xmlData.giftbagArray;
         this._vbox.removeAllChild();
         var _loc4_:int = 0;
         while(_loc4_ <= _loc2_.length - 1)
         {
            _loc1_ = new ConsumeRankItem(_loc4_);
