package bagAndInfo.bag
{
   ...
   public class NecklacePtetrochemicalView extends BaseAlerFrame
   {
      ...
      {
         if(Boolean(param1.changedProperties["necklaceExp"]) || Boolean(param1.changedProperties["necklaceExpAdd"]))
         {
            this.updateView();
         }
      }
      ...
      {
         var _loc1_:int = 0;
         var _loc2_:int = PlayerManager.Instance.Self.necklaceExp;
         var _loc3_:int = PlayerManager.Instance.Self.necklaceExpAdd;
         var _loc4_:int = PlayerManager.Instance.Self.necklaceLevel;
         this._numText.text = this._num.toString();
         this._expText.text = String(_loc3_);
         this._levelText.text = "Lv. " + _loc4_.toString();
         var _loc5_:int = StoreEquipExperience.getNecklaceStrengthPlus(_loc4_);
         this._currentLevel.text = LanguageMgr.GetTranslation("bagAndInfo.bag.NecklacePtetrochemicalView.info",_loc5_);
         if(_loc4_ < StoreEquipExperience.NECKLACE_MAX_LEVEL)
         {
            _loc1_ = StoreEquipExperience.getNecklaceStrengthPlus(_loc4_ + 1);
            this._nextLevel.text = LanguageMgr.GetTranslation("bagAndInfo.bag.NecklacePtetrochemicalView.info",_loc1_);
         }
         else
         {
         ...
         if(_loc4_ < StoreEquipExperience.NECKLACE_MAX_LEVEL)
         {
            this._progress.setProgress(StoreEquipExperience.getNecklaceCurrentlevelExp(_loc2_),StoreEquipExperience.getNecklaceCurrentlevelMaxExp(_loc4_));
         }
         else
         {
            this._progress.setProgress(1,1);
         }
         var _loc6_:int = PlayerManager.Instance.Self.getBag(BagInfo.PROPBAG).getItemCountByTemplateId(EquipType.NECKLACE_PTETROCHEM_STONE);
         if(this._num > _loc6_)
         {
            if(Boolean(this._stoneInfo))
            {
               this._numText.text = String(_loc6_);
               this._num = _loc6_;
            }
         }
      }
