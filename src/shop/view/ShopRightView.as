package shop.view
{
   ...
   public class ShopRightView extends Sprite implements Disposeable
   {
      ...
      private var _goodItems:Vector.<shop.view.ShopGoodItem>;
      ...
      private function initBtns() : void
      {
         ...
         {
            this._goodItems[_loc1_] = ComponentFactory.Instance.creatCustomObject("shop.GoodItem");
            _loc2_ = this._goodItems[_loc1_].width;
            _loc3_ = this._goodItems[_loc1_].height;
            _loc2_ *= int(_loc1_ % 2);
            _loc3_ *= int(_loc1_ / 2);
            this._goodItems[_loc1_].x = _loc2_;
            this._goodItems[_loc1_].y = _loc3_;
            this._goodItemContainerAll.addChild(this._goodItems[_loc1_]);
            this._goodItems[_loc1_].setItemLight(this._rightItemLightMc);
         ...
         {
            this._topBtns[_loc1_].addEventListener(MouseEvent.CLICK,this.__topBtnClick);
            this._topBtnsContainer.addChild(this._topBtns[_loc1_]);
            this._topBtnsGroup.addSelectItem(this._topBtns[_loc1_]);
            if(_loc1_ == 0)
            {
               this._topBtnsGroup.selectIndex = _loc1_;
            }
            this._subBtnsGroups[_loc1_] = new SelectedButtonGroup();
            _loc1_++;
         }
         ...
         var _loc5_:Array = [3,8,13,15,20,21];
         _loc4_ = 0;
         _loc1_ = 0;
         while(_loc1_ < this._subBtns.length)
         {
            ...
            if(this._subBtnsContainers[_loc4_] == null)
            {
               _loc4_++;
            }
            this._subBtns[_loc1_].addEventListener(MouseEvent.CLICK,this.__subBtnClick);
            this._subBtnsContainers[_loc4_].addChild(this._subBtns[_loc1_]);
            this._subBtnsGroups[_loc4_].addSelectItem(this._subBtns[_loc1_]);
            if(_loc1_ == 0)
            {
               this._subBtnsGroups[_loc4_].selectIndex = _loc1_;
            }
            _loc1_++;
         }
         ...
         _loc1_ = 0;
         while(_loc1_ < this._subBtnsContainers.length)
         {
            if(Boolean(this._subBtnsContainers[_loc1_]))
            {
               addChild(this._subBtnsContainers[_loc1_]);
               this._subBtnsContainers[_loc1_].visible = false;
               if(_loc1_ == 0)
               {
                  this._subBtnsContainers[_loc1_].visible = true;
               }
            }
            _loc1_++;
         }
      ...
      public function searchList(param1:Vector.<ShopItemInfo>) : void
      {
         ...
         CURRENT_PAGE = 1;
         var _loc3_:int = 0;
         while(_loc3_ < this._subBtnsContainers.length)
         {
            _loc2_ = this._subBtnsContainers[_loc3_] as HBox;
            if(Boolean(_loc2_))
            {
      ...
      private function runSearch() : void
      {
         ...
         if(CURRENT_PAGE > 0 && CURRENT_PAGE <= this._searchItemTotalPage)
         {
            ...
            while(_loc3_ < _loc2_)
            {
               ...
               if(Boolean(this._searchShopItemList[_loc3_ + _loc1_]))
               {
      ...
      private function __itemSelect(param1:ItemEvent) : void
      {
         ...
         var _loc3_:shop.view.ShopGoodItem = param1.currentTarget as ShopGoodItem;
         for each(_loc2_ in this._goodItems)
         {
      ...
      private function __itemClick(param1:ItemEvent) : void
      {
         ...
         var _loc8_:shop.view.ShopGoodItem = param1.currentTarget as ShopGoodItem;
         ...
         if(Boolean(_loc8_.shopItemInfo) && Boolean(_loc8_.shopItemInfo.TemplateInfo))
         {
            ...
            if(EquipType.dressAble(_loc8_.shopItemInfo.TemplateInfo))
            {
               ...
               if(_loc8_.shopItemInfo.TemplateInfo.NeedSex != 0 && this._genderGroup.selectIndex != _loc6_)
               {
      ...
      private function itemClick(param1:shop.view.ShopGoodItem) : void
      {
      ...
      private function showSubBtns(param1:int) : void
      {
         ...
         if(Boolean(this._subBtnsContainers[param1]))
         {
      ...
      public function gotoPage(param1:int = -1, param2:int = -1, param3:int = 1, param4:int = 1) : void
      {
         ...
         var _loc6_:int = 0;
         while(_loc6_ < this._subBtnsContainers.length)
         {
            _loc5_ = this._subBtnsContainers[_loc6_] as HBox;
            if(Boolean(_loc5_))
            {
