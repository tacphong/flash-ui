package shop.view
{
   ...
   public class ShopCheckOutView extends Sprite implements Disposeable
   {
      ...
      private var shopPresent:shop.view.ShopPresentView;
      
      protected var _list:Array;
      
      private var _shopPresentClearingFrame:shop.view.ShopPresentClearingFrame;
      ...
      private function notPresentGoods() : Array
      {
         ...
         var _loc2_:Array = [];
         for each(_loc1_ in this._tempList)
         {
            if(this._giveArray.indexOf(_loc1_) == -1)
            {
               _loc2_.push(_loc1_);
            }
         }
      ...
      private function onBuyedGoods(param1:CrazyTankSocketEvent) : void
      {
         ...
         {
            this._model.clearCurrentTempList(this._model.fittingSex ? int(1) : int(2));
            for each(_loc2_ in this._model.currentLeftList)
            {
               this._model.addTempEquip(_loc2_);
            }
         ...
         {
            _loc3_ = 0;
            while(_loc3_ < this._buyArray.length)
            {
               this._model.removeFromShoppingCar(this._buyArray[_loc3_] as ShopCarItemInfo);
               _loc3_++;
            }
