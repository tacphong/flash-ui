package shop.view
{
   ...
   public class SetsShopView extends ShopCheckOutView
   {
      ...
      public function initialize(param1:Array) : void
      {
         ...
         this.initEvent();
         setList(param1);
         _commodityPricesText2.text = _commodityPricesText3.text = "0";
      ...
      override protected function updateTxt() : void
      {
         var _loc1_:int = 0;
         var _loc2_:int = 0;
         var _loc3_:SetsShopItem = null;
         this._totalPrice = 0;
         var _loc4_:int = 0;
         while(_loc4_ < _cartList.numChildren)
         {
            _loc3_ = _cartList.getChildAt(_loc4_) as SetsShopItem;
            if(Boolean(_loc3_))
            {
               _loc2_++;
               if(_loc3_.selected)
               {
                  this._totalPrice += _loc3_.shopItemInfo.AValue1;
                  _loc1_++;
               }
            }
            _loc4_++;
      ...
      override protected function __purchaseConfirmationBtnClick(param1:MouseEvent = null) : void
      {
         ...
         {
            _loc2_ = _cartList.getChildAt(_loc5_) as SetsShopItem;
            if(Boolean(_loc2_) && _loc2_.selected)
            {
               _loc4_.push(_loc2_.shopItemInfo.GoodsID);
            }
            _loc5_++;
         }
         SocketManager.Instance.out.sendUseCard(-1,-1,_loc4_,1);
