package shop.view
{
   ...
   public class ShopLeftView extends Sprite implements Disposeable
   {
      ...
      private var prop:shop.view.ShopLeftViewPropCollection;
      ...
      {
         super();
         this.prop = new shop.view.ShopLeftViewPropCollection();
         this.prop.setup();
      ...
      private function __addTempEquip(param1:ShopEvent) : void
      {
         ...
         if(EquipType.dressAble(_loc4_.TemplateInfo) && _loc4_.CategoryID != EquipType.CHATBALL)
         {
            _loc2_ = int(EquipType.CategeryIdToPlace(_loc4_.CategoryID)[0]);
            _loc3_ = this._model.getBagItems(_loc2_,true);
            this.prop.playerCells[_loc3_].shopItemInfo = _loc4_;
            _loc4_.place = _loc2_;
            this._model.currentModel.setPartStyle(_loc4_.CategoryID,_loc4_.TemplateInfo.NeedSex,_loc4_.TemplateID,_loc4_.Color);
         }
         this.__updateCar(param1);
         this.updateButtons();
         this.prop.lastItem.shopItemInfo = _loc4_;
         this.__updateColorEditor();
         if(this.prop.panelBtnGroup.selectIndex != 1)
         {
            if(_loc4_.TemplateInfo.NeedSex == MALE)
            {
               ++this.prop.addedManNewEquip;
            }
            else if(_loc4_.TemplateInfo.NeedSex == FEMALE)
            {
      ...
      private function __fittingSexChanged(param1:ShopEvent = null) : void
      {
         ...
         var _loc6_:int = 0;
         while(_loc6_ < this.prop.playerCells.length)
         {
            _loc4_ = this._model.getBagItems(_loc6_);
            if(Boolean(this._model.currentModel.Bag.items[_loc4_]))
            {
               this.prop.playerCells[_loc6_].info = this._model.currentModel.Bag.items[_loc4_];
               this.prop.playerCells[_loc6_].locked = true;
            }
            else
            {
               this.prop.playerCells[_loc6_].shopItemInfo = null;
            }
            _loc6_++;
         }
         for each(_loc2_ in this._model.currentTempList)
         {
            _loc5_ = new ShopEvent("shop",_loc2_);
            this.__addTempEquip(_loc5_);
         }
         this.updateButtons();
         _loc3_ = this._model.currentTempList;
         if(_loc3_.length > 0)
         {
            this.prop.lastItem.shopItemInfo = _loc3_[_loc3_.length - 1];
         }
         else
         {
      ...
      {
         if(Boolean(param1.changedProperties[PlayerInfo.MONEY]) || Boolean(param1.changedProperties[PlayerInfo.GIFT]) || Boolean(param1.changedProperties[PlayerInfo.MEDAL]))
         {
      ...
      {
         var _loc2_:ShopCartItem = null;
         var _loc3_:ShopCarItemInfo = param1.param as ShopCarItemInfo;
         var _loc4_:uint = 0;
         while(this.prop.cartList.numChildren > 0)
         {
            _loc2_ = this.prop.cartList.getChildAt(_loc4_) as ShopCartItem;
            if(_loc2_.shopItemInfo == _loc3_)
            {
               break;
            }
            _loc4_++;
      ...
      private function __updateCar(param1:ShopEvent) : void
      {
         ...
         var _loc3_:uint = 0;
         var _loc4_:ShopCarItemInfo = param1.param as ShopCarItemInfo;
         var _loc5_:Array = this._model.allItems;
         var _loc6_:Array = new Array();
         var _loc7_:int = 0;
         while(_loc7_ < this.prop.cartList.numChildren)
         {
            _loc2_ = this.prop.cartList.getChildAt(_loc7_) as ShopCartItem;
            _loc6_.push(_loc2_.shopItemInfo);
            _loc7_++;
         }
         if(_loc6_.length < _loc5_.length)
         {
            this.addCarEquip(_loc4_);
         }
         else if(_loc6_.length == _loc5_.length)
         {
            ...
            {
               if(_loc5_.indexOf(_loc6_[_loc3_]) < 0)
               {
                  (this.prop.cartList.getChildAt(_loc3_) as ShopCartItem).shopItemInfo = _loc4_;
                  break;
               }
               _loc3_++;
            }
         ...
         else if(_loc6_.length > _loc5_.length)
         {
            ...
            while(_loc3_ < _loc6_.length)
            {
               if(_loc5_.indexOf(_loc6_[_loc3_]) < 0)
               {
                  ...
                  {
                     _loc2_ = this.prop.cartList.getChildAt(_loc7_) as ShopCartItem;
                     if(_loc2_.shopItemInfo == _loc6_[_loc3_])
                     {
                        _loc2_.removeEventListener(ShopCartItem.DELETE_ITEM,this.__deleteItem);
      ...
      private function __removeTempEquip(param1:ShopEvent) : void
      {
         ...
         var _loc5_:ShopCarItemInfo = param1.param as ShopCarItemInfo;
         if(this.prop.lastItem.shopItemInfo == _loc5_)
         {
            this.prop.lastItem.shopItemInfo = null;
         }
         this.__updateColorEditor();
         if(_loc5_.TemplateInfo.NeedSex == MALE)
         {
         ...
         else if(_loc5_.TemplateInfo.NeedSex == FEMALE)
         {
         ...
         if(param1.model == this._model.currentModel)
         {
            _loc2_ = _loc5_.place;
            _loc3_ = this._model.getBagItems(_loc2_,true);
            _loc4_ = this._model.currentModel.Bag.items[_loc2_];
            if(Boolean(_loc4_))
            {
               this.prop.playerCells[_loc3_].info = _loc4_;
               this.prop.playerCells[_loc3_].locked = true;
            }
            else
            {
               this.prop.playerCells[_loc3_].info = null;
            }
            this.updateButtons();
         }
         this.__updateCar(param1);
      ...
      private function setColorLayer(param1:int) : void
      {
         ...
         var _loc5_:ShopCarItemInfo = this.prop.lastItem.shopItemInfo;
         if(_loc5_ && EquipType.isEditable(_loc5_.TemplateInfo) && int(_loc5_.colorValue) != param1)
         {
            _loc2_ = this.prop.lastItem.editLayer - 1;
            _loc3_ = int(EquipType.CategeryIdToPlace(_loc5_.CategoryID)[0]);
            _loc4_ = _loc5_.Color.split("|");
            _loc4_[_loc2_] = String(param1);
            _loc5_.Color = _loc4_.join("|");
            this.prop.lastItem.setColor(_loc5_.Color);
            this._model.currentModel.setPartColor(_loc5_.CategoryID,_loc5_.Color);
         }
      }
      ...
      private function setSkinColor(param1:String) : void
      {
         ...
         if(Boolean(_loc2_) && _loc2_.CategoryID == EquipType.FACE)
         {
            _loc2_.skin = param1;
         }
      ...
      protected function __reductiveColor(param1:Event) : void
      {
         ...
         if(this.prop.colorEditor.selectedType == COLOR)
         {
            if(Boolean(_loc2_) && EquipType.isEditable(_loc2_.TemplateInfo))
            {
               _loc2_.Color = "";
