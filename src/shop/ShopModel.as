package shop
{
   ...
   public class ShopModel extends EventDispatcher
   {
      ...
      public function removeLatestItem() : void
      {
         ...
         var _loc5_:int = 0;
         var _loc6_:Array = this._sex ? this._manTempList : this._womanTempList;
         if(this.currentHistoryList.length > 0)
         {
            _loc1_ = this.currentHistoryList.pop();
            for each(_loc2_ in _loc1_)
            {
               this.removeTempEquip(_loc2_);
            }
         }
         var _loc7_:Array = [];
         var _loc8_:int = this.currentHistoryList.length - 1;
         while(_loc8_ > -1)
         {
            _loc3_ = this.currentHistoryList[_loc8_];
            for each(_loc4_ in _loc3_)
            {
               _loc5_ = this.currentTempListHasItem(_loc4_.TemplateInfo.CategoryID);
               if(_loc5_ <= -1)
               {
                  this.currentTempList.push(_loc4_);
                  dispatchEvent(new ShopEvent(ShopEvent.ADD_TEMP_EQUIP,_loc4_));
                  _loc4_.addEventListener(Event.CHANGE,this.__onItemChange);
               }
            }
            _loc8_--;
         }
      ...
      {
         var _loc2_:ShopCarItemInfo = null;
         var _loc3_:Array = this.currentTempList;
         for each(_loc2_ in _loc3_)
         {
            if(_loc2_.TemplateInfo.CategoryID == param1)
            {
               return _loc3_.indexOf(_loc2_);
            }
         }
         return -1;
      }
      ...
      public function random() : void
      {
         ...
         var _loc2_:int = 0;
         var _loc3_:Array = this._sex ? this.maleCollocation : this.femaleCollocation;
         var _loc4_:Array = [];
         for each(_loc1_ in _loc3_)
         {
            _loc2_ = Math.floor(Math.random() * _loc1_.length);
            _loc4_.push(this.fillToShopCarInfo(_loc1_[_loc2_]));
         }
         this.addTempEquip(_loc4_);
         this.updateCost();
      }
      ...
      public function addTempEquip(param1:*) : Boolean
      {
         ...
         var _loc9_:Boolean = this.isCarListMax();
         if(_loc9_)
         {
         ...
         if(param1 is Array)
         {
            ...
            for each(_loc4_ in _loc2_)
            {
               ...
               if(_loc5_ > -1)
               {
                  this.currentTempList.splice(_loc5_,1);
               }
               _loc6_ = this.fillToShopCarInfo(_loc4_);
               _loc6_.dressing = true;
               _loc6_.ModelSex = this.currentModel.Sex;
               this.currentTempList.push(_loc6_);
               dispatchEvent(new ShopEvent(ShopEvent.ADD_TEMP_EQUIP,_loc6_));
               this.updateCost();
               _loc6_.addEventListener(Event.CHANGE,this.__onItemChange);
               _loc3_.push(_loc6_);
            }
            this.currentHistoryList.push(_loc3_);
         }
         else
         {
            _loc7_ = this.currentTempListHasItem(param1.TemplateInfo.CategoryID);
            if(_loc7_ > -1)
            {
               this.currentTempList.splice(_loc7_,1);
            }
            _loc8_ = this.fillToShopCarInfo(param1);
            _loc8_.dressing = true;
            _loc8_.ModelSex = this.currentModel.Sex;
            this.currentTempList.push(_loc8_);
            dispatchEvent(new ShopEvent(ShopEvent.ADD_TEMP_EQUIP,_loc8_));
            this.updateCost();
            _loc8_.addEventListener(Event.CHANGE,this.__onItemChange);
            this.currentHistoryList.push([_loc8_]);
         }
         return !_loc9_;
      }
      ...
      public function isOverCount(param1:ShopItemInfo) : Boolean
      {
         var _loc2_:ShopCarItemInfo = null;
         var _loc3_:uint = 0;
         var _loc4_:Array = this.allItems;
         var _loc5_:int = 0;
         while(_loc5_ < _loc4_.length)
         {
            _loc2_ = _loc4_[_loc5_] as ShopCarItemInfo;
            if(param1.TemplateID == _loc2_.TemplateID)
            {
               _loc3_++;
            }
            _loc5_++;
         }
         return _loc3_ >= param1.LimitCount && param1.LimitCount != -1 ? Boolean(true) : Boolean(false);
      }
      ...
      public function removeTempEquip(param1:ShopCarItemInfo) : void
      {
         ...
         var _loc4_:int = this._manTempList.indexOf(param1);
         if(_loc4_ != -1)
         {
            this._manTempList.splice(_loc4_,1);
            _loc2_ = this._manModel;
         }
         else
         {
            _loc4_ = this._womanTempList.indexOf(param1);
            if(_loc4_ != -1)
            {
               this._womanTempList.splice(_loc4_,1);
               _loc2_ = this._womanModel;
            }
         }
         if(Boolean(_loc2_))
         {
            _loc3_ = _loc2_.Bag.items[param1.place];
            if(Boolean(_loc3_))
            {
               if(_loc3_.CategoryID >= 1 && _loc3_.CategoryID <= 6 || param1.CategoryID == EquipType.SUITS || param1.CategoryID == EquipType.WING)
               {
                  _loc2_.setPartStyle(param1.CategoryID,param1.TemplateInfo.NeedSex,_loc3_.TemplateID,_loc3_.Color);
               }
               if(param1.CategoryID == EquipType.FACE)
               {
                  _loc2_.Skin = this._self.Skin;
               }
            }
            ...
            dispatchEvent(new ShopEvent(ShopEvent.REMOVE_TEMP_EQUIP,param1,_loc2_));
         }
         this.updateCost();
         param1.removeEventListener(Event.CHANGE,this.__onItemChange);
      ...
      {
         var _loc2_:PlayerInfo = null;
         if(Boolean(this.currentModel) && Boolean(param1.changedProperties[PlayerInfo.STYLE]))
         {
            this._defaultModel = 1;
