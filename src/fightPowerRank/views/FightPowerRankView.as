package fightPowerRank.views
{
   ...
   public class FightPowerRankView extends Sprite implements IRightView
   {
      ...
      public function FightPowerRankView()
      {
         super();
      }
      ...
      {
         FightPowerRankManager.instance.view = this;
         this.initView();
         this.updateView();
         this.initTimer();
      }
      ...
      {
         this._bg = ComponentFactory.Instance.creat("wonderfulactivity.fightPowerRank.bg");
         addChild(this._bg);
         ...
         this._rankLabelTxt.text = LanguageMgr.GetTranslation("fightPowerRank.rankLabel",5000);
      ...
      public function updateView() : void
      {
         ...
         var _loc1_:* = 0;
         var _loc2_:FightPowerRankVo = null;
         var _loc3_:GmActivityInfo = FightPowerRankManager.instance.xmlData;
         ...
         this._checkBg.tipData = LanguageMgr.GetTranslation("fightPowerRank.helpTxt",_loc3_.remain2);
         _loc6_ = FightPowerRankManager.instance.rankList;
         _loc1_ = FightPowerRankManager.instance.myConsume;
         this._myRank = -1;
         var _loc7_:int = 0;
         while(_loc7_ <= _loc6_.length - 1)
         {
            _loc2_ = _loc6_[_loc7_] as FightPowerRankVo;
         ...
         if(this._myRank >= 0)
         {
            ...
            else
            {
               ...
               _loc8_ = (_loc6_[this._myRank - 1] as FightPowerRankVo).consume - _loc1_ + 1;
               this._rankLabelTxt.text = LanguageMgr.GetTranslation("fightPowerRank.rankLabel",_loc8_);
            }
            if(FightPowerRankManager.instance.status == 2)
            {
               this._rankLabelTxt.visible = true;
         ...
         else
         {
            ...
            if(_loc6_.length > 0)
            {
               _loc8_ = (_loc6_[_loc6_.length - 1] as FightPowerRankVo).consume - _loc1_ + 1;
            }
            else
            {
               _loc8_ = 1;
            }
            this._outOfRankLabel.text = LanguageMgr.GetTranslation("fightPower.outOfRank");
            this._rankLabelTxt.text = LanguageMgr.GetTranslation("fightPowerRank.outOfRankLabel",_loc1_,_loc8_);
            if(FightPowerRankManager.instance.status == 2)
            {
               this._rankLabelTxt.visible = true;
               this._rankLabelTxt.textColor = 16711680;
      ...
      {
         var _loc1_:FightPowerRankItem = null;
         var _loc2_:Array = FightPowerRankManager.instance.rankList;
         var _loc3_:Array = FightPowerRankManager.instance.xmlData.giftbagArray;
         this._vbox.removeAllChild();
         var _loc4_:int = 0;
         while(_loc4_ <= _loc2_.length - 1)
         {
            _loc1_ = new FightPowerRankItem(_loc4_);
      ...
      {
         var _loc1_:Date = DateUtils.getDateByStr(FightPowerRankManager.instance.xmlData.endTime);
      ...
      {
         FightPowerRankManager.instance.view = null;
         WonderfulActivityManager.Instance.delTimerFun("consumeRank");
