package quest
{
   ...
   public class QuestRewardView extends Sprite implements Disposeable
   {
      ...
      {
         this._bg.setFrame(param1 == TaskMainFrame.NORMAL ? int(1) : int(2));
         this._eligiblyWord.setFrame(param1 == TaskMainFrame.NORMAL ? int(1) : int(2));
         this._eligiblyBG.setFrame(param1 == TaskMainFrame.NORMAL ? int(1) : int(2));
         this._style = param1;
      }
      ...
      private function update() : void
      {
         ...
         if(this._info.RewardBuffID != 0)
         {
            ...
            if(Boolean(this._info.data) && Boolean(this._info.data.quality))
            {
         ...
         this._bg.height = this._itemAwardPosY > 220 ? Number(this._itemAwardPosY + 10) : Number(220);
      ...
      private function drawItemAwardSet(param1:Array, param2:Boolean = false) : void
      {
         ...
         while(_loc6_ < param1.length)
         {
            ...
            _loc3_.taskType = this._style;
            _loc3_.opitional = param2;
            _loc3_.x = _loc6_ % 2 * 186 + this.ROW_X;
            _loc3_.y = this._itemAwardPosY + int(_loc6_ / 2) * this.REWARDCELL_HEIGHT;
            ...
            if(_loc4_.IsCount && this._info.data && Boolean(this._info.data.quality))
            {
               _loc5_.Count = _loc4_.count * this._info.data.quality;
            }
            ItemManager.fill(_loc5_);
            if(Boolean(_loc5_) && _loc5_.TemplateID != 0)
            {
               _loc3_.visible = true;
               _loc3_.info = _loc5_;
               this._itemAwardCells.push(_loc3_);
               if(param2)
               {
                  TaskManager.itemAwardSelected = -1;
                  _loc3_.canBeSelected();
                  _loc3_.addEventListener(RewardSelectedEvent.ITEM_SELECTED,this.__chooseItemReward);
                  this._optionalCellArr.push(_loc3_);
               }
               addChild(_loc3_);
            }
            else
