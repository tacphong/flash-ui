package auctionHouse.view
{
   ...
   public class AuctionBuyView extends Sprite implements Disposeable
   {
      ...
      private var _bidMoney:auctionHouse.view.BidMoneyView;
      
      private var _right:auctionHouse.view.AuctionBuyRightView;
      ...
      private function __selectRightStrip(param1:AuctionHouseEvent) : void
      {
         ...
         if(Boolean(_loc2_))
         {
            ...
            this._mouthful_btnR.enable = this._mouthful_btn.enable = _loc2_.Mouthful == 0 ? Boolean(false) : Boolean(true);
            this._bid_btnR.enable = this._bid_btn.enable = _loc2_.BuyerID == PlayerManager.Instance.Self.ID ? Boolean(false) : Boolean(true);
      ...
      private function __mouthFull(param1:MouseEvent) : void
      {
         ...
         this._btClickLock = true;
         var _loc3_:AuctionGoodsInfo = this._right.getSelectInfo();
         if(_loc3_.Mouthful > PlayerManager.Instance.Self.Money)
         {
         ...
         var _loc4_:BaseAlerFrame = AlertManager.Instance.simpleAlert(LanguageMgr.GetTranslation("AlertDialog.Info"),LanguageMgr.GetTranslation("tank.auctionHouse.view.AuctionBrowseView.buy"),LanguageMgr.GetTranslation("ok"),LanguageMgr.GetTranslation("cancel"),false,false,false,LayerManager.ALPHA_BLOCKGOUND);
      ...
      private function __callMouthFull() : void
      {
         ...
         if(this._btClickLock)
         {
            ...
            if(Boolean(_loc2_))
            {
               ...
               IMController.Instance.saveRecentContactsID(_loc2_.AuctioneerID);
      ...
      private function __cannel() : void
      {
         ...
         if(Boolean(_loc1_))
         {
            this._mouthful_btnR.enable = this._mouthful_btn.enable = _loc1_.Mouthful == 0 ? Boolean(false) : Boolean(true);
            this._bid_btnR.enable = this._bid_btn.enable = _loc1_.BuyerID == PlayerManager.Instance.Self.ID ? Boolean(false) : Boolean(true);
         }
         else
         {
