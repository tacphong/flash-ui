package store
{
   ...
   public class StoreTips extends Sprite implements Disposeable
   {
      ...
      private function showHoleTip(param1:InventoryItemInfo) : String
      {
         ...
         var _loc4_:String = "";
         var _loc5_:String = LanguageMgr.GetTranslation("store.storeTip.openHole");
         if(param1.CategoryID == EquipType.HEAD || param1.CategoryID == EquipType.CLOTH)
         {
            ...
            {
               _loc5_ += SPACINGIII + LanguageMgr.GetTranslation("store.storeTip.weaponOpenProperty");
               _loc4_ = LanguageMgr.GetTranslation("store.storeTip.OpenProperty");
            }
            if(param1.StrengthenLevel == 6)
            {
               _loc5_ += SPACINGIII + LanguageMgr.GetTranslation("store.storeTip.clothOpenDefense");
               _loc4_ = LanguageMgr.GetTranslation("store.storeTip.OpenDefense");
            }
         }
         else if(param1.CategoryID == EquipType.ARM)
         {
            ...
            {
               _loc5_ += SPACINGIII + LanguageMgr.GetTranslation("store.storeTip.weaponOpenAttack");
               _loc4_ = LanguageMgr.GetTranslation("store.storeTip.OpenAttack");
            }
         }
         ...
         {
            _loc2_ = param1.Hole.split("|");
            _loc3_ = param1.StrengthenLevel / 3;
            if(_loc2_[_loc3_ - 1].split(",")[1] > 0 && param1["Hole" + _loc3_] >= 0)
            {
               this._lastTipString += "\n" + _loc5_;
               return _loc4_;
            }
