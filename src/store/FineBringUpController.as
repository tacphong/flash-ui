package store
{
   ...
   public class FineBringUpController extends EventDispatcher
   {
      ...
      /*public function eatBtnClick(param1:InventoryItemInfo, param2:InventoryItemInfo = null) : void
      {
		 ...
		 if(_quickEatInfo.TemplateID == EquipType.CULTURE_ITEM4)
		 {
			 ...
			 _selectNumView.maxCount = 9999;
			 _selectNumView.item = _quickEatInfo;
			 _selectNumView.TitleTxt = LanguageMgr.GetTranslation("tank.view.bagII.GoodsTipPanel.bringUp");
			 ...
			 _loc3_ = ComponentFactory.Instance.creatComponentByStylename("evolutionAler.content.text");
			 _loc3_.htmlText = LanguageMgr.GetTranslation("store.frame.evolution.band.prompt");
			 _loc3_.x = 62;
			 _loc3_.y = 105;
			 _selectNumView.addChild(_loc3_);
	  ...
	  public function eatBtnClick(param1:InventoryItemInfo, param2:InventoryItemInfo = null) : Boolean
	  {
		  var _loc3_:* = null;
		  _eatBtnClk = true;
		  hideNewHandTip();
		  ...
		  if(_quickEatInfo.TemplateID == 12252)
		  {
			  if(_selectNumView == null)
			  {
				  _selectNumView = ComponentFactory.Instance.creatComponentByStylename("store.selectNumView");
			  ...
			  _selectNumView.maxCount = 9999;
			  _selectNumView.item = _quickEatInfo;
			  _selectNumView.TitleTxt = LanguageMgr.GetTranslation("tank.view.bagII.GoodsTipPanel.bringUpTitle");
			  _selectNumView.ContentTxt = LanguageMgr.GetTranslation("stor.view.evolution.alter.selectNum");
			  _selectNumView.callback = materialNumSelect;
			  _loc3_ = ComponentFactory.Instance.creatComponentByStylename("evolutionAler.content.text");
			  _loc3_.htmlText = LanguageMgr.GetTranslation("store.frame.evolution.band.prompt");
	  ...
	  {
		  _singleNum = param1;
		  GameInSocketOut.sendCultureBringUp(1,_quickEatInfo.Place,_singleNum);
	  }
      ...
      public function getCanBringUpData() : BagInfo
      {
		 ...
		 var _loc2_:DictionaryData = PlayerManager.Instance.Self.getBag(1).items;
		 for each(item in _loc2_)
		 {
			 if(item.TemplateID == EquipType.CULTURE_ITEM4)
			 {
				 _bagBringUpInfo.addItem(item);
			 }
		 }
