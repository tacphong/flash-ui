package store.view.fusion
{
   ...
   public class StoreIIFusionBG extends Sprite implements IStoreViewBG
   {
      ...
      private var _accessoryFrameII:store.view.fusion.AccessoryFrameII;
      ...
      public static function getRemainIndexByEmpty(param1:int, param2:IStoreViewBG) : String
      {
         ...
         if(param2 is StoreIIFusionBG)
         {
            ...
            while(_loc4_ < 5)
            {
               _loc3_ = (param2 as StoreIIFusionBG).area[_loc4_];
               if(!_loc3_.info)
               {
                  _loc6_.push(_loc3_.index);
               }
               _loc4_++;
            }
            switch(param1)
            {
               case 2:
                  _loc5_ = _loc6_.concat(findDiff(_loc6_)).slice(0,param1).splice(",");
                  break;
               case 3:
                  _loc5_ = _loc6_.concat(findDiff(_loc6_)).slice(0,param1).splice(",");
            }
      ...
      private static function getAutoSplitSendParam(param1:String, param2:int) : Array
      {
         ...
         if(Boolean(_loc7_) && param2 >= 1)
         {
            _loc3_ = param2 % _loc7_.length;
            _loc4_ = int(param2 / _loc7_.length);
            _loc5_ = 0;
      ...
      private static function clearFusion(param1:IStoreViewBG, param2:Array = null) : void
      {
         ...
         while(_loc5_ < param2.length)
         {
            ...
            {
               _loc3_ = (param1 as StoreIIFusionBG).area[_loc4_];
               if(Boolean(_loc3_.info) && _loc3_.index == int(param2[_loc5_]))
               {
                  SocketManager.Instance.out.sendMoveGoods(BagInfo.STOREBAG,_loc3_.index,_loc3_.itemBagType,-1);
                  break;
               }
      ...
      private function init() : void
      {
         ...
         this._accessoryFrameII = new store.view.fusion.AccessoryFrameII();
      ...
      private function exeItemTip() : Boolean
      {
         ...
         {
            _loc1_ = int(PlayerManager.Instance.Self.StoreBag.items[1].FusionType);
            _loc2_ = 2;
            while(_loc2_ <= 4)
            {
      ...
      private function getCellsPoint() : void
      {
         ...
         while(_loc2_ < 6)
         {
            _loc1_ = ComponentFactory.Instance.creatCustomObject("store.Fusionpoint" + _loc2_);
         ...
         while(_loc3_ < 5)
         {
            ...
            if(_loc2_ is FusionItemCell)
            {
               ...
               else
               {
                  ...
                  {
                     this.__moveGoods(_loc4_,1);
                     DragManager.acceptDrag(_loc2_,DragEffect.NONE);
                     return;
                  }
                  if(_loc2_.info == null)
                  {
                     this.__moveGoods(_loc4_,_loc2_.index);
                     DragManager.acceptDrag(_loc2_,DragEffect.NONE);
                     return;
                  }
               }
      ...
      private function _showDontClickTip() : Boolean
      {
         ...
         var _loc2_:int = 0;
         var _loc3_:int = 0;
         var _loc4_:int = this.checkItemEmpty();
         if(_loc4_ == 0)
         {
         ...
         if(_loc4_ == 4)
         {
            ...
            while(_loc3_ < _loc2_)
            {
               if(_loc1_ != PlayerManager.Instance.Self.StoreBag.items[_loc3_].FusionType)
               {
      ...
      public function refreshData(param1:Dictionary) : void
      {
         ...
         if(this._items[1].info != null && this._items[2].info != null && this._items[3].info != null && this._items[4].info != null)
         {
            ...
            _loc4_ = int(PlayerManager.Instance.Self.StoreBag.items[1].FusionType);
            _loc5_ = 5;
            _loc6_ = 2;
            while(_loc6_ < _loc5_)
            {
      ...
      private function __upPreview(param1:CrazyTankSocketEvent) : void
      {
         var _loc2_:Boolean = false;
         var _loc3_:PreviewInfoII = null;
         var _loc4_:PreviewInfoII = null;
         this.hideArr();
         this._resulteFusion = false;
         var _loc5_:int = param1.pkg.readInt();
         this._itemsPreview = new Array();
         var _loc6_:int = 0;
         while(_loc6_ < _loc5_)
         {
            _loc3_ = new PreviewInfoII();
            _loc3_.data(param1.pkg.readInt(),param1.pkg.readInt());
            _loc3_.rate = param1.pkg.readInt();
            this._itemsPreview.push(_loc3_);
            _loc6_++;
         }
         _loc2_ = param1.pkg.readBoolean();
         var _loc7_:int = 0;
         while(_loc7_ < this._itemsPreview.length)
         {
            _loc4_ = this._itemsPreview[_loc7_] as PreviewInfoII;
