package store.view.shortcutBuy
{
   ...
   public class ShortcutBuyCell extends BaseCell
   {
      ...
      private function initII() : void
      {
         ...
         var _loc3_:FilterFrameText = ComponentFactory.Instance.creatComponentByStylename("store.ShortCutTxt");
         _loc3_.mouseEnabled = false;
         if(EquipType.isComposeStone(this._itemInfo))
         {
            _loc3_.text = LanguageMgr.GetTranslation("store.view.ShortcutBuyCell.Stone" + this._itemInfo.Property3);
         }
         else
         {
            _loc1_ = "";
            ...
            while(_loc2_ < this._nameArr.length)
            {
               ...
               {
                  _loc1_ = String(this._nameArr[_loc2_]);
                  break;
               }
               _loc2_++;
            }
