package store.view.transfer
{
   ...
   public class StoreIITransferBG extends Sprite implements IStoreViewBG
   {
      ...
      private var _area:store.view.transfer.TransferDragInArea;
      
      private var _items:Vector.<store.view.transfer.TransferItemCell>;
      ...
      {
         var _loc1_:store.view.transfer.TransferItemCell = null;
         var _loc2_:int = 0;
         _loc1_ = null;
         ...
         this.transShine.mouseChildren = this.transShine.mouseEnabled = false;
         ...
         while(_loc2_ < 2)
         {
            _loc1_ = new store.view.transfer.TransferItemCell(_loc2_);
            _loc1_.addEventListener(Event.CHANGE,this.__itemInfoChange);
            _loc1_.x = this._pointArray[_loc2_].x;
            _loc1_.y = this._pointArray[_loc2_].y;
            addChild(_loc1_);
            this._items.push(_loc1_);
            _loc2_++;
         }
         this._area = new store.view.transfer.TransferDragInArea(this._items);
      ...
      private function getCellsPoint() : void
      {
         ...
         while(_loc2_ < 2)
         {
            _loc1_ = ComponentFactory.Instance.creatCustomObject("store.Transferpoint" + _loc2_);
      ...
      public function get area() : Vector.<store.view.transfer.TransferItemCell>
      {
         return this._items;
      }
      ...
      {
         var _loc2_:store.view.transfer.TransferItemCell = null;
         if(param1 == null)
         {
            return;
         }
         ...
         for each(_loc2_ in this._items)
         {
            if(_loc2_.info == null)
            {
               if(this._items[0].info && this._items[0].info.CategoryID != _loc3_.CategoryID || this._items[1].info && this._items[1].info.CategoryID != _loc3_.CategoryID)
               {
               ...
               DragManager.acceptDrag(_loc2_,DragEffect.NONE);
               return;
            }
         }
      }
      ...
      private function __transferHandler(param1:MouseEvent) : void
      {
         ...
         var _loc4_:store.view.transfer.TransferItemCell = this._items[0] as TransferItemCell;
         var _loc5_:store.view.transfer.TransferItemCell = this._items[1] as TransferItemCell;
         if(this._showDontClickTip())
         {
            return;
         }
         if(Boolean(_loc4_.info) && Boolean(_loc5_.info))
         {
            ...
            if(EquipType.isArm(_loc4_.info) || EquipType.isCloth(_loc4_.info) || EquipType.isHead(_loc4_.info) || EquipType.isArm(_loc5_.info) || EquipType.isCloth(_loc5_.info) || EquipType.isHead(_loc5_.info))
            {
               this.confirmTransferHole();
            }
            else
            {
               _loc3_ = AlertManager.Instance.simpleAlert(LanguageMgr.GetTranslation("AlertDialog.Info"),LanguageMgr.GetTranslation("store.view.transfer.StoreIITransferBG.sure"),LanguageMgr.GetTranslation("ok"),LanguageMgr.GetTranslation("cancel"),false,true,true,LayerManager.ALPHA_BLOCKGOUND);
      ...
      private function __itemInfoChange(param1:Event) : void
      {
         ...
         var _loc2_:store.view.transfer.TransferItemCell = this._items[0];
         var _loc3_:store.view.transfer.TransferItemCell = this._items[1];
         if(Boolean(_loc2_.info))
         {
            _loc2_.categoryId = -1;
            if(Boolean(_loc3_.info))
            {
         ...
         if(Boolean(_loc2_.info) && Boolean(_loc3_.info))
         {
         ...
         if(Boolean(_loc2_.info) && !_loc3_.info)
         {
            ConsortiaRateManager.instance.sendTransferShowLightEvent(_loc2_.info,true);
         }
         else if(Boolean(_loc3_.info) && !_loc2_.info)
         {
      ...
      {
         var _loc1_:store.view.transfer.TransferItemCell = this._items[0];
         var _loc2_:store.view.transfer.TransferItemCell = this._items[1];
