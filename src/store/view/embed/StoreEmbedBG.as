package store.view.embed
{
   ...
   public class StoreEmbedBG extends Sprite implements IStoreViewBG
   {
      ...
      private var _stoneCells:Vector.<store.view.embed.EmbedStoneCell>;
      
      private var _embedItemCell:store.view.embed.EmbedItemCell;
      
      private var _quick:QuickBuyFrame;
      
      private var _embedBackoutDownItem:BaseButton;
      
      private var _openFiveHoleBtn:store.view.embed.MultipleButton;
      
      private var _openSixHoleBtn:store.view.embed.MultipleButton;
      
      private var _embedStoneCell:store.view.embed.EmbedStoneCell;
      
      private var _embedBackoutMouseIcon:ScaleFrameImage;
      
      private var _help:BaseButton;
      
      private var _embedBackoutBtn:store.view.embed.EmbedBackoutButton;
      ...
      private var _hole5ExpBar:store.view.embed.HoleExpBar;
      
      private var _hole6ExpBar:store.view.embed.HoleExpBar;
      ...
      {
         var _loc1_:int = 0;
         var _loc2_:store.view.embed.EmbedStoneCell = null;
         _loc1_ = 0;
         _loc2_ = null;
         ...
         this.getCellsPoint();
         this._embedItemCell = new store.view.embed.EmbedItemCell(0);
      ...
      private function getCellsPoint() : void
      {
         ...
         while(_loc2_ < 7)
         {
            _loc1_ = ComponentFactory.Instance.creatCustomObject("store.Embedpoint" + _loc2_);
      ...
      private function getDrillByLevel(param1:int) : InventoryItemInfo
      {
         ...
         var _loc3_:DictionaryData = PlayerManager.Instance.Self.PropBag.items;
         for each(_loc2_ in _loc3_)
         {
            if(EquipType.isDrill(_loc2_) && _loc2_.Level == param1 + 1)
            {
               return _loc2_;
            }
         }
            ...
            if(Boolean(_loc2_))
            {
               if(_loc2_ is StoneCell)
               {
                  if(_loc2_.info == null)
                  {
                     ...
                     {
                        SocketManager.Instance.out.sendMoveGoods(_loc4_.BagType,_loc4_.Place,BagInfo.STOREBAG,_loc2_.index,1);
               ...
               else if(_loc2_ is EmbedItemCell)
               {
                  ...
                  while(_loc3_ < 7)
                  {
                     if(_loc4_.CategoryID == EquipType.HEAD || _loc4_.CategoryID == EquipType.CLOTH || _loc4_.CategoryID == EquipType.ARM)
                     {
      ...
      private function __EmbedBackoutFrame(param1:Event) : void
      {
         SoundManager.instance.play("008");
         param1.stopImmediatePropagation();
         if(Boolean(this._embedStoneCell) && param1.type == MouseEvent.CLICK)
         {
      ...
      public function refreshData(param1:Dictionary) : void
      {
         ...
         if(this._embedItemCell.info == null)
         {
            this._hole5ExpBar.visible = this._hole6ExpBar.visible = false;
         }
         if(Boolean(this._embedItemCell.itemInfo) && this._embedItemCell.itemInfo.Hole5Level >= 0)
         {
         ...
         if(Boolean(this._embedItemCell.itemInfo) && this._embedItemCell.itemInfo.Hole6Level >= 0)
         {
      ...
      public function stoneStartShine(param1:int, param2:InventoryItemInfo) : void
      {
         ...
         var _loc6_:InventoryItemInfo = this._embedItemCell.itemInfo;
         var _loc7_:Array = _loc6_.Hole.split("|");
         var _loc8_:int = 0;
         while(_loc8_ < this._stoneCells.length)
         {
            _loc3_ = int(_loc6_["Hole" + (_loc8_ + 1)]);
            if(_loc8_ < 4)
            {
               _loc4_ = String(_loc7_[_loc8_]);
               _loc5_ = _loc4_.split(",");
               if(_loc6_["Hole" + (_loc8_ + 1)] >= 0 && (this._stoneCells[_loc8_] as EmbedStoneCell).StoneType == param1)
               {
                  this._stoneCells[_loc8_].startShine();
               }
            }
            else if((this._stoneCells[_loc8_] as EmbedStoneCell).StoneType == param1 && _loc6_["Hole" + (_loc8_ + 1) + "Level"] > 0)
            {
               this._stoneCells[_loc8_].startShine();
            }
            _loc8_++;
      ...
      {
         var _loc1_:store.view.embed.EmbedStoneCell = null;
