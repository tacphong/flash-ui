package store.fineStore.view.pageBringUp
{
	...
	public class FineBringUpCell extends LockableBagCell
	{
		...
		{
			var _loc7_:Shape;
			(_loc7_ = new Shape()).graphics.beginFill(0,0);
			_loc7_.graphics.drawRect(0,0,65,65);
			_loc7_.graphics.endFill();
			var _loc6_:Bitmap;
			(_loc6_ = new Bitmap(new BitmapData(65,65))).bitmapData.draw(_loc7_);
			(param4 = _loc6_).alpha = 0;
		...
		{
			super.info = param1;
			param1 && (_lastLevel = int(param1.Property1));
			if(_tipData && _tipData is GoodTipInfo)
			{
				(_tipData as GoodTipInfo).latentEnergyItemId = _latentEnergyItemId;
			}
			...
			_observer && _observer.update(param1);
		}
		...
		private function equipMoveHandler(param1:LatentEnergyEvent) : void
		{
			...
			if(param1.info.TemplateID == EquipType.CULTURE_ITEM4)
			{
				return;
			}
			if(info)
