package accumulativeLogin.view
{
   ...
   public class AccumulativeLoginView extends Sprite implements IRightView
   {
      ...
      private var _movieVector:Vector.<accumulativeLogin.view.AccumulativeMovieSprite>;
      ...
      {
         this._loginDayNum = PlayerManager.Instance.Self.accumulativeLoginDays > 7 ? int(7) : int(PlayerManager.Instance.Self.accumulativeLoginDays);
      ...
      private function initView() : void
      {
         ...
         var _loc4_:accumulativeLogin.view.AccumulativeMovieSprite = null;
         ...
         while(_loc5_ < 8)
         {
            _loc1_ = ComponentFactory.Instance.creatComponentByStylename("wonderfulactivity.accumulativeLogin.dayTxt");
            addChild(_loc1_);
            _loc1_.text = "" + _loc5_;
            _loc1_.x = _loc5_ == 7 ? Number(700) : Number(334 + 62 * (_loc5_ - 1));
            _loc1_.y = 150;
            this._dayTxtArr.push(_loc1_);
            _loc5_++;
         }
         ...
         while(_loc6_ < 6)
         {
            _loc2_ = ComponentFactory.Instance.creat("wonderfulactivity.login.barItem");
            _loc2_.x = 334 + 62 * _loc6_;
            _loc2_.y = 170;
            addChild(_loc2_);
            this._progressBarItemArr.push(_loc2_);
            _loc6_++;
         }
         ...
         while(_loc7_ < 7)
         {
            ...
            if(_loc7_ != 6)
            {
               _loc3_.graphics.drawRect(this._progressBarItemArr[_loc7_].x,170,this._progressBarItemArr[_loc7_].width,this._progressBarItemArr[_loc7_].height);
            }
            else
            {
               _loc3_.graphics.drawRect(this._progressBarItemArr[5].x + 58,170,this._progressBarItemArr[5].width + 8,this._progressBarItemArr[5].height);
            }
            _loc3_.graphics.endFill();
            _loc3_.addEventListener(MouseEvent.CLICK,this.__showAwardHandler);
            addChild(_loc3_);
            this._clickSpriteArr.push(_loc3_);
            _loc7_++;
         }
         this._hBox = ComponentFactory.Instance.creatComponentByStylename("wonderful.accumulativeLogin.Hbox");
         addChild(this._hBox);
         var _loc8_:int = 0;
         while(_loc8_ < this._movieStringArr.length)
         {
            _loc4_ = new accumulativeLogin.view.AccumulativeMovieSprite(this._movieStringArr[_loc8_]);
            _loc4_.addEventListener(MouseEvent.CLICK,this.__onClickHandler);
            addChild(_loc4_);
            PositionUtils.setPos(_loc4_,"wonderful.accumulativeLogin.moviePos" + (_loc8_ + 1));
            this._movieVector.push(_loc4_);
            _loc8_++;
         }
      ...
      {
         var _loc2_:accumulativeLogin.view.AccumulativeMovieSprite = null;
      ...
      {
         var _loc1_:accumulativeLogin.view.AccumulativeMovieSprite = null;
         var _loc2_:accumulativeLogin.view.AccumulativeMovieSprite = null;
      ...
      private function initViewWithData() : void
      {
         ...
         var _loc2_:AccumulativeLoginRewardData = null;
         var _loc3_:Sprite = null;
         this.checkMovieCanClick();
         this._loginDayTxt.text = "" + this._loginDayNum;
         if(this._loginDayNum < 7)
         {
            this._progressBar.width = Boolean(this._progressBarItemArr[this._loginDayNum - 1]) ? Number(this._progressBarItemArr[this._loginDayNum - 1].x - 265) : Number(0);
         ...
         {
            _loc1_ = new Array();
            for each(_loc2_ in this._dataDic[_loc4_])
            {
               _loc3_ = this.createBagCellSp(_loc2_,_loc4_);
               _loc1_.push(_loc3_);
            }
            this._dayGiftPackDic[_loc4_] = _loc1_;
            _loc4_++;
         }
         var _loc5_:int = 0;
         while(_loc5_ < this._movieVector.length)
         {
            this._movieVector[_loc5_].tipData = this._fiveWeaponArr[_loc5_].tipData;
            this._movieVector[_loc5_].data = this._dataDic[7][_loc5_];
            _loc5_++;
         }
      }
