package calendar
{
   ...
   public class CalendarManager
   {
      ...
      private static var _ins:calendar.CalendarManager;
       
      
      private var _localVisible:Boolean = false;
      
      private var _model:calendar.CalendarModel;
      ...
      public static function getInstance() : calendar.CalendarManager
      {
         if(_ins == null)
         {
            _ins = new calendar.CalendarManager();
         }
         return _ins;
      }
      ...
      public function open() : void
      {
         ...
         if(this._initialized && !this._localVisible && Boolean(this._today))
         {
            this._localVisible = true;
            this._model = new calendar.CalendarModel(this._today,this._signCount,this._dayLogDic,this._signAwards,this._signAwardCounts,this._eventActives);
            this._model.luckyNum = this._luckyNum;
            this._model.myLuckyNum = this._myLuckyNum;
            _loc1_ = new Date();
      ...
      private function localToNextDay(param1:calendar.CalendarModel, param2:Date) : void
      {
         var _loc3_:int = 0;
         var _loc4_:int = 0;
         if(param2.date == 1)
      ...
      public function sign(param1:Date) : Boolean
      {
         ...
         {
            _loc2_ = this._model.today;
            if(param1.fullYear == _loc2_.fullYear && param1.month == _loc2_.month && param1.date == _loc2_.date && !this._model.hasSigned(param1))
            {
               ...
               _loc5_ = true;
               _loc3_ = int(this._model.awardCounts.length);
               _loc4_ = 0;
               while(_loc4_ < _loc3_)
               {
                  if(this._model.signCount == this._model.awardCounts[_loc4_])
                  {
                     this.receive(this._model.awardCounts[_loc4_],this._model.awards);
                     return _loc5_;
                  }
                  _loc4_++;
               }
      ...
      {
         var _loc3_:DaylyGiveInfo = null;
         var _loc4_:SignAwardFrame = null;
         SocketManager.Instance.out.sendSignAward(param1);
         var _loc5_:Array = [];
         for each(_loc3_ in param2)
         {
            if(_loc3_.AwardDays == param1)
            {
               _loc5_.push(_loc3_);
            }
         }
         _loc4_ = ComponentFactory.Instance.creatComponentByStylename("SignAwardFrame");
         _loc4_.show(param1,_loc5_);
         LayerManager.Instance.addToLayer(_loc4_,LayerManager.GAME_DYNAMIC_LAYER,true,LayerManager.ALPHA_BLOCKGOUND);
      }
      ...
      {
         var _loc1_:BaseLoader = LoaderManager.Instance.creatLoader(PathManager.solveRequestPath("ActiveConvertItemInfo.xml"),BaseLoader.COMPRESS_TEXT_LOADER);
         _loc1_.loadErrorMessage = LanguageMgr.GetTranslation("ddt.loader.loadingActivityInformationFailure");
         _loc1_.analyzer = new ActiveExchangeAnalyzer(this.setActivityExchange);
         _loc1_.addEventListener(LoaderEvent.LOAD_ERROR,this.__onLoadError);
         _loc1_.addEventListener(LoaderEvent.COMPLETE,this.__complete);
         return _loc1_;
      }
      ...
      {
         this._activeExchange = param1.list;
      }
