package email.model
{
   ...
   public class EmailModel extends EventDispatcher
   {
      ...
      public function set emails(param1:Array) : void
      {
         ...
         {
            _loc2_ = this.calculateRemainTime(param1[_loc3_].SendTime,param1[_loc3_].ValidDate);
            if(_loc2_ > -1)
            {
               this._emails.push(param1[_loc3_]);
            }
            _loc3_++;
         }
         this.getNoReadMails();
      ...
      {
         var _loc2_:EmailInfo = null;
         var _loc3_:Array = [];
         var _loc4_:int = 0;
         while(_loc4_ < param1.length)
         {
            _loc2_ = param1[_loc4_] as EmailInfo;
            if(Boolean(_loc2_))
            {
               if(MailManager.Instance.calculateRemainTime(_loc2_.SendTime,_loc2_.ValidDate) > 0)
               {
                  _loc3_.push(_loc2_);
               }
            }
      ...
      public function getNoReadMails() : void
      {
         ...
         {
            if(Boolean(SharedManager.Instance.spacialReadedMail[PlayerManager.Instance.Self.ID]) && SharedManager.Instance.spacialReadedMail[PlayerManager.Instance.Self.ID].indexOf(_loc1_.ID) > -1)
            {
               _loc1_.IsRead = true;
            }
            if(!_loc1_.IsRead)
      ...
      {
         var _loc2_:int = int(this._emails.length);
         var _loc3_:uint = 0;
         while(_loc3_ < _loc2_)
         {
      ...
      public function getViewData() : Array
      {
         ...
         if(Boolean(this.currentDate))
         {
            _loc1_ = this.currentPage * 7 - 7;
            _loc2_ = _loc1_ + 7 > this.currentDate.length ? int(int(this.currentDate.length)) : int(int(_loc1_ + 7));
            _loc3_ = this.currentDate.slice(_loc1_,_loc2_);
         }
         return _loc3_;
      }
