package times
{
   ...
   public class TimesController extends EventDispatcher
   {
      ...
      private static var _instance:times.TimesController;
      ...
      public static function get Instance() : times.TimesController
      {
         if(_instance == null)
         {
            _instance = new times.TimesController();
         }
         return _instance;
      }
      ...
      private function __gotoContent(param1:TimesEvent) : void
      {
         ...
         this.currentPointer = _loc3_.targetCategory;
         this._timesView.menuSelected(this.currentPointer);
         this._contentViews[this.currentPointer].frame = _loc3_.targetPage;
         this.tryRemoveAllViews();
         this._timesView.addChild(this._contentViews[this.currentPointer]);
         this.tryShowTipDisplay(this.currentPointer,_loc3_.targetPage);
         this.tryShowEgg();
         this.updateGuildViewPoint(this.currentPointer,_loc3_.targetPage);
      ...
      private function __pushTipItems(param1:TimesEvent) : void
      {
         ...
         var _loc3_:TimesPicInfo = param1.info as TimesPicInfo;
         var _loc4_:MovieClip = param1.params[0] as MovieClip;
         var _loc5_:String = String(_loc3_.category) + String(_loc3_.page);
         if(Boolean(this._tipItemMcs[_loc5_]))
         {
            _loc2_ = this._tipItemMcs[_loc5_];
         }
         else
         {
            _loc2_ = [];
         ...
         _loc2_.push(_loc4_);
         this._tipItemMcs[_loc5_] = _loc2_;
      }
      
      public function updateGuildViewPoint(param1:int = -1, param2:int = -1) : void
      {
         var _loc3_:int = 0;
         var _loc4_:TimesPicInfo = null;
         var _loc5_:int = 0;
         if(param1 == -1 && param2 == -1)
         {
            this._thumbnailView.pointIdx = _loc5_;
            return;
         }
         var _loc6_:int = 0;
         while(_loc6_ < this._model.contentInfos.length)
         {
            _loc3_ = 0;
            while(_loc3_ < this._model.contentInfos[_loc6_].length)
            {
               _loc4_ = this._model.contentInfos[_loc6_][_loc3_];
               if(param1 == _loc4_.category && param2 == _loc4_.page)
               {
                  this._thumbnailView.pointIdx = _loc5_;
                  return;
               }
      ...
      private function tryRemoveAllViews() : void
      {
         ...
         {
            if(Boolean(this._contentViews[_loc1_]) && this._timesView.contains(this._contentViews[_loc1_]))
            {
      ...
      private function __stopEffect(param1:MouseEvent) : void
      {
         ...
         var _loc2_:int = int(this._extraDisplays[this._tipCurrentKey].getChildIndex(param1.currentTarget));
      ...
      public function tryShowTipDisplay(param1:int, param2:int) : void
      {
         ...
         if(Boolean(this._extraDisplays) && Boolean(this._extraDisplays[this._tipCurrentKey]))
         {
            this._timesView.addChild(this._extraDisplays[this._tipCurrentKey]);
            if(Boolean(this._egg) && this._timesView.contains(this._egg))
            {
      ...
      private function removeTipDisplay() : void
      {
         ...
         {
            if(Boolean(this._extraDisplays[_loc1_].parent))
            {
      ...
      private function deleteTipDisplay() : void
      {
         ...
         for(_loc1_ in this._extraDisplays)
         {
            _loc2_ = int(this._extraDisplays[_loc1_]);
            _loc3_ = 0;
            while(_loc3_ < _loc2_)
            {
      ...
      public function tryShowEgg() : void
      {
         ...
         {
            if(Boolean(_egg) && Boolean(_egg.parent))
            {
      ...
      public function dispose() : void
      {
         ...
         this._model.dispose();
         var _loc4_:int = 0;
         while(_loc4_ < this._thumbnailLoaders.length)
         {
            _loc3_ = 0;
            while(_loc3_ < this._thumbnailLoaders[_loc4_].length)
            {
               ObjectUtils.disposeObject(this._thumbnailLoaders[_loc4_][_loc3_]);
               this._thumbnailLoaders[_loc4_][_loc3_] = null;
               _loc3_++;
            }
            _loc4_++;
         }
         this._thumbnailLoaders = null;
         for each(_loc1_ in this._extraDisplays)
         {
            if(Boolean(_loc1_))
            {
         ...
         this._extraDisplays = null;
         _loc2_ = 0;
         while(_loc2_ < this._tipItemMcs.length)
         {
            if(Boolean(this._tipItemMcs[_loc2_]))
            {
               if(Boolean(this._tipItemMcs[_loc2_].parent))
               {
                  this._tipItemMcs[_loc2_].parent.removeChild(this._tipItemMcs[_loc2_]);
               }
               MovieClip(this._tipItemMcs[_loc2_]).loaderInfo.loader.unloadAndStop();
               this._tipItemMcs[_loc2_] = null;
            }
            _loc2_++;
         }
         if(this._statisticsInstance)
