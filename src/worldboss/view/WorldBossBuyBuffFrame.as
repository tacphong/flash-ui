   ...
   public class WorldBossBuyBuffFrame extends Sprite implements Disposeable
   {
      ...
      private function setList() : void
      {
         ...
         while(_loc3_ < _loc2_.length)
         {
            ...
            this._cartList.addChild(_loc1_);
            this._selectedArr.push(_loc1_);
            _loc1_.selected(_autoBuyBuffItem.hasKey(_loc1_.buffID));
            _loc1_.addEventListener(Event.SELECT,this.__itemSelected);
            _loc3_++;
         }
      ...
      {
         if(Boolean(param1.changedProperties[PlayerInfo.MONEY]))
         {
            this._money.text = PlayerManager.Instance.Self.Money.toString();
         }
      }
      ...
      private function updatePrice() : void
      {
         var _loc1_:BuffCartItem = null;
         var _loc2_:int = 0;
         for each(_loc1_ in this._selectedArr)
         {
      ...
      private function __getBuff(param1:CrazyTankSocketEvent) : void
      {
         ...
         var _loc3_:int = 0;
         var _loc4_:int = 0;
         var _loc5_:BuffCartItem = null;
         var _loc6_:PackageIn = param1.pkg;
         if(_loc6_.readBoolean())
         {
            ...
            while(_loc3_ < _loc2_)
            {
               ...
               for each(_loc5_ in this._selectedArr)
               {
                  if(_loc4_ == _loc5_.buffID)
                  {
                     WorldBossManager.Instance.bossInfo.myPlayerVO.buffID = _loc4_;
                     _loc5_.changeStatusBuy();
                  }
               }
               _loc3_++;
            }
