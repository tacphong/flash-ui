package fightLib.view
{
   ...
   public class FightLibAwardView extends Component implements Disposeable
   {
      ...
      override public function dispose() : void
      {
         ...
         if(Boolean(this._expShape))
         {
            ObjectUtils.disposeObject(this._expShape);
            this._expShape = null;
         }
         if(Boolean(this._giftShape))
         {
      ...
      private function drawBackground() : void
      {
         ...
         _loc1_.drawRect(0,0,_width <= 0 ? Number(1) : Number(_width),_height <= 0 ? Number(1) : Number(_height));
         _loc1_.endFill();
      }
      ...
      private function updateList() : void
      {
         ...
         var _loc3_:AwardCell = this._cells.shift();
         while(_loc3_ != null)
         {
            ObjectUtils.disposeObject(_loc3_);
            _loc3_ = this._cells.shift();
         }
         var _loc4_:Point = ComponentFactory.Instance.creatCustomObject("fightLib.Award.AwardList.cell.PicPos");
         var _loc5_:Point = ComponentFactory.Instance.creatCustomObject("fightLib.Award.AwardList.cell.ContentSize");
         for each(_loc1_ in this._items)
         {
            _loc2_ = ItemManager.Instance.getTemplateById(_loc1_.id);
            _loc3_ = ComponentFactory.Instance.creatCustomObject("fightLib.Award.AwardList.cell");
            _loc3_.info = _loc2_;
