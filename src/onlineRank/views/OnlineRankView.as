package onlineRank.views
{
   ...
   public class OnlineRankView extends Sprite implements IRightView
   {
      ...
      public function OnlineRankView()
      {
         super();
      }
      ...
      {
         OnlineRankManager.instance.view = this;
         this.initView();
         this.updateView();
         this.initTimer();
      }
      ...
      {
         this._bg = ComponentFactory.Instance.creat("wonderfulactivity.onlineRank.bg");
         addChild(this._bg);
         ...
         this._rankLabelTxt.text = LanguageMgr.GetTranslation("onlineRank.rankLabel",5000);
      ...
      public function updateView() : void
      {
         ...
         var _loc1_:* = 0;
         var _loc2_:OnlineRankVo = null;
         var _loc3_:GmActivityInfo = OnlineRankManager.instance.xmlData;
         ...
         this._checkBg.tipData = LanguageMgr.GetTranslation("onlineRank.helpTxt",_loc3_.remain2);
         _loc6_ = OnlineRankManager.instance.rankList;
         _loc1_ = OnlineRankManager.instance.myConsume;
         this._myRank = -1;
         var _loc7_:int = 0;
         while(_loc7_ <= _loc6_.length - 1)
         {
            _loc2_ = _loc6_[_loc7_] as OnlineRankVo;
         ...
         if(this._myRank >= 0)
         {
            ...
            else
            {
               ...
               _loc8_ = (_loc6_[this._myRank - 1] as OnlineRankVo).consume - _loc1_ + 1;
               this._rankLabelTxt.text = LanguageMgr.GetTranslation("onlineRank.rankLabel",_loc8_);
            }
            if(OnlineRankManager.instance.status == 2)
            {
               this._rankLabelTxt.visible = true;
         ...
         else
         {
            ...
            if(_loc6_.length > 0)
            {
               _loc8_ = (_loc6_[_loc6_.length - 1] as OnlineRankVo).consume - _loc1_ + 1;
            }
            else
            {
               _loc8_ = 1;
            }
            this._outOfRankLabel.text = LanguageMgr.GetTranslation("online.outOfRank");
            this._rankLabelTxt.text = LanguageMgr.GetTranslation("onlineRank.outOfRankLabel",_loc1_,_loc8_);
            if(OnlineRankManager.instance.status == 2)
            {
               this._rankLabelTxt.visible = true;
               this._rankLabelTxt.textColor = 16711680;
      ...
      {
         var _loc1_:OnlineRankItem = null;
         var _loc2_:Array = OnlineRankManager.instance.rankList;
         var _loc3_:Array = OnlineRankManager.instance.xmlData.giftbagArray;
         this._vbox.removeAllChild();
         var _loc4_:int = 0;
         while(_loc4_ <= _loc2_.length - 1)
         {
            _loc1_ = new OnlineRankItem(_loc4_);
      ...
      {
         var _loc1_:Date = DateUtils.getDateByStr(OnlineRankManager.instance.xmlData.endTime);
      ...
      {
         OnlineRankManager.instance.view = null;
         WonderfulActivityManager.Instance.delTimerFun("consumeRank");
