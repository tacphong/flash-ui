package effortView.completeIcon
{
   ...
   public class EffortCompleteIconTipsView extends Sprite implements Disposeable, ITip
   {
      ...
      private function update() : void
      {
         ...
         if(Boolean(this._tipData.CompleteStateInfo))
         {
            _loc7_ = this._tipData.CompleteStateInfo.CompletedDate;
         }
         if(Boolean(this._tipData.CompleteStateInfo) && EffortManager.Instance.isSelf)
         {
            this._date.text = String(_loc7_.fullYear) + "/" + String(_loc7_.month + 1) + "/" + String(_loc7_.date);
         }
         else
         {
            this._date.text = "";
         ...
         if(Boolean(this._tipData) && Boolean(this._tipData.effortRewardArray))
         {
            _loc1_ = 0;
            while(_loc1_ < this._tipData.effortRewardArray.length)
            {
               _loc2_ = new ChatData();
               if((this._tipData.effortRewardArray[_loc1_] as EffortRewardInfo).RewardType == 3)
               {
                  _loc2_.htmlMessage += LanguageMgr.GetTranslation("tank.view.effort.EffortRigthItemView.honorNameIV");
                  _loc3_ = int(EffortManager.Instance.splitTitle((this._tipData.effortRewardArray[_loc1_] as EffortRewardInfo).RewardPara));
                  _loc4_ = ItemManager.Instance.getTemplateById(_loc3_);
                  _loc5_ = ChatFormats.creatGoodTag("[" + _loc4_.Name + "]",ChatFormats.CLICK_GOODS,_loc4_.TemplateID,_loc4_.Quality,true,_loc2_);
                  _loc2_.htmlMessage += _loc5_;
                  _loc6_ = "";
                  _loc6_ += Helpers.deCodeString(_loc2_.htmlMessage);
                  this._goodsTxt.htmlText = "<a>" + _loc6_ + "</a>";
      ...
      private function updateComponent() : void
      {
         ...
         {
            this._bg.height = this._honor.text == "" ? Number(TIP_CENTER_HIGHT) : Number(TIP_MAX_HIGHT);
            this._goodsTxt.visible = true;
