package effortView.rightView
{
   ...
   public class EffortFullItemView extends Sprite implements Disposeable
   {
      ...
      private var _effortIcon:effortView.rightView.EffortIconView;
      
      private var _achievementPointView:effortView.rightView.AchievementPointView;
      ...
      private function update() : void
      {
         ...
         {
            _loc2_ = this._info.CompleteStateInfo.CompletedDate;
         }
         if(Boolean(this._info.CompleteStateInfo) && EffortManager.Instance.isSelf)
         {
            this._dateText.text = String(_loc2_.fullYear) + "/" + String(_loc2_.month + 1) + "/" + String(_loc2_.date);
         }
         else
         {
            this._dateText.text = "";
         ...
         {
            _loc1_ = 0;
            while(_loc1_ < this._info.effortRewardArray.length)
            {
               if((this._info.effortRewardArray[_loc1_] as EffortRewardInfo).RewardType == 1)
               {
