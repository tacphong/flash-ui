package effortView.movieClip
{
   ...
   public class EffortHonorMovieClipView extends EffortMovieClipView
   {
      ...
      override protected function init() : void
      {
         ...
         var _loc2_:int = 0;
         while(_loc2_ < _info.effortRewardArray.length)
         {
            if((_info.effortRewardArray[_loc2_] as EffortRewardInfo).RewardType == 1)
            {
               _movieClipView.honorText.text = EffortManager.Instance.splitTitle((_info.effortRewardArray[_loc2_] as EffortRewardInfo).RewardPara);
            }
            _loc2_++;
         }
         _movieClipView.honorText.mouseEnabled = false;
         _loc1_ = ComponentFactory.Instance.creatCustomObject("effortView.EffortMovieClipView.EffortMovieClipViewPos");
         _movieClipView.x = _loc1_.x;
         _movieClipView.y = _loc1_.y;
