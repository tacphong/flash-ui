package consortion.view.selfConsortia
{
   ...
   public class MemberList extends Sprite implements Disposeable
   {
      ...
      private var _searchMemberFrame:consortion.view.selfConsortia.SearchMemberFrame;
      ...
      public function __updataMemberHandler(param1:ConsortionEvent) : void
      {
         ...
         var _loc3_:ConsortiaPlayerInfo = param1.data as ConsortiaPlayerInfo;
         var _loc4_:int = int(this._list.vectorListModel.elements.length);
         var _loc5_:int = 0;
         while(_loc5_ < _loc4_)
         {
            _loc2_ = this._list.vectorListModel.elements[_loc5_] as ConsortiaPlayerInfo;
      ...
      private function search(param1:String) : Boolean
      {
         ...
         var _loc4_:Array = ConsortionModelControl.Instance.model.memberList.list;
         var _loc5_:Array = [];
         ...
         while(_loc7_ < _loc4_.length)
         {
            ...
            if(_loc2_.NickName.indexOf(_loc3_) != -1)
            {
         ...
         if(_loc3_ == LanguageMgr.GetTranslation("consortion.view.selfConsortia.SearchMemberFrame.warningII"))
         {
            MessageTipManager.getInstance().show(LanguageMgr.GetTranslation("consortion.view.selfConsortia.SearchMemberFrame.default"));
            return false;
         }
         ...
         if(_loc8_ < 6)
         {
            while(_loc8_ < 6)
            {
               this._list.vectorListModel.append(null,_loc8_);
               _loc8_++;
            }
         }
         this._list.list.updateListView();
      ...
      {
         var _loc3_:int = 0;
         var _loc4_:int = ConsortionModelControl.Instance.model.memberList.length;
         if(_loc4_ < 6)
         {
            this._list.vectorListModel.elements.splice(_loc4_,6 - _loc4_ + 1);
         }
         if(param1 == "Init")
         {
            this._list.vectorListModel.elements.sortOn("Grade",2 | 1 | 16);
            _loc3_ = 0;
            while(_loc3_ < this._list.vectorListModel.elements.length)
            {
               if(Boolean(this._list.vectorListModel.elements[_loc3_]) && this._list.vectorListModel.elements[_loc3_].ID == PlayerManager.Instance.Self.ID)
               {
                  this._list.vectorListModel.elements.unshift(this._list.vectorListModel.elements[_loc3_]);
                  this._list.vectorListModel.elements.splice(_loc3_ + 1,1);
               }
               _loc3_++;
            }
         }
         ...
         if(_loc4_ < 6)
         {
            while(_loc4_ < 6)
            {
               this._list.vectorListModel.append(null,_loc4_);
               _loc4_++;
            }
         }
         this._list.list.updateListView();
