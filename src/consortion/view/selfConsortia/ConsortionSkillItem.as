package consortion.view.selfConsortia
{
   ...
   public class ConsortionSkillItem extends Sprite implements Disposeable
   {
      ...
      private var _cells:Vector.<consortion.view.selfConsortia.ConsortionSkillCell>;
      
      private var _btns:Vector.<consortion.view.selfConsortia.ConsortionSkillItenBtn>;
      ...
      {
         var _loc2_:consortion.view.selfConsortia.ConsortionSkillCell = null;
         var _loc3_:consortion.view.selfConsortia.ConsortionSkillItenBtn = null;
         var _loc4_:int = 0;
         while(_loc4_ < param1.length)
         {
            _loc2_ = new consortion.view.selfConsortia.ConsortionSkillCell();
            _loc2_.tipData = param1[_loc4_];
            _loc2_.contentRect(54,54);
            addChild(_loc2_);
            PositionUtils.setPos(_loc2_,"consortion.killItem.cellPos" + _loc4_);
            _loc3_ = new consortion.view.selfConsortia.ConsortionSkillItenBtn();
            addChild(_loc3_);
            PositionUtils.setPos(_loc3_,"consortion.killItem.btnPos" + _loc4_);
            if(param1[_loc4_].type == 1)
            {
               _loc3_.setValue(LanguageMgr.GetTranslation("ddt.consortion.skillItem.oneDay"),param1[_loc4_].riches + LanguageMgr.GetTranslation("consortia.Money"));
            }
            else if(this._isMetal)
            {
               _loc3_.setValue(LanguageMgr.GetTranslation("ddt.consortion.skillItem.oneDay"),param1[_loc4_].metal + LanguageMgr.GetTranslation("medal"));
            }
            else
            {
               _loc3_.setValue(LanguageMgr.GetTranslation("ddt.consortion.skillItem.oneDay"),param1[_loc4_].riches + LanguageMgr.GetTranslation("ddt.consortion.skillCell.btnPersonal.rich"));
            }
            _loc2_.addEventListener(MouseEvent.CLICK,this.__clickHandler);
            _loc3_.addEventListener(MouseEvent.CLICK,this.__clickHandler);
            this._cells[_loc4_] = _loc2_;
            this._btns[_loc4_] = _loc3_;
            _loc4_++;
         }
      }
