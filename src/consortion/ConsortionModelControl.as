package consortion
{
   ...
   public class ConsortionModelControl extends EventDispatcher
   {
      ...
      private static var _instance:consortion.ConsortionModelControl;
       
      
      private var _model:consortion.ConsortionModel;
      ...
      {
         super();
         this._model = new consortion.ConsortionModel();
         this._taskModel = new ConsortiaTaskModel();
         this.addEvent();
      }
      
      public static function get Instance() : consortion.ConsortionModelControl
      {
         if(_instance == null)
         {
            _instance = new consortion.ConsortionModelControl();
         }
         return _instance;
      }
      
      public function get model() : consortion.ConsortionModel
      {
         return this._model;
      }
      ...
      private function __consortiaResponse(param1:CrazyTankSocketEvent) : void
      {
         ...
         switch(_loc44_)
         {
            case 1:
               ...
               _loc5_.LastDate = _loc2_.readDateString();
               _loc5_.Grade = _loc2_.readInt();
               _loc5_.DutyLevel = _loc2_.readInt();
               _loc9_ = _loc2_.readInt();
               _loc5_.playerState = new PlayerState(_loc9_);
               _loc5_.Sex = _loc2_.readBoolean();
               _loc5_.Right = _loc2_.readInt();
               ...
               _loc5_.LoginName = _loc2_.readUTF();
               _loc5_.FightPower = _loc2_.readInt();
               _loc5_.AchievementPoint = _loc2_.readInt();
               _loc5_.honor = _loc2_.readUTF();
               _loc5_.UseOffer = _loc2_.readInt();
               if(!(_loc6_ && _loc5_.ID == PlayerManager.Instance.Self.ID))
               {
                  if(_loc5_.ID == PlayerManager.Instance.Self.ID)
                  {
                     MessageTipManager.getInstance().show(LanguageMgr.GetTranslation("tank.manager.PlayerManager.one",_loc5_.ConsortiaName));
                  }
               }
               _loc10_ = "";
               if(_loc5_.ID == PlayerManager.Instance.Self.ID)
               {
                  this.setPlayerConsortia(_loc5_.ConsortiaID,_loc5_.ConsortiaName);
                  this.getConsortionMember(this.memberListComplete);
                  this.getConsortionList(this.selfConsortionComplete,1,6,_loc5_.ConsortiaName,-1,-1,-1,_loc5_.ConsortiaID);
                  if(_loc6_)
                  {
                     _loc10_ = LanguageMgr.GetTranslation("tank.manager.PlayerManager.isInvent.msg",_loc5_.ConsortiaName);
                  }
                  else
                  {
                     _loc10_ = LanguageMgr.GetTranslation("tank.manager.PlayerManager.pass",_loc5_.ConsortiaName);
                  }
                  ...
                  {
                     ExternalInterfaceManager.sendToAgent(5,PlayerManager.Instance.Self.ID,PlayerManager.Instance.Self.NickName,ServerManager.Instance.zoneName,-1,_loc5_.ConsortiaName);
                  }
               }
               else
               {
                  this._model.addMember(_loc5_);
                  _loc10_ = LanguageMgr.GetTranslation("tank.manager.PlayerManager.player",_loc5_.NickName);
               }
               _loc10_ = StringHelper.rePlaceHtmlTextField(_loc10_);
            ...
            case 3:
               ...
               if(PlayerManager.Instance.Self.ID == _loc3_)
               {
                  ...
                  if(_loc12_)
                  {
                     _loc38_ = LanguageMgr.GetTranslation("tank.manager.PlayerManager.delect",_loc13_);
                     MessageTipManager.getInstance().show(LanguageMgr.GetTranslation("tank.manager.PlayerManager.hit"));
                  }
                  else
                  {
                     _loc38_ = LanguageMgr.GetTranslation("tank.manager.PlayerManager.leave");
                  }
                  if(StateManager.currentStateType == StateType.CONSORTIA)
                  {
                     StateManager.back();
                  }
                  _loc38_ = StringHelper.rePlaceHtmlTextField(_loc38_);
                  ChatManager.Instance.sysChatRed(_loc38_);
               }
               else
               {
                  this.removeConsortiaMember(_loc3_,_loc12_,_loc13_);
               }
               break;
            case 4:
               this.quitConstrion = false;
               ...
               if(SharedManager.Instance.showCI)
               {
                  if(this.str != _loc17_)
                  {
                     ...
                     _loc39_ = _loc17_ + LanguageMgr.GetTranslation("tank.manager.PlayerManager.come",_loc19_);
                     ...
                     this._enterConfirm = AlertManager.Instance.simpleAlert(LanguageMgr.GetTranslation("tank.manager.PlayerManager.request"),_loc39_,LanguageMgr.GetTranslation("tank.manager.PlayerManager.sure"),LanguageMgr.GetTranslation("tank.manager.PlayerManager.refuse"),false,true,true,LayerManager.ALPHA_BLOCKGOUND,CacheConsts.ALERT_IN_FIGHT);
                     this._enterConfirm.addEventListener(FrameEvent.RESPONSE,this.__enterConsortiaConfirm);
                     this.str = _loc17_;
                     if(_loc40_ is TextField)
                     {
                        if(TextField(_loc40_).type == TextFieldType.INPUT)
                        {
            ...
            case 6:
               ...
               if(PlayerManager.Instance.Self.ConsortiaID == _loc20_)
               {
                  PlayerManager.Instance.Self.consortiaInfo.Level = _loc22_;
                  ChatManager.Instance.sysChatYellow(LanguageMgr.GetTranslation("tank.manager.PlayerManager.upgrade",_loc22_,this._model.getLevelData(_loc22_).Count));
                  TaskManager.requestClubTask();
            ...
            case 8:
               ...
               if(_loc23_ != 1)
               {
                  if(_loc23_ == 2)
                  {
                     this.updateDutyInfo(_loc27_,_loc28_,_loc29_);
                  }
                  else if(_loc23_ == 3)
                  {
                     this.upDateSelfDutyInfo(_loc27_,_loc28_,_loc29_);
                  }
                  else if(_loc23_ == 4)
                  {
                     this.upDateSelfDutyInfo(_loc27_,_loc28_,_loc29_);
                  ...
                  {
                     this.updateConsortiaMemberDuty(_loc25_,_loc27_,_loc28_,_loc29_);
                     _loc41_ = "";
                     if(_loc25_ == PlayerManager.Instance.Self.ID)
                     {
                        _loc41_ = LanguageMgr.GetTranslation("tank.manager.PlayerManager.youUpgrade",_loc31_,_loc28_);
                     }
                     else if(_loc25_ == _loc30_)
                     {
                        _loc41_ = LanguageMgr.GetTranslation("tank.manager.PlayerManager.upgradeSelf",_loc26_,_loc28_);
                     }
                     else
                     {
                        _loc41_ = LanguageMgr.GetTranslation("tank.manager.PlayerManager.upgradeOther",_loc31_,_loc26_,_loc28_);
                     }
                     _loc41_ = StringHelper.rePlaceHtmlTextField(_loc41_);
                     ChatManager.Instance.sysChatYellow(_loc41_);
                  }
                  else if(_loc23_ == 7)
                  {
                     this.updateConsortiaMemberDuty(_loc25_,_loc27_,_loc28_,_loc29_);
                     ...
                     if(_loc25_ == PlayerManager.Instance.Self.ID)
                     {
                        _loc42_ = LanguageMgr.GetTranslation("tank.manager.PlayerManager.youDemotion",_loc31_,_loc28_);
                     }
                     else if(_loc25_ == _loc30_)
                     {
                        _loc42_ = LanguageMgr.GetTranslation("tank.manager.PlayerManager.demotionSelf",_loc26_,_loc28_);
                     }
                     else
                     {
                        _loc42_ = LanguageMgr.GetTranslation("tank.manager.PlayerManager.demotionOther",_loc31_,_loc26_,_loc28_);
                     }
                  ...
                  {
                     this.updateConsortiaMemberDuty(_loc25_,_loc27_,_loc28_,_loc29_);
                     PlayerManager.Instance.Self.consortiaInfo.ChairmanName = _loc26_;
                     _loc43_ = "<" + _loc26_ + ">" + LanguageMgr.GetTranslation("tank.manager.PlayerManager.up") + _loc28_;
            ...
            case 9:
               ...
               _loc35_ = _loc2_.readInt();
               if(_loc32_ != PlayerManager.Instance.Self.ConsortiaID)
               {
                  return;
               }
               _loc36_ = "";
               if(PlayerManager.Instance.Self.ID == _loc33_)
               {
                  _loc36_ = LanguageMgr.GetTranslation("tank.manager.PlayerManager.contributionSelf",_loc35_);
               }
               else
               {
                  _loc36_ = LanguageMgr.GetTranslation("tank.manager.PlayerManager.contributionOther",_loc34_,_loc35_);
               }
               ChatManager.Instance.sysChatYellow(_loc36_);
               break;
            case 10:
         ...
         for each(_loc5_ in this._model.memberList)
         {
            if(_loc5_.ID == param1)
            {
               ...
               if(_loc5_.ID == PlayerManager.Instance.Self.ID)
               {
                  ...
                  PlayerManager.Instance.Self.consortiaInfo.Level = PlayerManager.Instance.Self.consortiaInfo.Level == 0 ? int(1) : int(PlayerManager.Instance.Self.consortiaInfo.Level);
                  PlayerManager.Instance.Self.commitChanges();
      ...
      private function __skillChangehandler(param1:CrazyTankSocketEvent) : void
      {
         ...
         while(_loc7_ < _loc6_)
         {
            ...
            this._model.updateSkillInfo(_loc2_,_loc3_,_loc4_,_loc5_);
            _loc7_++;
         }
         if(_loc6_ > 0)
         {
      ...
      private function __buyBadgeHandler(param1:CrazyTankSocketEvent) : void
      {
         ...
         var _loc7_:Date = _loc3_.readDate();
         var _loc8_:Boolean = _loc3_.readBoolean();
         if(_loc4_ == PlayerManager.Instance.Self.ConsortiaID)
         {
            PlayerManager.Instance.Self.consortiaInfo.BadgeBuyTime = DateUtils.dateFormat(_loc7_);
            PlayerManager.Instance.Self.consortiaInfo.BadgeID = _loc5_;
            PlayerManager.Instance.Self.consortiaInfo.ValidDate = _loc6_;
            PlayerManager.Instance.Self.badgeID = _loc5_;
            _loc2_ = BadgeInfoManager.instance.getBadgeInfoByID(_loc5_);
            PlayerManager.Instance.Self.consortiaInfo.Riches -= _loc2_.Cost;
         }
      }
