   ...
   public class StringHelper
   {
      ...
      public static function stringToPath(param1:String) : Array
      {
         ...
         while(_loc5_ < param1.length)
         {
            ...
            _loc4_.push(new Point(_loc2_ - OFFSET,_loc3_ - OFFSET));
            _loc5_ += 2;
         }
         return _loc4_;
      }
      ...
      {
         var _loc4_:uint = Boolean(param1.text) ? uint(param1.text.match(reg).join("").length) : uint(0);
         var _loc5_:uint = Boolean(param3) ? uint(param3.match(reg).join("").length) : uint(0);
         param1.maxChars = param2 > _loc4_ + _loc5_ ? int(param2 - _loc4_ - _loc5_) : (param2 > _loc5_ ? int(param2 - _loc5_) : int(param2 / 2));
      }
      ...
      public static function trimHtmlText(param1:String) : String
      {
         ...
         {
            _loc4_ = int(_loc6_.index);
            _loc7_.push(param1.substring(_loc3_,_loc4_));
            _loc7_.push(_loc6_[0]);
            _loc3_ = _loc5_.lastIndex;
            _loc6_ = _loc5_.exec(param1);
         }
         _loc7_.push(param1.substring(_loc3_));
