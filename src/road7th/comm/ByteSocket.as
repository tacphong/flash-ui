package road7th.comm
{
   ...
   public class ByteSocket extends EventDispatcher
   {
      ...
      private function handleIncoming(param1:ProgressEvent) : void
      {
         ...
         {
            _loc2_ = int(this._socket.bytesAvailable);
      ...
      private function readPackage() : void
      {
         ...
         var _loc3_:int = this._writeOffset - this._readOffset;
         while(true)
         {
            _loc1_ = 0;
            while(this._readOffset + 4 < this._writeOffset)
            {
               ...
               {
                  _loc1_ = int(this._headerTemp.readUnsignedShort());
                  break;
               }
               ++this._readOffset;
            }
            _loc3_ = this._writeOffset - this._readOffset;
            if(!(_loc3_ >= _loc1_ && _loc1_ != 0))
            {
               break;
            }
            ...
            if(this._encrypted)
            {
               _loc2_.loadE(this._readBuffer,_loc1_,RECEIVE_KEY);
            }
            else
            {
               _loc2_.load(this._readBuffer,_loc1_);
            }
            this._readOffset += _loc1_;
            _loc3_ = this._writeOffset - this._readOffset;
            this.handlePackage(_loc2_);
            if(_loc3_ < PackageIn.HEADER_SIZE)
            {
               break;
            }
         }
         ...
         if(_loc3_ > 0)
         {
            this._readBuffer.writeBytes(this._readBuffer,this._readOffset,_loc3_);
         }
         this._readOffset = 0;
         this._writeOffset = _loc3_;
      }
      ...
      private function tracePkg(param1:ByteArray, param2:String, param3:int = -1) : void
      {
         ...
         var _loc5_:int = param3 < 0 ? int(param1.length) : int(param3);
         var _loc6_:int = 0;
         while(_loc6_ < _loc5_)
         {
...
class FSM
{
   ...
   public function FSM(param1:int, param2:int)
   {
      super();
