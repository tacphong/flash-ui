package equipDebt.view
{
   ...
   public class EquipAddMoneyItem extends Sprite implements Disposeable
   {
      ...
      private var _cell:equipDebt.view.EquipCell;
      ...
      public function set info(param1:InventoryItemInfo) : void
      {
         ...
         while(_loc4_ < 3)
         {
            _loc3_ = ComponentFactory.Instance.creatComponentByStylename("shop.CartItemSelectBtn");
            _loc3_.text = ShopItemInfo(_loc2_[0]).getItemPrice(_loc4_ + 1).moneyValue.toString() + LanguageMgr.GetTranslation("shop.ShopIIShoppingCarItem.stipple") + ShopItemInfo(_loc2_[0]).getTimeToString(_loc4_ + 1);
            this._cartItemSelectVBox.addChild(_loc3_);
            this._cartItemGroup.addSelectItem(_loc3_);
            _loc4_++;
         }
