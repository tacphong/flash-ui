package game.view
{
   ...
   public class CheckCodeMixedBack extends Sprite
   {
      ...
      private function creatCurver() : void
      {
         ...
         var _loc3_:int = 0;
         while(_loc3_ < CuverNum)
         {
            _loc1_ = Math.random() * this._width;
            _loc2_ = Math.random() * this._height;
            this._renderBox.graphics.moveTo(_loc1_ + (Math.random() * CUVER_MAX - CUVER_MAX),_loc2_ + (Math.random() * CUVER_MAX - CUVER_MAX));
            this._renderBox.graphics.curveTo(_loc1_ + (Math.random() * CUVER_MAX - CUVER_MAX),_loc2_ + (Math.random() * CUVER_MAX - CUVER_MAX),_loc1_,_loc2_);
            _loc3_++;
         }
         this._renderBox.graphics.endFill();
      }
