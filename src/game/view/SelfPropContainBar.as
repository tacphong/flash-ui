package game.view
{
   ...
   public class SelfPropContainBar extends BaseGamePropBarView
   {
      ...
      public function SelfPropContainBar(param1:LocalPlayer)
      {
         ...
         addChild(this._back);
         _loc2_ = ComponentFactory.Instance.creatCustomObject("asset.game.itemContainerPos");
      ...
      private function initData() : void
      {
         ...
         var _loc3_:BagInfo = this._info.FightBag;
         for each(_loc1_ in _loc3_.items)
         {
            _loc2_ = new PropInfo(_loc1_);
            _loc2_.Place = _loc1_.Place;
            this.addProp(_loc2_);
         }
      }
      ...
      private function __updateProp(param1:BagEvent) : void
      {
         ...
         {
            _loc3_ = this._info.FightBag.getItemAt(_loc2_.Place);
            if(Boolean(_loc3_))
            {
               _loc4_ = new PropInfo(_loc3_);
               _loc4_.Place = _loc3_.Place;
               this.addProp(_loc4_);
            }
            else
            {
               _loc5_ = new PropInfo(_loc2_);
               _loc5_.Place = _loc2_.Place;
               this.removeProp(_loc5_);
            }
         }
      }
