package game.view.card
{
   ...
   public class LargeCardsView extends SmallCardsView
   {
      ...
      private function __showAllCard(param1:CrazyTankSocketEvent) : void
      {
         ...
         {
            _loc2_ = new Object();
            _loc2_.index = _loc3_.readByte();
            _loc2_.templateID = _loc3_.readInt();
            _loc2_.count = _loc3_.readInt();
            this._showCardInfos.push(_loc2_);
            _loc5_++;
         }
         this.showAllCard();
      }
      ...
      override protected function createCards() : void
      {
         ...
         {
            _loc2_ = new Point();
            _loc3_ = new LuckyCard(_loc4_,LuckyCard.AFTER_GAME_CARD);
            _loc2_.x = _loc4_ % _cardColumns * (_loc1_.x + _loc3_.width);
            _loc2_.y = int(_loc4_ / _cardColumns) * (_loc1_.y + _loc3_.height);
            _loc3_.x = (_loc1_.x + _loc3_.width) * 3;
            _loc3_.y = _loc1_.y + _loc3_.height;
            _loc3_.allowClick = _gameInfo.selfGamePlayer.GetCardCount > 0;
