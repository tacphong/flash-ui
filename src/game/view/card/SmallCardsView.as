package game.view.card
{
   ...
   public class SmallCardsView extends Sprite implements Disposeable
   {
      ...
      protected var _cards:Vector.<game.view.card.LuckyCard>;
      ...
      protected var _countDownView:game.view.card.CardCountDown;
      ...
      protected function init() : void
      {
         ...
         this._countDownView = new game.view.card.CardCountDown();
      ...
      protected function createCards() : void
      {
         ...
         _loc2_ = null;
         var _loc3_:game.view.card.LuckyCard = null;
         _loc1_ = new Point(26,25);
         var _loc4_:int = 0;
         while(_loc4_ < this._cardCnt)
         {
            _loc2_ = new Point();
            ...
            {
               _loc3_ = new game.view.card.LuckyCard(_loc4_,LuckyCard.WITHIN_GAME_CARD);
               _loc3_.allowClick = this._canTakeOut = this._gameInfo.selfGamePlayer.BossCardCount > 0;
            }
            else if(this._roomInfo.type == RoomInfo.FIGHT_LIB_ROOM)
            {
               _loc3_ = new game.view.card.LuckyCard(_loc4_,LuckyCard.AFTER_GAME_CARD);
               _loc3_.allowClick = this._canTakeOut = true;
            }
            else
            {
               _loc3_ = new game.view.card.LuckyCard(_loc4_,LuckyCard.AFTER_GAME_CARD);
               _loc3_.allowClick = this._canTakeOut = this._gameInfo.selfGamePlayer.GetCardCount > 0;
            }
            _loc2_.x = _loc4_ % this._cardColumns * (_loc1_.x + _loc3_.width) + 87;
            _loc2_.y = int(_loc4_ / this._cardColumns) * (_loc1_.y + _loc3_.height) + 32;
            _loc3_.x = _loc1_.x + _loc3_.width + 87;
            _loc3_.y = _loc1_.y + _loc3_.height + 32;
            _loc3_.msg = LanguageMgr.GetTranslation("tank.gameover.DisableGetCard");
            addChild(_loc3_);
            this._posArr.push(_loc2_);
            this._cards.push(_loc3_);
            _loc4_++;
         }
      }
      ...
      protected function cardTweenComplete(param1:game.view.card.LuckyCard) : void
      {
      ...
      {
         var _loc1_:game.view.card.LuckyCard = null;
         this.removeEvents();
