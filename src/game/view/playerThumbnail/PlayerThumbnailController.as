package game.view.playerThumbnail
{
   ...
   public class PlayerThumbnailController extends Sprite
   {
      ...
      private var _list1:game.view.playerThumbnail.PlayerThumbnailList;
      
      private var _list2:game.view.playerThumbnail.PlayerThumbnailList;
      
      private var _bossThumbnailContainer:game.view.playerThumbnail.BossThumbnail;
      ...
      {
         this.initInfo();
         this._list1 = new game.view.playerThumbnail.PlayerThumbnailList(this._team1,-1);
         this._list2 = new game.view.playerThumbnail.PlayerThumbnailList(this._team2);
      ...
      private function initInfo() : void
      {
         ...
         var _loc2_:DictionaryData = this._info.livings;
         for each(_loc1_ in _loc2_)
         {
            if(_loc1_ is Player)
            {
               if(_loc1_.team == 1)
               {
                  this._team1.add((_loc1_ as Player).playerInfo.ID,_loc1_);
               }
               else if(this._info.gameMode != 5)
               {
                  this._team2.add((_loc1_ as Player).playerInfo.ID,_loc1_);
               }
            }
         }
      }
      ...
      public function set currentBoss(param1:Living) : void
      {
         ...
         this._bossThumbnailContainer = new game.view.playerThumbnail.BossThumbnail(param1);
      ...
      private function __removePlayer(param1:DictionaryEvent) : void
      {
         ...
         if(Boolean(_loc2_.character))
         {
            _loc2_.character.resetShowBitmapBig();
         }
         if(Boolean(this._bossThumbnailContainer) && this._bossThumbnailContainer.Id == _loc2_.LivingID)
         {
            this._bossThumbnailContainer.dispose();
            this._bossThumbnailContainer = null;
         }
