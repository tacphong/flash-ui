package game.view.playerThumbnail
{
   ...
   public class PlayerThumbnailList extends Sprite implements Disposeable
   {
      ...
      private function __onTipClick(param1:Event) : void
      {
         ...
         {
            if(Boolean((param1.currentTarget as PlayerThumbnailTip).tipDisplay))
            {
      ...
      private function arrange() : void
      {
         ...
         while(_loc2_ < numChildren)
         {
            ...
            if(this._dirct < 0)
            {
               _loc1_.x = (_loc2_ + 1) * (_loc1_.width + 4) * this._dirct;
            }
            else
            {
               _loc1_.x = _loc2_ * (_loc1_.width + 4) * this._dirct;
            }
            _loc2_++;
         }
      }
      ...
      public function __removePlayer(param1:DictionaryEvent) : void
      {
         ...
         if(Boolean(_loc2_) && Boolean(_loc2_.playerInfo))
         {
            if(Boolean(this._players[_loc2_.playerInfo.ID]))
            {
      ...
      public function dispose() : void
      {
         ...
         {
            if(Boolean(this._players[_loc1_]))
            {
