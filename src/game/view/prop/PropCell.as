package game.view.prop
{
   ...
   public class PropCell extends Sprite implements Disposeable, ITipedDisplay, IDragable, IAcceptDrag
   {
      ...
      public function dragStop(param1:DragEffect) : void
      {
         ...
         var _loc5_:int = int(SharedManager.Instance.GameKeySets[_loc2_.shortcutKey]);
      ...
      public function set info(param1:PropInfo) : void
      {
         ...
         this._info = param1;
         var _loc3_:DisplayObject = this._asset;
         if(this._info != null)
         {
            _loc2_ = _loc2_ = ComponentFactory.Instance.creatBitmap("game.crazyTank.view.Prop" + this._info.Template.Pic + "Asset");
            if(Boolean(_loc2_))
            {
               _loc2_.smoothing = true;
               _loc2_.x = _loc2_.y = 1;
               _loc2_.width = _loc2_.height = 35;
               addChildAt(_loc2_,getChildIndex(this._fore));
            }
            this._asset = _loc2_;
