package game.view.prop
{
   ...
   public class SoulPropBar extends FightPropBar
   {
      ...
      override protected function drawCells() : void
      {
         var _loc1_:int = 0;
         var _loc2_:int = 0;
         var _loc3_:SoulPropCell = null;
         var _loc4_:int = 0;
         var _loc5_:Point = new Point(4,4);
         while(_loc4_ < 20)
         {
            _loc3_ = new SoulPropCell();
            _loc3_.addEventListener(MouseEvent.CLICK,this.__itemClicked);
            _loc1_ = _loc4_ % 10 * (_loc3_.width + 1);
            if(_loc4_ >= 10)
            {
               _loc2_ = _loc3_.height + 2;
            }
            _loc3_.setPossiton(_loc1_ + _loc5_.x,_loc2_ + _loc5_.y);
            addChild(_loc3_);
            _cells.push(_loc3_);
            _loc4_++;
         }
      ...
      {
         var _loc1_:PropInfo = null;
         var _loc2_:int = 0;
         while(_loc2_ < this._propDatas.length)
         {
            _loc1_ = new PropInfo(ItemManager.Instance.getTemplateById(this._propDatas[_loc2_]));
            _loc1_.Place = -1;
            _cells[_loc2_].info = _loc1_;
            _loc2_++;
         }
      }
