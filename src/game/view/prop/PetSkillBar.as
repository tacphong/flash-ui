package game.view.prop
{
   ...
   public class PetSkillBar extends FightPropBar
   {
      ...
      private var _skillCells:Vector.<game.view.prop.PetSkillCell>;
      ...
      {
         var _loc1_:game.view.prop.PetSkillCell = null;
      ...
      {
         var _loc2_:game.view.prop.PetSkillCell = null;
         ...
         var _loc4_:String = String(this.letters[_loc3_]);
         for each(_loc2_ in this._skillCells)
         {
            if(_loc2_.shortcutKey == _loc4_ && _loc2_.skillInfo && _loc2_.skillInfo.isActiveSkill && _loc2_.isEnabled && _loc2_.enabled)
            {
               _loc2_.useProp();
               break;
            }
      ...
      private function skillInfoInit(param1:Event) : void
      {
         var _loc2_:int = 0;
         var _loc3_:int = 0;
         var _loc4_:game.view.prop.PetSkillCell = null;
         var _loc5_:Array = GameManager.Instance.petSkillList;
         if(Boolean(_loc5_))
         {
            ...
            while(_loc3_ < _loc2_)
            {
               ...
               {
                  if(Boolean(_loc4_.skillInfo) && _loc4_.skillInfo.ID == _loc5_[_loc3_].id)
                  {
                     _loc4_.turnNum = _loc4_.skillInfo.ColdDown + 1 - _loc5_[_loc3_].cd;
                     break;
                  }
               }
               _loc3_++;
      ...
      private function __petSkillCD(param1:CrazyTankSocketEvent) : void
      {
         ...
         for each(_loc2_ in this._skillCells)
         {
            if(_loc2_.skillInfo.ID == _loc4_)
            {
               _loc2_.turnNum = _loc2_.skillInfo.ColdDown + 1 - _loc5_;
            }
         }
         this.updateCellEnable();
      }
      ...
      {
         var _loc1_:game.view.prop.PetSkillCell = null;
         var _loc2_:Boolean = _self.petSkillEnabled;
         for each(_loc1_ in this._skillCells)
         {
            if(Boolean(_loc1_.skillInfo))
            {
               switch(_loc1_.skillInfo.BallType)
               {
                  case PetSkillTemplateInfo.BALL_TYPE_0:
                     _loc1_.enabled = _loc2_ && _self.isAttacking && !this._usedPetSkill && !this._usedSpecialSkill && _loc1_.skillInfo.CostMP <= _self.currentPet.MP && _loc1_.turnNum > _loc1_.skillInfo.ColdDown;
                     break;
                  case PetSkillTemplateInfo.BALL_TYPE_1:
                     _loc1_.enabled = _loc2_ && _self.isAttacking && !this._usedPetSkill && !this._usedItem && !this._usedSpecialSkill && _loc1_.skillInfo.CostMP <= _self.currentPet.MP && _loc1_.turnNum > _loc1_.skillInfo.ColdDown;
                     break;
                  case PetSkillTemplateInfo.BALL_TYPE_2:
                     _loc1_.enabled = _loc2_ && _self.isAttacking && !this._usedPetSkill && !this._usedItem && !this._usedSpecialSkill && !_self.iscalcForce && _loc1_.skillInfo.CostMP <= _self.currentPet.MP && _loc1_.turnNum > _loc1_.skillInfo.ColdDown;
                     break;
                  case PetSkillTemplateInfo.BALL_TYPE_3:
                     _loc1_.enabled = _loc2_ && _self.isAttacking && !this._usedPetSkill && !this._usedSpecialSkill && _loc1_.skillInfo.CostMP <= _self.currentPet.MP && _loc1_.turnNum > _loc1_.skillInfo.ColdDown;
               }
            }
            if(PetBagController.instance().petModel.petGuildeOptionOnOff[ArrowType.USE_PET_SKILL] > 0 && _loc1_.enabled)
            {
      ...
      private function onCellClick(param1:MouseEvent) : void
      {
         ...
         var _loc2_:game.view.prop.PetSkillCell = param1.currentTarget as PetSkillCell;
      ...
      override protected function drawCells() : void
      {
         ...
         if(Boolean(_self.currentPet))
         {
            ...
            {
               _loc4_ = new game.view.prop.PetSkillCell(this.letters[_loc3_],FightPropMode.HORIZONTAL,false,33,33);
               _loc5_ = int(_self.currentPet.equipedSkillIDs[_loc3_]);
               if(_loc5_ > 0)
               {
