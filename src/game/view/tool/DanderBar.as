package game.view.tool
{
   ...
   public class DanderBar extends Sprite implements Disposeable, ITipedDisplay
   {
      ...
      private function drawProgress(param1:Number) : void
      {
         ...
         var _loc4_:int = Min - Math.abs(Max - Min) * param1;
         var _loc5_:Graphics = this._maskShape.graphics;
         _loc5_.clear();
         var _loc6_:int = 52;
         _loc5_.beginFill(0,1);
         _loc5_.moveTo(0,0);
         _loc5_.lineTo(0,_loc6_);
         var _loc7_:int = Min;
         while(_loc7_ >= _loc4_)
         {
            _loc2_ = _loc6_ * Math.cos(_loc7_ / 180 * Math.PI);
            _loc3_ = _loc6_ * Math.sin(_loc7_ / 180 * Math.PI);
            _loc5_.lineTo(_loc2_,_loc3_);
            _loc7_--;
         }
         _loc5_.lineTo(0,0);
      }
