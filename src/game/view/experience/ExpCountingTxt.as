package game.view.experience
{
   ...
   public class ExpCountingTxt extends Sprite implements Disposeable
   {
      ...
      protected function updateText() : void
      {
         ...
         var _loc2_:String = String(this._text.text);
         if(this._value < 0)
         {
            _loc1_ = String(Math.round(this._value)) + " ";
         }
         else
         {
            _loc1_ = this._plus + String(Math.round(this._value)) + " ";
         }
         if(Boolean(_loc1_.indexOf("+")) && Boolean(_loc1_.indexOf("-")))
         {
