package game.view.smallMap
{
   ...
   public class SmallMapView extends Sprite implements Disposeable
   {
      ...
      private var _titleBar:game.view.smallMap.SmallMapTitleBar;
      ...
      public function random(param1:int = 0, param2:int = 2147483647) : int
      {
         if(param1 == param2)
         {
            return param1;
      ...
      private function initView() : void
      {
         ...
         this._titleBar = new game.view.smallMap.SmallMapTitleBar(this._missionInfo);
      ...
      private function updateSpliter() : void
      {
         ...
         this._texts = [];
         var _loc2_:Number = this._screen.width / 10;
         this._split.graphics.clear();
         this._split.graphics.lineStyle(1,16777215,1);
         var _loc3_:int = 1;
         while(_loc3_ < 10)
         {
            this._split.graphics.moveTo(_loc2_ * _loc3_,0);
            this._split.graphics.lineTo(_loc2_ * _loc3_,this._screen.height);
            _loc1_ = ClassUtils.CreatInstance(NUMBERS_ARR[_loc3_ - 1]);
            _loc1_.x = _loc2_ * _loc3_;
            _loc1_.y = (this._screen.height - _loc1_.height) / 2;
            _loc1_.stop();
            this._split.addChild(_loc1_);
            this._texts.push(_loc1_);
            _loc3_++;
         }
      ...
      public function get titleBar() : game.view.smallMap.SmallMapTitleBar
      {
...
class ThingProcesser
{
   ...
   public function removeThing(param1:SmallObject) : void
   {
      ...
      var _loc2_:int = int(this._objectList.length);
      var _loc3_:int = 0;
      while(_loc3_ < _loc2_)
      {
   ...
   {
      var _loc2_:SmallObject = null;
      var _loc3_:int = getTimer();
      var _loc4_:int = _loc3_ - this._lastTime;
      var _loc5_:int = getTimer();
      for each(_loc2_ in this._objectList)
      {
         _loc2_.onFrame(_loc4_);
      }
