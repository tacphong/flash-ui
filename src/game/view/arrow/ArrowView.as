package game.view.arrow
{
   ...
   public class ArrowView extends Sprite implements Disposeable
   {
      ...
      private var _bg:game.view.arrow.ArrowBg;
      ...
      public function ArrowView(param1:LocalPlayer)
      {
         ...
         var _loc3_:FilterFrameText = ComponentFactory.Instance.creatComponentByStylename("asset.game.rotationCountFieldText");
         this.rotationCountField = new GradientText(_loc3_,RANDOW_COLORSII);
         _loc2_ = ComponentFactory.Instance.creatCustomObject("asset.game.rotationCountPos");
         this.rotationCountField.x = _loc2_.x;
         this.rotationCountField.y = _loc2_.y;
      ...
      private function __enterFrame(param1:Event) : void
      {
         ...
         if(_loc3_ - this._currentAngleChangeTime < ANGLE_NEXTCHANGE_TIME)
         {
            return;
         }
         ...
         if(KeyboardManager.isDown(KeyStroke.VK_S.getCode()) || KeyboardManager.isDown(Keyboard.DOWN))
         {
            ...
            {
               _loc4_ = this._info.manuallySetGunAngle(this._info.gunAngle - WeaponInfo.ROTATITON_SPEED * this._info.reverse);
            }
            else
            {
