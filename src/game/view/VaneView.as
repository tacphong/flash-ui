package game.view
{
   ...
   public class VaneView extends Sprite
   {
      ...
      private var mixedBg1:game.view.CheckCodeMixedBack;
      ...
      {
         this.mixedBg1 = new game.view.CheckCodeMixedBack(20,20,7238008);
         this.mixedBg1.x = 0;
         this.mixedBg1.y = 0;
      ...
      public function update(param1:Number, param2:Boolean = false, param3:Array = null) : void
      {
         ...
         if(param1 != 0)
         {
            this._bmVaneTitle.x = param1 > 0 ? Number(this._vaneTitlePos2.x) : Number(this._vaneTitlePos.x);
         }
         this.vane1_mc.scaleX = param3[0] == true ? Number(1) : Number(-1);
         this.vane1_mc.x = param3[0] == true ? Number(this._vanePos2.x) : Number(this._vanePos.x);
         this._windNumShape.x = param3[0] == true ? Number(this._vaneValuePos.x) : Number(this._vaneValuePos2.x);
         this._windNumShape.y = param3[0] == true ? Number(this._vaneValuePos.y) : Number(this._vaneValuePos2.y);
      ...
      private function drawNum(param1:Array) : void
      {
         ...
         var _loc3_:int = 0;
         var _loc4_:Graphics = this._windNumShape.graphics;
         _loc4_.clear();
         var _loc5_:Matrix = new Matrix();
         for each(_loc3_ in param1)
         {
            _loc2_ = WindPowerManager.Instance.getWindPicById(_loc3_);
            if(Boolean(_loc2_))
            {
               _loc5_.tx = this._windNumShape.width;
               _loc4_.beginBitmapFill(_loc2_,_loc5_);
               _loc4_.drawRect(this._windNumShape.width,0,_loc2_.width,_loc2_.height);
               _loc4_.endFill();
            }
         }
      }
      ...
      private function lastTurn(param1:Number) : void
      {
         ...
         this._bmPreviousDirection.scaleX = param1 > 0 ? Number(1) : Number(-1);
         this._bmPreviousDirection.x = param1 > 0 ? Number(81) : Number(this._previousDirectionPos.x);
