package game.view.map
{
   ...
   public class MapView extends Map
   {
      ...
      private function updateSky() : void
      {
         ...
         var _loc3_:Number = (sky.height * this._scale - PathInfo.GAME_HEIGHT) / (bound.height * this._scale - PathInfo.GAME_HEIGHT);
         var _loc4_:Number = (sky.width * this._scale - PathInfo.GAME_WIDTH) / (bound.width * this._scale - PathInfo.GAME_WIDTH);
         _loc3_ = isNaN(_loc3_) || _loc3_ == Number.NEGATIVE_INFINITY || _loc3_ == Number.POSITIVE_INFINITY ? Number(1) : Number(_loc3_);
         _loc4_ = isNaN(_loc4_) || _loc4_ == Number.NEGATIVE_INFINITY || _loc4_ == Number.POSITIVE_INFINITY ? Number(1) : Number(_loc4_);
         _sky.y = this.y * (_loc3_ - 1) / this._scale;
         _sky.x = this.x * (_loc4_ - 1) / this._scale;
         if(Boolean(_middle))
         {
            _loc1_ = (_middle.height * this._scale - PathInfo.GAME_HEIGHT) / (bound.height * this._scale - PathInfo.GAME_HEIGHT);
            _loc2_ = (_middle.width * this._scale - PathInfo.GAME_WIDTH) / (bound.width * this._scale - PathInfo.GAME_WIDTH);
            _loc1_ = isNaN(_loc1_) || _loc1_ == Number.NEGATIVE_INFINITY || _loc1_ == Number.POSITIVE_INFINITY ? Number(1) : Number(_loc1_);
            _loc2_ = isNaN(_loc2_) || _loc2_ == Number.NEGATIVE_INFINITY || _loc2_ == Number.POSITIVE_INFINITY ? Number(1) : Number(_loc2_);
            _middle.y = this.y * (_loc1_ - 1) / this._scale;
            _middle.x = this.x * (_loc2_ - 1) / this._scale;
         }
      ...
      public function setExpressionLoction() : void
      {
         ...
         var _loc4_:int = 0;
         while(_loc4_ < this._gamePlayerList.length)
         {
            _loc1_ = this._gamePlayerList[_loc4_];
            if(_loc1_ == null || !_loc1_.isLiving || _loc1_.facecontainer == null)
            {
               this._gamePlayerList.splice(_loc4_,1);
            }
            else if(_loc1_.facecontainer.isActingExpression)
            {
               if(!(_loc1_.facecontainer.expressionID >= 49 || _loc1_.facecontainer.expressionID <= 0))
               {
                  _loc2_ = this.localToGlobal(new Point(_loc1_.x,_loc1_.y));
                  _loc3_ = this.onStageFlg(_loc2_);
                  if(_loc3_ == 0)
                  {
                     _loc1_.facecontainer.x = 0;
                     _loc1_.facecontainer.y = -100;
                     this.resetDicAndVec(_loc1_);
                     _loc1_.facecontainer.isShowNickName = false;
                  }
                  else if(_loc3_ == 1)
                  {
                     _loc1_.facecontainer.x = _loc1_.facecontainer.width / 2 + 30 - _loc2_.x;
                     _loc1_.facecontainer.y = 270 + _loc1_.facecontainer.height / 2 - _loc2_.y;
                     this.controlExpNum(_loc1_);
                     _loc1_.facecontainer.isShowNickName = true;
                  }
                  if(this.expName.length == 2)
                  {
      ...
      private function onStageFlg(param1:Point) : int
      {
         ...
         if(param1.x >= 0 && param1.x <= 995 && param1.y >= 0 && param1.y <= 595)
         {
            return 0;
         }
         return 1;
      ...
      override public function removePhysical(param1:Physics) : void
      {
         ...
         {
            _loc2_ = param1 as PhysicalObj;
            if(Boolean(this._objects) && Boolean(this._objects[_loc2_.Id]))
            {
               delete this._objects[_loc2_.Id];
            }
            if(Boolean(this._smallMap) && Boolean(_loc2_.smallView))
            {
      ...
      override public function removeMapThing(param1:Physics) : void
      {
         ...
         {
            _loc2_ = param1 as PhysicalObj;
            if(Boolean(this._objects[_loc2_.Id]))
            {
               delete this._objects[_loc2_.Id];
            }
            if(Boolean(_loc2_.smallView))
      ...
      private function playPickBoxAwardMove(param1:Event) : void
      {
         ...
         {
            this._picList = [];
            _loc2_ = int(this._picIdList.length);
            _loc3_ = -1;
            _loc4_ = 0;
            while(_loc4_ < _loc2_)
            {
      ...
      private function getTwoHundredDisPoint(param1:Number, param2:Number, param3:Number, param4:Number, param5:int) : Point
      {
         ...
         param2 = 150;
         var _loc7_:Number = param1 + 200 * param5 + param3 * param5;
         if(!this.IsOutMap(_loc7_,param2) && this.IsEmpty(_loc7_,param2))
         {
            return findYLineNotEmptyPointDown(param1 + 200 * param5,param2,this.bound.height);
         }
         param5 *= -1;
         _loc7_ = param1 + 200 * param5 + param3 * param5;
         return findYLineNotEmptyPointDown(param1 + 200 * param5,param2,this.bound.height);
      }
