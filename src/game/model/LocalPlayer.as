package game.model
{
   ...
   public class LocalPlayer extends Player
   {
      ...
      override public function die(param1:Boolean = true) : void
      {
         ...
         var _loc3_:DictionaryData = GameManager.Instance.Current.findTeam(team);
         for each(_loc2_ in _loc3_)
         {
            if(!_loc2_.isSelf && _loc2_.isLiving)
            {
               this.isLast = false;
               break;
            }
      ...
      private function pushUseProp(param1:int, param2:PropInfo) : Boolean
      {
         ...
         {
            _loc3_ = this._numObject[param2.TemplateID] as int;
            if(_loc3_ == 2)
            {
         ...
         {
            _loc4_ = this._numObject[EquipType.ADD_TWO_ATTACK] as int;
            _loc5_ = this._numObject[EquipType.THREEKILL] as int;
            _loc6_ = this._numObject[EquipType.ADD_ONE_ATTACK] as int;
