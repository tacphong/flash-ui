package game
{
   ...
   public class TryAgain extends Sprite implements Disposeable
   {
      ...
      private function drawMark(param1:int) : void
      {
         ...
         var _loc4_:int = 0;
         var _loc5_:Graphics = this._markshape.graphics;
         _loc5_.clear();
         ...
         if(param1 == 10)
         {
            ...
            {
               _loc3_ = "Blue" + _loc6_.substr(_loc4_,1);
               _loc2_ = this._numDic[_loc3_];
               _loc5_.beginBitmapFill(_loc2_,new Matrix(1,0,0,1,this._markshape.width));
               _loc5_.drawRect(this._markshape.width,0,_loc2_.width,_loc2_.height);
               _loc5_.endFill();
               _loc4_++;
            }
            this._markshape.x = (this._back.width - _loc2_.width >> 1) - 20;
         }
         else
         {
            _loc2_ = this._numDic["Blue" + _loc6_];
            _loc5_.beginBitmapFill(_loc2_);
            _loc5_.drawRect(0,0,_loc2_.width,_loc2_.height);
            _loc5_.endFill();
            this._markshape.x = this._back.width - _loc2_.width >> 1;
         }
      }
