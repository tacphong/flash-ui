package game.objects
{
   ...
   public class GameLiving extends PhysicalObj
   {
      ...
      {
         if(param1.paras[0] == BuffType.Turn && Boolean(this._info))
         {
            this._buffBar.update(this._info.turnBuffs);
      ...
      protected function __playSkillMovie(param1:LivingEvent) : void
      {
         ...
         var _loc5_:MovieClip = ClassUtils.CreatInstance("asset.game.propanimation." + param1.paras[0]);
         if(Boolean(_loc5_))
         {
            _loc2_ = new MovieClipWrapper(_loc5_,true,true);
            _loc2_.addEventListener(Event.COMPLETE,this.__skillMovieComplete);
            if(param1.paras[0] == "guild")
            {
               _loc3_ = _map;
               _loc4_ = parent.localToGlobal(this._info.pos);
               _loc4_ = _loc3_.globalToLocal(_loc4_);
               _loc2_.movie.x = _loc4_.x;
               _loc2_.movie.y = _loc4_.y;
               _loc3_.addChild(_loc2_.movie);
            }
            else
            {
      ...
      protected function __say(param1:LivingEvent) : void
      {
         ...
         var _loc3_:int = 0;
         if(Boolean(param1.paras[1]))
         {
            _loc3_ = int(param1.paras[1]);
         }
      ...
      protected function updateTargetsBlood(param1:Array) : void
      {
         ...
         {
            if(Boolean(param1[_loc4_]) && Boolean(param1[_loc4_].target) && Boolean(param1[_loc4_].target.isLiving))
            {
               _loc2_ = param1[_loc4_].target;
               _loc2_.isHidden = false;
               _loc2_.showAttackEffect(param1[_loc4_].attackEffect);
               _loc2_.updateBlood(param1[_loc4_].targetBlood,3,param1[_loc4_].damage);
               if(!_loc3_)
               {
                  this.map.setCenter(_loc2_.pos.x,_loc2_.pos.y - 150,true);
               }
               if(_loc2_.isSelf)
               {
                  _loc3_ = true;
      ...
      {
         var _loc2_:ShootPercentView = null;
         var _loc3_:int = 0;
         var _loc4_:int = 0;
         ...
         var _loc5_:Number = param1.value - param1.old;
         var _loc6_:int = int(param1.paras[0]);
         switch(_loc6_)
         {
            case 0:
               _loc5_ = Number(param1.paras[1]);
               if(_loc5_ != 0 && this._info.blood != 0)
               {
                  _loc2_ = new ShootPercentView(Math.abs(_loc5_),1,true);
                  _loc2_.x = this.x + this.offset();
                  _loc2_.y = this.y - 50 + this.offset(25);
                  _loc2_.scaleX = _loc2_.scaleY = 0.8 + Math.random() * 0.4;
                  _map.addToPhyLayer(_loc2_);
                  if(this._info.isHidden)
                  {
               ...
               break;
            case 90:
               _loc2_ = new ShootPercentView(0,2);
               _loc2_.x = this.x + this.offset();
               _loc2_.y = this.y - 50 + this.offset(25);
               _loc2_.scaleX = _loc2_.scaleY = 0.8 + Math.random() * 0.4;
               _map.addToPhyLayer(_loc2_);
            ...
            case 5:
               break;
            case 3:
               _loc4_ = int(param1.paras[1]);
               _loc5_ = _loc4_;
               if(_loc5_ != 0)
               {
                  _loc2_ = new ShootPercentView(Math.abs(_loc5_),1,false);
                  _loc2_.x = this.x + this.offset();
                  _loc2_.y = this.y - 50 + this.offset(25);
                  _loc2_.scaleX = _loc2_.scaleY = 0.8 + Math.random() * 0.4;
                  _map.addToPhyLayer(_loc2_);
            ...
            case 11:
               ...
               if(_loc5_ != 0)
               {
                  _loc2_ = new ShootPercentView(Math.abs(_loc5_),param1.paras[0],false);
                  _loc2_.x = this.x - 70;
                  _loc2_.y = this.y - 80;
                  _loc2_.scaleX = _loc2_.scaleY = 0.8 + Math.random() * 0.4;
                  _map.addToPhyLayer(_loc2_);
      ...
      protected function __moveTo(param1:LivingEvent) : void
      {
         ...
         var _loc3_:String = String(param1.paras[4]);
         this.doAction(_loc3_);
         var _loc4_:int = int(param1.paras[5]);
         if(_loc4_ == 0)
         {
            _loc4_ = 7;
         }
         var _loc5_:Point = param1.paras[1] as Point;
         var _loc6_:int = int(param1.paras[2]);
         var _loc7_:String = String(param1.paras[6]);
         if(this.x == _loc5_.x && this.y == _loc5_.y)
         {
            return;
         }
         var _loc8_:Array = [];
         var _loc9_:int = this.x;
         var _loc10_:int = this.y;
         var _loc11_:Point = new Point(this.x,this.y);
         var _loc12_:int = _loc5_.x > _loc9_ ? int(1) : int(-1);
         var _loc13_:Point = new Point(this.x,this.y);
         if(_loc3_.substr(0,3) == "fly")
         {
            _loc2_ = new Point(_loc5_.x - _loc13_.x,_loc5_.y - _loc13_.y);
            while(_loc2_.length > _loc4_)
            {
               _loc2_.normalize(_loc4_);
               _loc13_ = new Point(_loc13_.x + _loc2_.x,_loc13_.y + _loc2_.y);
               _loc2_ = new Point(_loc5_.x - _loc13_.x,_loc5_.y - _loc13_.y);
               if(!Boolean(_loc13_))
               {
                  _loc8_.push(_loc5_);
         ...
         {
            while((_loc5_.x - _loc9_) * _loc12_ > 0)
            {
               _loc13_ = _map.findNextWalkPoint(_loc9_,_loc10_,_loc12_,_loc4_ * npcStepX,_loc4_ * npcStepY);
               if(!Boolean(_loc13_))
               {
                  break;
               }
         ...
         if(_loc8_.length > 0)
         {
            this._info.act(new LivingMoveAction(this,_loc8_,_loc6_,_loc7_));
         }
         else if(_loc7_ != "")
         {
            this.doAction(_loc7_);
         }
         else
         {
      ...
      protected function __showAttackEffect(param1:LivingEvent) : void
      {
         ...
         var _loc2_:int = int(param1.paras[0]);
      ...
      public function showEffect(param1:String) : void
      {
         ...
         if(Boolean(param1) && ModuleLoader.hasDefinition(param1))
         {
      ...
      public function showBuffEffect(param1:String, param2:int) : void
      {
         ...
         if(Boolean(param1) && ModuleLoader.hasDefinition(param1))
         {
            ...
            if(Boolean(this._buffEffect) && this._buffEffect.hasKey(param2))
            {
      ...
      {
         var _loc2_:DisplayObject = null;
         if(Boolean(this._buffEffect) && this._buffEffect.hasKey(param1))
         {
      ...
      protected function creatAttackEffectAssetByID(param1:int) : MovieClip
      {
         ...
         if(this.hasMovie(_loc3_))
         {
            return this._moviePool[_loc3_] as MovieClip;
         }
         _loc2_ = ClassUtils.CreatInstance("asset.game.AttackEffect" + param1.toString()) as MovieClip;
         this._moviePool[_loc3_] = _loc2_;
         return _loc2_;
      }
