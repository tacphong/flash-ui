package game.objects
{
   ...
   public class SimpleBomb extends BaseBomb
   {
      ...
      protected function initMovie() : void
      {
         ...
         this._dir = this._info.VX >= 0 ? int(1) : int(-1);
         x = this._info.X;
         y = this._info.Y;
      ...
      override public function bomb() : void
      {
         ...
         var _loc3_:Array = this.map.getPhysicalObjectByPoint(pos,100,this);
         for each(_loc1_ in _loc3_)
         {
            if(_loc1_ is TombView || _loc1_ is GamePet)
            {
               _loc1_.startMoving();
            }
         }
         ...
         if(!this.fastModel)
         {
            if(this._info.Template.Shake)
            {
               ...
               this.map.animateSet.addAnimation(new ShockMapAnimation(this,_loc2_));
            }
      ...
      override public function bombAtOnce() : void
      {
         ...
         this.fastModel = true;
         var _loc3_:int = 0;
         while(_loc3_ < this._info.Actions.length)
         {
            if(this._info.Actions[_loc3_].type == ActionType.BOMB)
            {
               _loc1_ = this._info.Actions[_loc3_];
               break;
            }
            _loc3_++;
         }
         var _loc4_:int = this._info.Actions.indexOf(_loc1_);
         var _loc5_:Array = this._info.Actions.splice(_loc4_,1);
         if(Boolean(_loc1_))
         {
