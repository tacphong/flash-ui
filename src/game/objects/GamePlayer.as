package game.objects
{
   ...
   public class GamePlayer extends GameTurnedLiving
   {
      ...
      private var _petMovie:game.objects.GamePetMovie;
      ...
      private var _tomb:game.objects.TombView;
      ...
      public function GamePlayer(param1:Player, param2:ShowCharacter, param3:GameCharacter = null)
      {
         ...
         {
            this._petMovie = new game.objects.GamePetMovie(param1.currentPet.petInfo,this);
      ...
      override protected function initView() : void
      {
         ...
         if(Boolean(this.player.playerInfo.ConsortiaName) || Boolean(this.player.playerInfo.honor))
         {
            ...
            {
               _loc3_ = GameManager.Instance.Current;
               _loc4_ = PlayerManager.Instance.Self;
               if(_loc4_.ConsortiaID == 0 || _loc4_.ConsortiaID == this.player.playerInfo.ConsortiaID && _loc4_.ZoneID == this.player.playerInfo.ZoneID || _loc3_ && _loc3_.gameMode == 2)
               {
         ...
         if(PlayerManager.Instance.Self.IsShowNewTitle == true)
         {
            ...
            {
               _loc1_ = NewTitleManager.instance.getTitleByName(this.player.playerInfo.honor);
               if(_loc1_ && _loc1_.Show != "0" && _loc1_.Pic != "0")
               {
                  _loc2_ = LoaderManager.Instance.creatLoader(PathManager.solvePath("image/title/" + _loc1_.Pic + "/icon.png"),0);
      ...
      protected function __onComplete(param1:LoaderEvent) : void
      {
         ...
         _loc2_.removeEventListener("complete",this.__onComplete);
         var _loc3_:Bitmap = _loc2_.content;
         if(Boolean(_loc3_) && this.isLiving)
         {
            this._newTitle = _loc3_;
            this._newTitle.x = -_loc3_.width / 2;
      ...
      {
         var _loc2_:String = String(param1.paras[0]);
      ...
      private function updateHp(param1:Array) : void
      {
         ...
         for each(_loc2_ in param1)
         {
            ...
            _loc4_ = int(_loc2_.hp);
            _loc5_ = int(_loc2_.damage);
            _loc6_ = int(_loc2_.dander);
      ...
      override public function die() : void
      {
         ...
         var _loc1_:game.objects.TombView = new game.objects.TombView();
         _loc1_.pos = this.pos;
         _map.addPhysical(_loc1_);
         _loc1_.startMoving();
         this._tomb = new game.objects.TombView();
      ...
      private function __getFace(param1:ChatEvent) : void
      {
         ...
         if(_loc4_["playerid"] == this.player.playerInfo.ID)
         {
            _loc2_ = int(_loc4_["faceid"]);
            _loc3_ = int(_loc4_["delay"]);
            if(_loc2_ >= 49)
            {
               setTimeout(this.showFace,_loc3_,_loc2_);
            }
            else
            {
               this.showFace(_loc2_);
            }
      ...
      public function getAction(param1:String) : PlayerAction
      {
         ...
         {
            param1 = String(this.labelMapping[param1]);
         }
         switch(param1)
         {
            case "stand":
