package game
{
   ...
   public class GameManager extends EventDispatcher
   {
      ...
      private static var _instance:game.GameManager;
      ...
      public static function get Instance() : game.GameManager
      {
         if(_instance == null)
         {
            _instance = new game.GameManager();
         }
         return _instance;
      }
      ...
      private function __updatePlayInfoInGame(param1:CrazyTankSocketEvent) : void
      {
         ...
         var _loc3_:RoomInfo = RoomManager.Instance.current;
         if(_loc3_ == null)
         {
            return;
         }
         ...
         var _loc11_:RoomPlayer = RoomManager.Instance.current.findPlayerByID(_loc6_);
         if(_loc5_ != PlayerManager.Instance.Self.ZoneID || _loc11_ == null || this._current == null)
         {
            return;
         }
         if(_loc11_.isSelf)
         {
            _loc2_ = new LocalPlayer(PlayerManager.Instance.Self,_loc8_,_loc7_,_loc9_);
         }
         else
         {
            _loc2_ = new Player(_loc11_.playerInfo,_loc8_,_loc7_,_loc9_);
         }
         _loc2_.isReady = _loc10_;
         if(Boolean(_loc2_.movie))
         {
            _loc2_.movie.setDefaultAction(_loc2_.movie.standAction);
         }
         this._current.addRoomPlayer(_loc11_);
         if(_loc11_.isViewer)
         {
            this._current.addGameViewer(_loc2_);
         }
         else
         {
            this._current.addGamePlayer(_loc2_);
         }
         if(_loc11_.isSelf)
         {
      ...
      private function __createGame(param1:CrazyTankSocketEvent) : void
      {
         ...
         if(Boolean(RoomManager.Instance.current))
         {
            ...
            CryptBossManager.instance.RoomType = _loc3_.roomType;
         ...
         if(Boolean(this._current))
         {
            ...
            while(_loc4_ <= _loc3_)
            {
               ...
               if(_loc6_ != null)
               {
                  ...
                  if(RoomManager.Instance.current.type != 5 && Boolean(_loc6_.playerInfo.currentPet))
                  {
      ...
      private function __missionAllOver(param1:CrazyTankSocketEvent) : void
      {
         ...
         while(_loc2_ < _loc8_)
         {
            ...
            _loc5_ = this._current.findPlayerByPlayerID(_loc4_);
            if(Boolean(_loc5_.expObj))
            {
               _loc3_ = _loc5_.expObj;
            }
            else
            {
               _loc3_ = new Object();
            ...
            if(Boolean(_loc5_))
            {
               _loc3_.killGP = _loc7_.readInt();
               _loc3_.hertGP = _loc7_.readInt();
               _loc3_.fightGP = _loc7_.readInt();
               _loc3_.ghostGP = _loc7_.readInt();
               _loc3_.gpForVIP = _loc7_.readInt();
               _loc3_.gpForSpouse = _loc7_.readInt();
               _loc3_.gpForServer = _loc7_.readInt();
               _loc3_.gpForApprenticeOnline = _loc7_.readInt();
               _loc3_.gpForApprenticeTeam = _loc7_.readInt();
               _loc3_.gpForDoubleCard = _loc7_.readInt();
               _loc3_.consortiaSkill = _loc7_.readInt();
               _loc3_.luckyExp = _loc7_.readInt();
               _loc3_.gainGP = _loc7_.readInt();
               _loc5_.isWin = _loc7_.readBoolean();
               _loc5_.expObj = _loc3_;
            }
            _loc2_++;
         }
         ...
         {
            this._current.missionInfo.missionOverNPCMovies.push(_loc7_.readUTF());
            _loc10_++;
         }
      }
      ...
      public function selfGetItemShowAndSound(param1:Dictionary) : Boolean
      {
         ...
         for each(_loc2_ in param1)
         {
            ...
            _loc4_ = LanguageMgr.GetTranslation("tank.data.player.FightingPlayerInfo.your");
            _loc5_ = ChatFormats.getTagsByChannel(_loc3_.channel);
            _loc6_ = ChatFormats.creatGoodTag("[" + _loc2_.Name + "]",ChatFormats.CLICK_GOODS,_loc2_.TemplateID,_loc2_.Quality,_loc2_.IsBinds,_loc3_);
            _loc3_.htmlMessage = _loc5_[0] + _loc4_ + _loc6_ + _loc5_[1] + "<BR>";
            ChatManager.Instance.chat(_loc3_,false);
            if(_loc2_.Quality >= 3)
            {
               _loc7_ = true;
            }
      ...
      public function isIdenticalGame(param1:int = 0) : Boolean
      {
         ...
         {
            if(_loc2_.playerInfo.ID == param1 && _loc2_.playerInfo.ZoneID == _loc4_.ZoneID)
            {
               return true;
            }
         }
      ...
      {
         var _loc1_:int = int(this._loaderArray.length);
         var _loc2_:int = 0;
         while(_loc2_ < _loc1_)
         {
