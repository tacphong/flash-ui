package game.actions.newHand
{
   ...
   public class NewHandFightHelpAction extends BaseNewHandFightHelpAction
   {
      ...
      private function checkShootDirection() : Boolean
      {
         ...
         var _loc2_:int = this._enemyPlayer.pos.x > this._player.pos.x ? int(1) : int(-1);
         var _loc3_:int = _loc1_.target.x >= _loc1_.X ? int(1) : int(-1);
         if(_loc2_ != _loc3_)
         {
      ...
      {
         var _loc1_:int = this._player.NewHandSelfBlood > 0 ? int(this._player.NewHandSelfBlood) : int(this._player.maxBlood);
      ...
      private function getRecentBomb() : Bomb
      {
         ...
         var _loc3_:int = 0;
         var _loc4_:int = -1;
         for each(_loc2_ in this._bombs)
         {
            _loc3_ = _loc2_.Template.ID;
            if(_loc2_ && (_loc3_ != 64 && _loc3_ != Bomb.FLY_BOMB && _loc3_ != Bomb.FREEZE_BOMB) && (_loc4_ == -1 || Math.abs(_loc2_.target.x - this._enemyPlayer.pos.x) < _loc4_))
            {
               _loc4_ = Math.abs(_loc2_.target.x - this._enemyPlayer.pos.x);
               _loc1_ = _loc2_;
            }
         }
         return _loc1_;
      ...
      private function checkHurtEnemy(param1:Boolean = true) : Boolean
      {
         ...
         else
         {
            ...
            {
               _loc3_ = this._enemyPlayer.pos.x > this._player.pos.x ? int(1) : int(-1);
               _loc4_ = _loc2_.target.x > this._enemyPlayer.pos.x ? int(1) : int(-1);
               if(param1)
               {
