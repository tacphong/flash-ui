package game.actions
{
   ...
   public class SelfPlayerWalkAction extends BaseAction
   {
      ...
      private function transmissionGate() : void
      {
         ...
         var _loc2_:Rectangle = this._player.getCollideRect();
         _loc2_.offset(this._player.x,this._player.y);
         var _loc3_:Array = this._player.map.getCollidedPhysicalObjects(_loc2_,this._player);
         if(_loc3_.length != 0 && this._transmissionGate)
         {
            for each(_loc1_ in _loc3_)
            {
               if(_loc1_ is SimpleObject && _loc1_.layerType == 3)
               {
