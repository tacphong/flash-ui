package game.animations
{
   ...
   public class SpellSkillAnimation extends EventDispatcher implements IAnimate
   {
      ...
      private var _effect:game.animations.ScaleEffect;
      ...
      {
         if(Boolean(this._skill) && Boolean(this._skill.parent))
         {
            this._skill.parent.removeChild(this._skill);
         }
         if(Boolean(this._skillAsset) && Boolean(this._skillAsset.parent))
         {
         ...
         if(Boolean(this.map))
         {
            ...
            if(Boolean(this.map.ground))
            {
               this.map.ground.alpha = 1;
            }
            if(Boolean(this.map.stone))
            {
               this.map.stone.alpha = 1;
            }
            if(Boolean(this.map.sky))
            {
      ...
      public function update(param1:MapView) : Boolean
      {
         ...
         try
         {
            this.map = movie;
            ++this._life;
            if(Boolean(this._skill) && Boolean(this._effect))
            {
            ...
            else if(this._life == 15)
            {
               ...
               this._effect = new game.animations.ScaleEffect(this._skillType,bmd);
               this._skill.addChild(this._effect);
               this._skill.scaleX = this._player.player.direction;
               this._skill.x = this._player.player.direction > 0 ? Number(0) : Number(1000);
