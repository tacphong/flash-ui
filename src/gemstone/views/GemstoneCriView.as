package gemstone.views
{
   ...
   public class GemstoneCriView extends Sprite implements Disposeable
   {
      ...
      public function GemstoneCriView()
      {
         ...
         while(_loc3_ < 3)
         {
            _loc1_ = new GemstoneContent(_loc3_,this._centerP);
            _loc1_.id = _loc3_ + 1;
            addChild(_loc1_);
            this._contArray.push(_loc1_);
            _loc2_ = new Point(_loc1_.x,_loc1_.y);
            this._startPointArr.push(_loc2_);
            _loc3_++;
         }
         this._item = new Item();
         this._item.x = -105;
      ...
      public function resetGemstoneList(param1:Vector.<GemstListInfo>) : void
      {
         ...
         while(_loc5_ < 3)
         {
            ...
            {
               this._contArray[0].info = param1[_loc5_];
            ...
            {
               this._contArray[1].info = param1[_loc5_];
               if(param1[_loc5_].level > 0)
               {
            ...
            {
               this._contArray[2].info = param1[_loc5_];
               if(param1[_loc5_].level > 0)
               {
         ...
         var _loc6_:int = this.curInfo.level;
         this.setCurInfo(param1[0].fightSpiritId,_loc6_);
         if(_loc6_ >= this.maxLevel)
         {
            _loc6_ = this.maxLevel;
            _loc3_ = this.staticDataList[_loc6_].Exp - this.staticDataList[_loc6_ - 1].Exp;
            GemstoneUpView(parent).expBar.initBar(_loc3_,_loc3_,true);
            return;
         }
         _loc6_++;
         _loc3_ = this.staticDataList[_loc6_].Exp - this.staticDataList[_loc6_ - 1].Exp;
         GemstoneUpView(parent).expBar.initBar(this.curInfo.exp,_loc3_);
      }
      ...
      private function setCurInfo(param1:int, param2:int) : void
      {
         ...
         _loc6_.curLve = LanguageMgr.GetTranslation("ddt.gemstone.curInfo.pdescriptTxt1") + param2;
         _loc6_.levHe = int(this._contArray[0].info.level) + int(this._contArray[1].info.level) + int(this._contArray[2].info.level);
         if(param1 == GemstoneManager.ID1)
         {
            _loc3_ = this.staticDataList[this._contArray[0].info.level].attack;
            _loc4_ = this.staticDataList[this._contArray[1].info.level].attack;
            _loc5_ = this.staticDataList[this._contArray[2].info.level].attack;
            if(param2 >= this.maxLevel)
            {
               param2 = this.maxLevel;
            }
            _loc6_.upGrdPro = LanguageMgr.GetTranslation("ddt.gemstone.curInfo.pdescriptTxt3") + this.staticDataList[param2].attack;
         }
         else if(param1 == GemstoneManager.ID2)
         {
            _loc3_ = this.staticDataList[this._contArray[0].info.level].defence;
            _loc4_ = this.staticDataList[this._contArray[1].info.level].defence;
            _loc5_ = this.staticDataList[this._contArray[2].info.level].defence;
            if(param2 >= this.maxLevel)
            {
               param2 = this.maxLevel;
            }
            _loc6_.upGrdPro = LanguageMgr.GetTranslation("ddt.gemstone.curInfo.pdescriptTxt3") + this.staticDataList[param2].defence;
         }
         else if(param1 == GemstoneManager.ID3)
         {
            _loc3_ = this.staticDataList[this._contArray[0].info.level].agility;
            _loc4_ = this.staticDataList[this._contArray[1].info.level].agility;
            _loc5_ = this.staticDataList[this._contArray[2].info.level].agility;
            if(param2 >= this.maxLevel)
            {
               param2 = this.maxLevel;
            }
            _loc6_.upGrdPro = LanguageMgr.GetTranslation("ddt.gemstone.curInfo.pdescriptTxt3") + this.staticDataList[param2].agility;
         }
         else if(param1 == GemstoneManager.ID4)
         {
            _loc3_ = this.staticDataList[this._contArray[0].info.level].luck;
            _loc4_ = this.staticDataList[this._contArray[1].info.level].luck;
            _loc5_ = this.staticDataList[this._contArray[2].info.level].luck;
            if(param2 >= this.maxLevel)
            {
               param2 = this.maxLevel;
            }
            _loc6_.upGrdPro = LanguageMgr.GetTranslation("ddt.gemstone.curInfo.pdescriptTxt3") + this.staticDataList[param2].luck;
         }
         else if(param1 == GemstoneManager.ID5)
         {
            _loc3_ = this.staticDataList[this._contArray[0].info.level].blood;
            _loc4_ = this.staticDataList[this._contArray[1].info.level].blood;
            _loc5_ = this.staticDataList[this._contArray[2].info.level].blood;
            if(param2 >= this.maxLevel)
            {
               param2 = this.maxLevel;
            }
            _loc6_.upGrdPro = LanguageMgr.GetTranslation("ddt.gemstone.curInfo.pdescriptTxt3") + this.staticDataList[param2].blood;
         }
         _loc6_.proHe = _loc3_ + _loc4_ + _loc5_;
         (parent as GemstoneUpView).upDataCur(_loc6_);
      }
      ...
      public function upGradeAction(param1:GemstoneUpGradeInfo) : void
      {
         ...
         if(!param1.isUp)
         {
            ...
            while(_loc3_ < 3)
            {
               if(param1.list[_loc3_].place == 2)
               {
                  if(Boolean(this.curInfo))
                  {
                     GemstoneManager.Instance.gemstoneFrame.getMaskMc().visible = false;
                     this.curInfo = param1.list[_loc3_];
                     break;
                  }
            ...
            _loc4_ = this.staticDataList[_loc2_].Exp - this.staticDataList[_loc2_ - 1].Exp;
            GemstoneManager.Instance.expBar.initBar(this.curInfo.exp,_loc4_);
            return;
         }
         this._isLeft = true;
         this._isAction = true;
         ...
         var _loc5_:int = this.staticDataList[_loc2_].Exp - this.staticDataList[_loc2_ - 1].Exp;
         GemstoneManager.Instance.expBar.initBar(_loc5_,_loc5_);
      ...
      public function gemstoAction() : void
      {
         ...
         var _loc6_:int = 0;
         var _loc7_:int = 0;
         var _loc8_:int = int(this._contArray.length);
         if(!this._isLeft)
         {
            if(this._contArray[0].x == this._startPointArr[2].x)
            {
               ...
               while(_loc1_ < 3)
               {
                  ...
                  TweenLite.to(this._contArray[_loc1_],0.5,{
                     "x":this._pointArray[_loc7_].x,
                     "y":this._pointArray[_loc7_].y,
                     "onComplete":this._funArray[_loc1_]
                  });
                  _loc1_++;
               }
            }
            else if(this._contArray[0].x == this._startPointArr[1].x)
            {
               ...
               {
                  _loc7_ = _loc2_;
                  TweenLite.to(this._contArray[_loc2_],0.5,{
                     "x":this._pointArray[_loc7_].x,
                     "y":this._pointArray[_loc7_].y,
                     "onComplete":this._funArray[_loc2_]
                  });
                  _loc2_++;
               }
            }
      ...
      private function enterframeHander(param1:Event) : void
      {
         ...
         if(this._upGradeMc.currentFrame == this._upGradeMc.totalFrames - 1)
         {
            ...
            _loc3_ = int(this._contArray.length);
            _loc4_ = 0;
            while(_loc4_ < _loc3_)
            {
