package
{
   ...
   public class FrameByFrameItem extends BitmapRendItem
   {
      ...
      private function initRectangles() : void
      {
         ...
         var _loc2_:int = Math.ceil(this._source.width / _itemWidth);
         var _loc3_:int = Math.ceil(this._source.height / _itemHeight);
         var _loc4_:int = 1;
         while(_loc4_ <= _loc3_)
         {
            _loc1_ = 1;
            while(_loc1_ <= _loc2_)
            {
               this._rects.push(new Rectangle((_loc1_ - 1) * _itemWidth,(_loc4_ - 1) * _itemHeight,_itemWidth,_itemHeight));
               _loc1_++;
            }
            _loc4_++;
         }
