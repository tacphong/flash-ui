package lottery.view
{
   ...
   public class CardChooseRightView extends Sprite implements Disposeable
   {
      ...
      private function initView() : void
      {
         ...
         this._btnReset.enable = this._btnSure.enable = false;
         this._enable = true;
      }
      ...
      {
         var _loc2_:ChoosedCardCell = null;
         if(!this._enable)
         {
            return;
         }
         ...
         var _loc3_:int = 0;
         while(_loc3_ < SELECTED_LIMIT)
         {
            _loc2_ = this._selectedContainer.getChildAt(_loc3_) as ChoosedCardCell;
            _loc2_.hide();
            _loc3_++;
         }
      ...
      public function setLastEmptyCardId(param1:int) : void
      {
         ...
         this._btnSure.enable = this._selectedCount >= SELECTED_LIMIT ? Boolean(true) : Boolean(false);
      ...
      private function cardLottery() : void
      {
         ...
         while(_loc3_ < SELECTED_LIMIT)
         {
            _loc1_ = this._selectedContainer.getChildAt(_loc3_) as ChoosedCardCell;
            if(_loc1_.cardId < 1)
            {
               return;
            }
            _loc2_.push(_loc1_.cardId);
            _loc3_++;
         }
         MessageTipManager.getInstance().show(LanguageMgr.GetTranslation("tank.lottery.cardLottery.wagerSuccess"));
         SocketManager.Instance.out.sendCardLotteryIds(_loc2_);
