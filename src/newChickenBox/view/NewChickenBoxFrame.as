package newChickenBox.view
{
   ...
   public class NewChickenBoxFrame extends Frame implements Disposeable
   {
      ...
      public var newBoxView:newChickenBox.view.NewChickenBoxView;
      ...
      private function initView() : void
      {
         ...
         this.newBoxView = new newChickenBox.view.NewChickenBoxView();
      ...
      private function firestGetTime() : Boolean
      {
         ...
         var _loc6_:Number = _loc5_.getTime();
         var _loc7_:Number = this._model.lastFlushTime.getTime();
         var _loc8_:Number = this._model.freeFlushTime * 60 * 1000;
         ...
         else
         {
            ...
            _loc4_ = (_loc2_ - _loc3_ * 1000 * 60 * 60) / (1000 * 60);
            this._refreshTimerTxt.text = LanguageMgr.GetTranslation("newChickenBox.flushTimecut",_loc3_,_loc4_);
            if(Boolean(this.flushBnt))
            {
      ...
      private function flushItem(param1:MouseEvent) : void
      {
         ...
         Mouse.show();
         var _loc11_:int = this._model.flushPrice;
         var _loc12_:Boolean = this.firestGetTime();
         if(!_loc12_ && PlayerManager.Instance.Self.Money < _loc11_)
         {
            LeavePageManager.showFillFrame();
            return;
         }
         if(this._model.AlertFlush && !_loc12_)
         {
            this.openAlertFrame();
         }
         else if(_loc12_)
         {
            this.startBnt.enable = true;
            this.eyeBtn.enable = false;
            _loc2_ = this._model.canOpenCounts + 1;
            this.countNum.setFrame(_loc2_);
            this._model.countTime = 0;
            this._model.countEye = 0;
            _loc3_ = TimeManager.Instance.Now();
            _loc4_ = _loc3_.getTime();
            _loc5_ = this._model.lastFlushTime.getTime();
            _loc6_ = this._model.freeFlushTime * 60 * 1000;
            _loc7_ = _loc6_;
            _loc8_ = _loc7_ / (1000 * 60 * 60);
            _loc9_ = (_loc7_ - _loc8_ * 1000 * 60 * 60) / (1000 * 60);
            this._refreshTimerTxt.text = LanguageMgr.GetTranslation("newChickenBox.flushTimecut",_loc8_,_loc9_);
            if(Boolean(this.flushBnt))
            {
               this.flushBnt.text = LanguageMgr.GetTranslation("newChickenBox.flushText");
            }
         ...
         {
            this.startBnt.enable = true;
            _loc10_ = this._model.canOpenCounts + 1;
            this.countNum.setFrame(_loc10_);
            this._model.countTime = 0;
            this._model.countEye = 0;
            this.eyeBtn.enable = false;
            this._model.canclickEnable = false;
      ...
      private function __onResponse(param1:FrameEvent) : void
      {
         ...
         _loc3_.removeEventListener(FrameEvent.RESPONSE,this.__onResponse);
         _loc3_.dispose();
