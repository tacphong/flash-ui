package church.view.churchScene
{
   ...
   public class MoonSceneMap extends SceneMap
   {
      ...
      public static const GAME_WIDTH:int = 995;
      
      public static const GAME_HEIGHT:int = 595;
      ...
      override public function setSalute(param1:int) : void
      {
         ...
         _loc2_ = new _loc4_();
         _loc3_ = ComponentFactory.Instance.creatCustomObject("church.MoonSceneMap.saluteFirePos");
         _loc2_.x = _loc3_.x;
         _loc2_.y = _loc3_.y;
         if(this.isSaluteFiring)
         {
            this.saluteQueue.push(_loc2_);
         }
         else
         {
            this.isSaluteFiring = true;
            _loc2_.addEventListener(Event.ENTER_FRAME,this.saluteFireFrameHandler);
            _loc2_.gotoAndPlay(1);
            this.saluteContainer.addChild(_loc2_);
         }
      }
      ...
      private function __timer(param1:TimerEvent) : void
      {
         ...
         if(_loc2_ < 6)
         {
            _loc3_ = !(Math.round(Math.random() * 9) % 3) ? Boolean(true) : Boolean(false);
            if(_loc3_)
            {
