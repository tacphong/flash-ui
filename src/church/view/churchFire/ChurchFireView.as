package church.view.churchFire
{
   ...
   public class ChurchFireView extends Sprite implements Disposeable
   {
      ...
      {
         var _loc1_:ChurchFireCell = null;
         var _loc2_:int = 0;
         while(_loc2_ < this.All_Fires.length)
         {
            _loc1_ = new ChurchFireCell(this.All_FireIcons[_loc2_],ShopManager.Instance.getGoldShopItemByTemplateID(this.All_Fires[_loc2_]),this.All_Fires[_loc2_]);
            _loc1_.addEventListener(MouseEvent.CLICK,this.itemClickHandler);
            this._fireListBox.addChild(_loc1_);
            _loc2_++;
         }
      }
