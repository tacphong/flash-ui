package hotSpring.view
{
   ...
   public class HotSpringMainView extends Sprite implements Disposeable
   {
      ...
      private var _roomList:hotSpring.view.RoomListView;
      ...
      protected function initialize() : void
      {
         ...
         this._roomList = new hotSpring.view.RoomListView(this._controller,this._model);
         _loc1_ = ComponentFactory.Instance.creatCustomObject("asset.HotSpringMainView.roomListPos");
      ...
      private function getPageList(param1:MouseEvent) : void
      {
         ...
         switch(param1.currentTarget)
         {
            case this._firstPageBtn:
               ...
               break;
            case this._preBtn:
               this._roomList.pageIndex = this._roomList.pageIndex - 1 > 0 ? int(this._roomList.pageIndex - 1) : int(1);
               break;
            case this._nextBtn:
               this._roomList.pageIndex = this._roomList.pageIndex + 1 <= this._roomList.pageCount ? int(this._roomList.pageIndex + 1) : int(this._roomList.pageCount);
               break;
      ...
      public function roomEnterConfirmSucceed(param1:CrazyTankSocketEvent) : void
      {
         ...
         this._roomVO = this._model.roomList[_loc4_];
         ...
         if(this._roomVO.roomType != 3)
         {
            ...
            {
               _loc2_ = AlertManager.Instance.simpleAlert(LanguageMgr.GetTranslation("AlertDialog.Info"),LanguageMgr.GetTranslation("tank.hotSpring.comfirm"),LanguageMgr.GetTranslation("ok"),LanguageMgr.GetTranslation("cancel"),false,false,false,LayerManager.BLCAK_BLOCKGOUND);
               _loc2_.moveEnable = false;
               _loc2_.addEventListener(FrameEvent.RESPONSE,this.__alertResponse);
            }
            else
            {
               _loc2_ = AlertManager.Instance.simpleAlert(LanguageMgr.GetTranslation("AlertDialog.Info"),LanguageMgr.GetTranslation("tank.hotSpring.comfirm1"),LanguageMgr.GetTranslation("ok"),LanguageMgr.GetTranslation("cancel"),false,false,false,LayerManager.BLCAK_BLOCKGOUND);
               _loc2_.moveEnable = false;
               _loc2_.addEventListener(FrameEvent.RESPONSE,this.__alertResponse);
            }
         }
