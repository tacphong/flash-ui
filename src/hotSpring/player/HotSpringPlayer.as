   ...
   public class HotSpringPlayer extends HotSpringPlayerBase
   {
      ...
      private function initPlayerName() : void
      {
         ...
         if(this._isShowName)
         {
            ...
            if(this.playerVO.playerInfo.IsVIP && Boolean(this._vipIcon))
            {
            ...
            _loc1_ = this.playerVO.playerInfo.IsVIP && Boolean(this._vipIcon) ? int(this._lblName.textWidth + this._vipIcon.width) : int(this._lblName.textWidth + 8);
            if(this.playerVO.playerInfo.IsVIP)
            {
      ...
      private function characterMirror() : void
      {
         if(Boolean(character))
         {
            ...
            character.y = this._playerVO.currentlyArea == 1 ? Number(-playerHeight + 12) : Number(-playerHeight + 63);
         }
      }
      ...
      private function fixPlayerPath() : void
      {
         ...
         {
            if(_walkPath[_loc3_].x == this._playerVO.currentWalkStartPoint.x && _walkPath[_loc3_].y == this._playerVO.currentWalkStartPoint.y)
            {
               _loc2_ = _loc3_;
               break;
            }
         ...
         if(_loc2_ > 0)
         {
            ...
            this._playerVO.walkPath = _loc1_.concat(this._playerVO.walkPath);
         }
      }
      ...
      private function setPlayer() : void
      {
         if(Boolean(this._playerVO))
         {
            if(this._playerVO.currentlyArea == 2)
            {
               ...
               {
                  this._wavePlayerAsset.scaleX = this._playerVO.playerInfo.Sex ? Number(-1.1) : Number(-1);
                  if(sceneCharacterDirection == SceneCharacterDirection.LT || sceneCharacterDirection == SceneCharacterDirection.RT)
                  {
                     this._wavePlayerAsset.x = this._playerVO.playerInfo.Sex ? Number(playerWitdh / 2 + 4) : Number(playerWitdh / 2 - 2);
                  }
                  else
                  {
                     this._wavePlayerAsset.x = this._playerVO.playerInfo.Sex ? Number(playerWitdh / 2 + 4) : Number(playerWitdh / 2);
                  }
               }
               else
               {
               ...
               if(Boolean(this._inWaterShowAsset))
               {
            ...
            {
               if(Boolean(this._wavePlayerAsset))
               {
                  if(Boolean(this._wavePlayerAsset.parent))
                  {
      ...
      override public function dispose() : void
      {
         ...
         if(Boolean(this._playerVO) && Boolean(this._playerVO.playerInfo))
         {
