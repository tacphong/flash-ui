package giftRank.views
{
   ...
   public class GiftRankView extends Sprite implements IRightView
   {
      ...
      {
         GiftRankManager.instance.view = this;
         this.initView();
         this.updateView();
         this.initTimer();
      }
      ...
      {
         this._bg = ComponentFactory.Instance.creat("wonderfulactivity.giftRank.bg");
         addChild(this._bg);
         ...
         this._rankLabelTxt.text = LanguageMgr.GetTranslation("giftRank.rankLabel",5000);
      ...
      public function updateView() : void
      {
         ...
         var _loc1_:* = 0;
         var _loc2_:GiftRankVo = null;
         var _loc3_:GmActivityInfo = GiftRankManager.instance.xmlData;
         ...
         this._checkBg.tipData = LanguageMgr.GetTranslation("giftRank.helpTxt",_loc3_.remain2);
         _loc6_ = GiftRankManager.instance.rankList;
         _loc1_ = GiftRankManager.instance.myConsume;
         this._myRank = -1;
         var _loc7_:int = 0;
         while(_loc7_ <= _loc6_.length - 1)
         {
            _loc2_ = _loc6_[_loc7_] as GiftRankVo;
         ...
         if(this._myRank >= 0)
         {
            ...
            else
            {
               ...
               _loc8_ = (_loc6_[this._myRank - 1] as GiftRankVo).consume - _loc1_ + 1;
               this._rankLabelTxt.text = LanguageMgr.GetTranslation("GiftRank.rankLabel",_loc8_);
            }
            if(GiftRankManager.instance.status == 2)
            {
               this._rankLabelTxt.visible = true;
         ...
         else
         {
            ...
            if(_loc6_.length > 0)
            {
               _loc8_ = (_loc6_[_loc6_.length - 1] as GiftRankVo).consume - _loc1_ + 1;
            }
            else
            {
               _loc8_ = 1;
            }
            this._outOfRankLabel.text = LanguageMgr.GetTranslation("gift.outOfRank");
            this._rankLabelTxt.text = LanguageMgr.GetTranslation("giftRank.outOfRankLabel",_loc1_,_loc8_);
            if(GiftRankManager.instance.status == 2)
            {
               this._rankLabelTxt.visible = true;
               this._rankLabelTxt.textColor = 16711680;
      ...
      {
         var _loc1_:GiftRankItem = null;
         var _loc2_:Array = GiftRankManager.instance.rankList;
         var _loc3_:Array = GiftRankManager.instance.xmlData.giftbagArray;
         this._vbox.removeAllChild();
         var _loc4_:int = 0;
         while(_loc4_ <= _loc2_.length - 1)
         {
            _loc1_ = new GiftRankItem(_loc4_);
      ...
      {
         var _loc1_:Date = DateUtils.getDateByStr(GiftRankManager.instance.xmlData.endTime);
      ...
      {
         GiftRankManager.instance.view = null;
         WonderfulActivityManager.Instance.delTimerFun("consumeRank");
