package cardSystem.view
{
   ...
   public class PropResetFrame extends BaseAlerFrame
   {
      ...
      private var _cardCell:cardSystem.view.ResetCardCell;
      ...
      private function initView() : void
      {
         ...
         while(_loc2_ < 16)
         {
            _loc1_ = ComponentFactory.Instance.creatComponentByStylename("PropResetFrame.PropText");
            if(_loc2_ < 4)
            {
               this._basicPropVec1[_loc2_] = _loc1_;
               this._basePropContainer1.addChild(this._basicPropVec1[_loc2_]);
            }
            else if(_loc2_ < 8)
            {
               this._basicPropVec2[_loc2_ % 4] = _loc1_;
               this._basePropContainer2.addChild(this._basicPropVec2[_loc2_ % 4]);
            }
            else if(_loc2_ < 12)
            {
               this._oldPropVec[_loc2_ % 4] = _loc1_;
               this._oldPropContainer.addChild(this._oldPropVec[_loc2_ % 4]);
            }
            else
            {
               this._newPropVec[_loc2_ % 4] = _loc1_;
               this._newPropContainer.addChild(this._newPropVec[_loc2_ % 4]);
            }
            _loc2_++;
         }
      ...
      public function show(param1:CardInfo) : void
      {
         ...
         {
            this._basicPropVec1[_loc3_].text = LanguageMgr.GetTranslation("ddt.cardSystem.PropResetFrame." + this._propertys[_loc3_].key,this._cardInfo["real" + this._propertys[_loc3_].key]);
            _loc3_++;
         }
         ...
         if(this._cardInfo.Attack != 0 || this._cardInfo.Defence != 0 || this._cardInfo.Agility != 0 || this._cardInfo.Luck || this._cardInfo.Guard != 0 || this._cardInfo.Damage != 0)
         {
            ...
            {
               this._oldPropVec[_loc2_].text = LanguageMgr.GetTranslation("ddt.cardSystem.PropResetFrame.prop",this._cardInfo[this._propertys[_loc2_].key] > 0 ? "+" + this._cardInfo[this._propertys[_loc2_].key] : this._cardInfo[this._propertys[_loc2_].key]);
               _loc2_++;
            }
         }
      ...
      private function upOldProp(param1:CardInfo) : void
      {
         var _loc2_:* = null;
         var _loc3_:int = 0;
         for(_loc2_ in this._propertyPool)
         {
            this._oldPropVec[_loc3_].text = LanguageMgr.GetTranslation("ddt.cardSystem.PropResetFrame.prop",param1[_loc2_] > 0 ? "+" + param1[_loc2_] : param1[_loc2_]);
            _loc3_++;
         }
      }
      ...
      protected function __upDate(param1:DictionaryEvent) : void
      {
         ...
         this.cardInfo = _loc4_;
         if(this._sendReplace)
         {
            this.upView();
            _loc2_ = 0;
            while(_loc2_ < this._newPropVec.length)
            {
               this._basicPropVec2[_loc2_].text = this._newPropVec[_loc2_].text = "";
               _loc2_++;
            }
            if(this._cardInfo.Attack != 0 || this._cardInfo.Defence != 0 || this._cardInfo.Agility != 0 || this._cardInfo.Luck || this._cardInfo.Guard != 0 || this._cardInfo.Damage != 0)
            {
